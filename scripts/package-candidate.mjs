...
const USAGE = "Usage: node scripts/package-candidate.mjs --run-id <run-id> --run-path <exact-run-directory> --output <new-artifact-directory> --public-pack <path> --public-pack-sha256 <sha256> [--title-overrides <path-to-json>] [--captions <path-to-json>] [--allow-strings <path-to-json>]";
...
export async function main(argv = process.argv.slice(2)) {
    ...
    process.stdout.write(`${JSON.stringify({ run_id: result.run_id, output_path: result.output_path, candidate_sha256: result.candidate_sha256 })}\n`);
    return result;
}
if (process.argv[1] && resolve(process.argv[1]) === resolve(fileURLToPath(import.meta.url))) {
    main().catch((error) => {
        process.stderr.write(`${error instanceof Error ? error.message : "Candidate packaging failed"}\n`);
        process.exitCode = 1;
    });
}
