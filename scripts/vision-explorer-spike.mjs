...
const GROUNDING_BASE_URL = process.env.MIDSCENE_MODEL_BASE_URL ??
...
const ONE_WAY_PATH = /(purchase|checkout|subscribe|billing|payment|cancel|delete|publish|send|invite|refund)/i;
...
export function classifyRequest(request, options = {}) {
    const { method, url, resourceType } = request;
    const { readOnly = false, ownOrigins = null, mine = false } = options;
...
export async function installActionBoundary(context, page, record) {
    ...
    cdp.on("Fetch.requestPaused", async ({ requestId, request, resourceType, }) => {
        const { verdict, reason } = classifyRequest({ method: request.method, url: request.url, resourceType }, { readOnly: record.readOnly, ownOrigins: record.origins, mine: record.mine });
...
// The Clerk REST shapes vary per endpoint; the callers below read one field each off the result.
export async function clerk(path, init = {}) {
    ...
    if (!res.ok)
        throw new Error(`Clerk ${path} failed: HTTP ${res.status}`);
    return body;
}
...
async function scaffoldToDragExercise(page) {
    ...
    while (Date.now() < deadline) {
        const index = await page.evaluate(() => Array.from(document.querySelectorAll('[role="radio"]')).findIndex((el) => {
            const rect = el.getBoundingClientRect();
            return (el.getAttribute("aria-disabled") !== "true" &&
                getComputedStyle(el).pointerEvents !== "none" &&
                rect.width > 0);
...
function parseJson(text) {
    ...
    if (!match)
        throw new Error(`Explorer did not return JSON: ${text.slice(0, 200)}`);
...
const onDragExercise = (page) => page
    .locator('[data-testid="build_card.answer_box"]')
    .count()
    .then((n) => n > 0);
function assembledSentence(page) {
    return page.evaluate(() => Array.from(document.querySelectorAll('[data-testid^="build_card.chip."]'))
        .filter((el) => (el.getAttribute("aria-valuetext") ??
...
// The grounded point keeps its own pair shape: older Midscene returned an {x, y} object rather
// than a tuple. A module-level function is what lets both layouts stay readable: its parameter
// carries the widened type, while an inline read keeps the tuple's flow type and the fallback
// branch stops compiling.
function pointPair(center) {
    return Array.isArray(center) ? [center[0], center[1]] : [center.x, center.y];
}
export async function execute(agent, page, decision) {
    ...
    if (decision.action === "scroll") {
        await agent.aiScroll({ direction: "down", scrollType: "once", distance: 400 }, decision.target ?? undefined);
    ...
    const pair = pointPair(located.center);
    const [x, y] = pair;
    ...
    return { executor: "midscene:aiLocate + our click", center: pair, rect: located.rect, dpr: located.dpr };
}
async function main() {
    ...
    if (!process.env.MIDSCENE_MODEL_API_KEY)
        throw new Error(`No API key for grounding family ${GROUNDING_FAMILY}`);
    ...
    try {
        ...
        if (SCAFFOLD === "drag") {
            ...
            console.log("scaffolded to the word-order drag exercise (harness clicks, not explorer decisions)");
        }
        ...
        const replay = REPLAY_FROM
            ? JSON.parse(await readFile(REPLAY_FROM, "utf8")).steps
            : null;
        ...
        for (let step = 1; step <= stepCount; step += 1) {
            ...
            try {
                ...
                executed = {
                    ...
                    failure: /failed to locate element/i.test(message)
                        ? "no-element-found"
                        : "other",
            ...
            console.log(`    ${executed.executor} ok=${executed.success} ${visibleChange ? "screen CHANGED" : "screen unchanged"} | grounding calls=${grounding.calls}${grounding.cache_hit ? " (CACHE HIT)" : ""} tok=${grounding.prompt_tokens}/${grounding.completion_tokens}${atDrag ? ` | answer row: "${dragVerified}"` : ""}${executed.message ? ` | ${executed.message}` : ""}`);
            if (decision.done)
                break;
        }
        ...
        trace.error = String(error instanceof Error ? error.stack ?? error : error);
    }
    finally {
        await agent?.destroy().catch(() => { });
        await browser.close().catch(() => { });
        await clerk(`/users/${user.id}`, { method: "DELETE" }).catch(() => console.log(`WARNING: could not delete ${user.id}`));
    }
    const tracePath = join(runDir, "trace.json");
    await writeFile(tracePath, JSON.stringify(trace, null, 2));
    console.log(`\ntrace: ${tracePath}`);
    console.log(`explorer tokens: ${trace.explorer_usage?.input ?? 0} in / ${trace.explorer_usage?.output ?? 0} out`);
    if (trace.error)
        process.exitCode = 1;
}
if (process.argv[1]?.endsWith("vision-explorer-spike.mjs"))
    await main();
