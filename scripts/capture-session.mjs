...
function parseArgs(argv) {
    const args = Object.fromEntries(Array.from({ length: argv.length / 2 }, (_, index) => argv.slice(index * 2, index * 2 + 2)));
    if (!args["--url"] || !args["--name"] || !NAME_PATTERN.test(args["--name"])) {
        throw new Error("Usage: node scripts/capture-session.mjs --url <login-url> --name <session-name> [--output-dir <dir>]");
    }
...
async function findVisibleChromium() {
    ...
    throw new Error("No visible Chromium build found under ~/Library/Caches/ms-playwright; set CAPTURE_SESSION_CHROMIUM_EXECUTABLE");
}
async function connectCdp(profileDirectory) {
    const version = await (await fetch(`http://127.0.0.1:${await waitForDevToolsPort(join(profileDirectory, "DevToolsActivePort"))}/json/version`)).json();
    const socket = new WebSocket(version.webSocketDebuggerUrl);
    await new Promise((resolvePromise, reject) => {
        socket.addEventListener("open", () => resolvePromise(), { once: true });
        socket.addEventListener("error", () => reject(new Error("Could not connect to the visible browser's DevTools socket")), { once: true });
    });
...
async function launchRealChromeWithProfile(url, profileName = "Profile 5", { chromeUserDataDir = join(process.env.HOME ?? "", "Library/Application Support/Google/Chrome"), } = {}) {
    const executable = process.env.CAPTURE_SESSION_REAL_CHROME_EXECUTABLE ??
...
async function saveSession(cdp, sessionId, { url, name, outputDir }) {
    ...
    console.log(`  node scripts/prepare-target-run.mjs --run-id <run-id> --mode source-blind-bounded-onboarding-v1 --target-origin <origin> --saved-session ${outputPath}`);
    return outputPath;
}
async function captureSession({ url, name, outputDir, }) {
    ...
    console.log("Log in by hand. Use a disposable/test account where you can -- whatever account you use,");
    console.log("this tool will gain the exact same powers that account has (it acts as that logged-in user).");
    console.log("Never use a personal or admin account if a test account is available.");
    console.log("");
    console.log(`When you are fully logged in, come back here and press Enter. The captured cookies and`);
...
export { captureSession, connectCdp, dumpStorage, findVisibleChromium, launchRealChromeWithProfile, launchVisibleBrowser, parseArgs, saveSession, };
