...
const DEFAULT_CACHE = resolve(new URL("../.runtime/screen-caption-cache.json", import.meta.url).pathname);
...
export function captionProvider(env = process.env) {
    ...
    if (env.CAPTION_BASE_URL || env.CAPTION_API_KEY) {
        if (!env.CAPTION_BASE_URL || !env.CAPTION_API_KEY)
            throw new Error(`CAPTION_BASE_URL and CAPTION_API_KEY must be set together: ${env.CAPTION_BASE_URL ? "CAPTION_API_KEY" : "CAPTION_BASE_URL"} is missing`);
        return {
    ...
    throw new Error("No key to caption screens with: set ANTHROPIC_API_KEY (the key `releashed map` already needs), or GEMINI_API_KEY, or CAPTION_BASE_URL + CAPTION_API_KEY for any OpenAI-compatible vision host");
}
const PROMPT = [
...
const SELF_TALK = /\b(?:let'?s|let me|i'll|i'd|i've|i should|i need|i will|we need|we should|make sure|wait|counting|check(?:ing)?\s+(?:the|that|this|my|our|its)\s+(?:answer|caption|constraints?|instructions?|rules?|phrase|count))\b/i;
const PROMPT_RULE_TALK = /\bsentence case\b|\bno full stop\b|\bword count\b|\bnoun phrase\b|\bcapital(?:ize|ization)?\b|\blowercase\b|\buppercase\b|\bproper noun\b|\bpunctuation\b/i;
const CHECKLIST_SHAPE = /:\s*$|:\s*(?:PASS|FAIL|Yes|No)\b|^\s*[*-]\s|^\s*\d+[.)]\s|\[\d\]|\(\d\)|-?>\s*\d+\s+words?\b|=\s*\d+\s+words?\b/im;
...
export function cleanCaption(raw) {
    ...
    for (const candidate of candidates) {
        ...
        const text = /"$/.test(cut) && (cut.match(/"/g)?.length ?? 0) % 2 === 1 ? cut.slice(0, -1).trim() : cut;
        if (text.length >= 12 &&
...
function usage() {
    throw new Error("Usage: node scripts/caption-screens.mjs --trace <observations.jsonl> --output <captions.json> [--cache <path>]");
}
function parseArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 2) {
        if (!["--trace", "--output", "--cache"].includes(argv[index]) || values[argv[index]])
            usage();
...
export async function captionScreens({ tracePath, outputPath, cachePath = DEFAULT_CACHE, }) {
    ...
    for (const [hash, shot] of shots) {
        ...
        try {
            ...
            failures += 1;
            process.stderr.write(`caption failed for ${shot.path}: ${error instanceof Error ? error.message : String(error)}\n`);
        }
    }
    await mkdir(dirname(cachePath), { recursive: true });
    await writeFile(cachePath, `${JSON.stringify(cache, null, 2)}\n`, { mode: 0o600 });
    await mkdir(dirname(resolve(outputPath)), { recursive: true });
    await writeFile(outputPath, `${JSON.stringify({ schema_version: 1, model: provider.model, kind: "screen-descriptions", captions }, null, 2)}\n`, { mode: 0o600 });
    return {
        screens: shots.size,
