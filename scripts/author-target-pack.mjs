...
function labels(html, tag, limit) {
    ...
    for (const match of html.matchAll(pattern)) {
        ...
        if (text && text.length <= 60 && !/^[\s\W]*$/.test(text))
            found.add(text);
        if (found.size >= limit)
            break;
    }
...
function claimsFrom(html, sourceId) {
    ...
    const add = (id, kind, text) => {
        if (typeof text === "string" && text)
            claims.push({ id, kind, text, source_ids: [sourceId] });
    };
    ...
    add("page-description", "promise", description && `The page's own meta description reads: "${description}".`);
    add("headline", "audience", headings.length && `The page's top heading${headings.length > 1 ? "s read" : " reads"}: ${headings.map((h) => `"${h}"`).join(", ")}.`);
    add("link-entry-points", "entry-point", linkLabels.length &&
        `The landing page's own links are labelled: ${linkLabels.map((l) => `"${l}"`).join(", ")}.`);
    add("button-controls", "capability", buttonLabels.length &&
...
export async function authorTargetPack(url, fetchImpl = fetch) {
    ...
    if (!response.ok)
        throw new Error(`${initialUrl} answered HTTP ${response.status}`);
    const raw = Buffer.from(await response.arrayBuffer());
    if (raw.byteLength > MAX_RAW_BYTES)
        throw new Error("Landing page is implausibly large for a public pack");
    ...
    if (claims.length < 2)
        throw new Error(`${finalUrl} served no readable title, description, headings, links or buttons -- nothing citable to build a public pack from`);
...
if (process.argv[1]?.endsWith("author-target-pack.mjs")) {
    ...
    if (!url)
        throw new Error("Usage: node scripts/author-target-pack.mjs <https url> [--out packs/<dir>]");
    const outIndex = rest.indexOf("--out");
    const outPath = outIndex >= 0 ? rest[outIndex + 1] : undefined;
    const stamp = new Date().toISOString().replace(/[-:]/g, "").replace(/\.\d+Z$/, "Z");
    const directory = outPath
        ? resolve(outPath)
        : resolve(import.meta.dirname, "..", "packs", `public-pack-${new URL(url).hostname.replace(/[^a-z0-9]+/gi, "-")}-${stamp}`);
