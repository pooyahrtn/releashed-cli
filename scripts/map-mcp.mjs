...
async function containedScreenshot(candidateDir, reference) {
    ...
    if (isAbsolute(rel) || !rel.startsWith(`screenshots${sep}`))
        return null;
    return path;
}
...
export async function loadMap(candidateDir) {
    ...
    if (start < 0)
        throw new Error(`no rendered map in ${candidateDir}/map.html`);
    ...
    const legacyStop = isRecord(provenance.stop) ? provenance.stop.screenshot_path : undefined;
    return {
        screens,
        transitions,
        findings: [],
        provenance,
        claimScreenshotPath: await containedScreenshot(candidateDir, legacyStop),
...
export function flowNames({ screens }) {
    return [...new Set(screens.map((screen) => screen.flow).filter((flow) => Boolean(flow)))];
}
...
export function flowTransitions(map, screens) {
    ...
    for (const edge of map.transitions) {
        ...
        if (fromIn && toIn)
            ...
            entering.push({ ...edge, from_flow: flowOf(edge.from) });
        else if (fromIn)
            leaving.push({ ...edge, to_flow: flowOf(edge.to) });
    }
...
const TOOLS = [
    {
        name: "list_screens",
        description: "List every screen in the map of this product: id, what the screen is, its URL, the flow it belongs to, and how many transitions leave it.",
    ...
    {
        name: "find_screen",
        description: "Find a screen by what a user would call it -- \"the checkout page\", \"where you pick a plan\". Returns the best matching screens, most likely first.",
        inputSchema: {
            type: "object",
    ...
    {
        name: "get_flow",
        description: "Get a whole flow by name -- \"onboarding\", \"checkout\" -- every screen in it, each screen's picture, the transitions between them, and the boundary: how you enter the flow and where it leads out. On a miss, the list of flow names in this map so you can retry.",
        inputSchema: {
            type: "object",
    ...
    },
    {
        ...
        description: "List the observed transitions between screens: the action taken and the screen it led to. Pass a screen id for the ones leaving that screen, or nothing for all of them.",
        inputSchema: {
            type: "object",
...
export async function handleCall(map, name, args = {}) {
    if (name === "list_screens")
        return text(screenSummaries(map));
    if (name === "list_transitions")
        return text(transitionsFrom(map, args.id));
    if (name === "find_screen")
        return text(findScreens(map, args.query));
    ...
    if (name === "get_screen") {
        ...
        if (!screen)
            return { isError: true, content: [{ type: "text", text: `no screen ${args.id}` }] };
    ...
    if (name === "get_flow") {
        ...
        if (screens.length === 0) {
            return text({
                ...
                note: "This map is one walk of the product, not the whole product. A flow can be absent because" +
        ...
        if (map.provenance?.directed_by)
            Object.assign(header, map.provenance);
        const content = [{ type: "text", text: JSON.stringify(header, null, 2) }];
        for (const screen of screens)
            content.push(...(await screenContent(screen)));
        const claimStop = isRecord(map.provenance?.stop) ? map.provenance.stop : undefined;
        if (claimStop?.screenshot_path) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    server.setRequestHandler(CallToolRequestSchema, (request) => handleCall(map, request.params.name, request.params.arguments ?? {}));
