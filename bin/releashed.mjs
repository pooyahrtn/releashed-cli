...
import { BROWSER_REFUSAL, MIN_NODE_MAJOR, missingModelKeys, nodeRefusal, spendNotice, } from "../lib/first-run.mjs";
...
import { alternativesFromStore, formatAlternatives, } from "../lib/goal-alternatives.mjs";
...
export async function writeAgentsBlock(candidateDir, directory = process.cwd(), log = console.log, { force = false, dryRun = false, reason = null, } = {}) {
...
export async function mapCommand(options, deps = {}) {
    ...
    const alternatives = options.goal && handoff && !handoff.reached
        ...
        : null;
    if (alternatives)
        for (const line of formatAlternatives(alternatives))
            log(line);
...
export async function mapPreflight(deps = {}) {
    const { env = process.env, keys = true, nodeVersion = process.versions.node, browser = async () => {
    ...
    if (outdatedNode)
        problems.push(outdatedNode);
    if (keys)
        problems.push(...missingModelKeys(env));
    try {
        await browser();
    }
    catch {
...
export async function doctor(deps = {}) {
    ...
    await check(`node ${MIN_NODE_MAJOR} or newer`, () => {
    ...
    "GEMINI_API_KEY works (points at controls; Midscene has no Anthropic grounding family)", async () => {
        if (!hasGeminiKey())
            throw new Error("not set");
    ...
    await check("chromium installed", browser, 
    ...
    log(failed.length === 0
        ? `\nall good -- try: releashed map https://example.com\n${spendNotice(40, "sonnet", null, "that default 40-step run")}`
...
export async function main(argv = process.argv.slice(2)) {
    ...
    if (options.command === "map") {
        ...
        if (problems.length)
            throw new Error(`releashed map cannot start. ${problems.length} thing${problems.length === 1 ? "" : "s"} to fix first:\n\n${problems.join("\n\n")}\n\nConfirm the fix, including whether the keys have credit, with: releashed doctor`);
