...
export function explorerModel(
  ...
  if (!apiKey)
    throw new Error(
      "SPIKE_EXPLORER_API_KEY is required when SPIKE_EXPLORER_BASE_URL is set",
    );
  return createOpenAICompatible({ name: "explorer", baseURL, apiKey }).chatModel(
    modelId,
  );
}
...
export const explorerMaxOutputTokens = (
  fallback: number,
  env: NodeJS.ProcessEnv = process.env,
): number => Number(env.SPIKE_EXPLORER_MAX_TOKENS) || fallback;
...
export async function callExplorer({
  ...
  if (typeof text !== "string" || text.length === 0)
    throw new Error(
      `Explorer model returned no text (finish reason ${JSON.stringify(result.finishReason)})`,
    );
