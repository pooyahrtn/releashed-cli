...
import { resolveLocatorConfig } from "./caller-locator.mjs";
import { inspectProspectiveOutput } from "./candidate-packager.mjs";
const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const require = createRequire(import.meta.url);
async function executable(command, cwd, env) {
    ...
    throw new Error("Configured executable unavailable; use the installed releashed executable or an absolute Node path.");
}
function storeRoot(cwd, env) {
    if (env.RELEASHED_OUT)
        return resolve(cwd, env.RELEASHED_OUT);
    try {
...
export async function captureDoctor(options, { parseArgs, cwd = process.cwd(), env = process.env, browser = async () => {
    ...
    const check = async (name, fn) => {
        try {
            ...
            checks.push({
                name,
                ok: false,
                detail: error instanceof Error ? error.message : "Local check failed.",
            });
            return false;
        }
    };
    ...
    await check("Chromium", async () => {
        try {
            ...
            throw new Error("Chromium unavailable for this installation; run releashed install-browser with this package.");
        }
    ...
    await check("preparation record", async () => {
        ...
        try {
            ...
            throw new Error(`Create a readable, nonempty preparation/attempt record at ${path} before authentication.`);
        }
    ...
    let run;
    let out;
    if (!(await check("MCP configuration", async () => {
        let config;
        try {
            config = JSON.parse(await readFile(options.mcpConfig, "utf8"));
        }
        catch {
            throw new Error(`Read a valid JSON MCP configuration from ${options.mcpConfig}.`);
        }
        server = config?.mcpServers?.[options.server];
        if (!server || typeof server !== "object")
            throw new Error(`Select an existing mcpServers entry with --server in ${options.mcpConfig}.`);
        if (server.disabled ||
            (server.type && server.type !== "stdio") ||
            server.url)
            throw new Error("Selected server must be an enabled local stdio entry; a remote/disabled instance cannot be checked here.");
        if (typeof server.command !== "string" ||
            !Array.isArray(server.args) ||
            server.args.some((value) => typeof value !== "string"))
            throw new Error("Selected mcpServers entry needs command and a string args array.");
        if (server.env &&
        ...
        if (server.cwd && resolve(cwd, server.cwd) !== resolve(cwd))
            throw new Error("Run doctor from the same working directory as the selected server.");
        ...
        const inspectedValues = [
            ...
            ...server.args.filter((_, index) => server.args[index - 1] !== "--auth-cmd"),
        ];
        if (inspectedValues.some((value) => value?.includes("${")))
            throw new Error("Resolve client variable substitutions in paths/run options before checking this configuration; their values are not known here.");
        serverEnv = { ...env, ...server.env };
        return `Selected entry in ${options.mcpConfig}; command and environment values withheld.`;
    })))
        return result();
    if (!(await check("MCP executable", async () => {
        ...
        if (commandPath === nodePath) {
            if (!server.args[0] || server.args[0].startsWith("-"))
                throw new Error("Use node <installed-package>/bin/releashed.mjs explore-mcp ... without Node wrappers/options.");
            ...
            if ((await executable("node", cwd, serverEnv)) !== nodePath)
                throw new Error("The server PATH selects a different Node; run doctor with that same Node installation.");
        }
        let actual;
        try {
            ...
            throw new Error("Configured CLI entry does not exist; point to the installed bin/releashed.mjs.");
        }
        if (actual !== (await realpath(join(ROOT, "bin", "releashed.mjs"))))
            throw new Error("Selected server uses a different executable/package. Run doctor from that installation or correct the server command; wrappers such as npx are not resolved.");
        // Reuse the CLI's parser but never echo a parser error: it may contain an auth command.
        try {
            run = parseArgs(args);
        }
        catch {
            throw new Error("Configured arguments are invalid; check explore-mcp --goal, --steps and capture metadata in the selected entry.");
        }
        out = storeRoot(cwd, serverEnv);
        return `${actual}; local output root ${out}`;
    })))
        return result();
    if (!(await check("output root", async () => {
        ...
        try {
            ...
            if (/private input area/.test(reason)) {
                throw new Error(`Selected server "${options.server}" output root is inside a private input area ` +
            ...
            throw new Error(`Selected server "${options.server}" output root passes through an unusable path component ` +
    ...
    await check("directed request", async () => {
        if (run.command !== "explore-mcp" || !run.goal?.trim())
            throw new Error("Selected server is not a directed explore-mcp capture; configure --goal on a dedicated instance.");
        if (!run.mine)
            throw new Error("Directed owner capture needs --mine in the selected server arguments.");
        if (!run.url || new URL(run.url).href !== new URL(options.url).href)
            throw new Error("Configured URL does not match the requested product URL.");
        if (run.goal !== options.goal)
            throw new Error("Configured --goal does not match the requested goal; bind the instance for this request.");
    ...
    await check("target locator", async () => {
        ...
        if (!resolution.enabled)
            throw new Error(`Target locator is not configured for the selected server: ${resolution.reason}. ` +
    ...
    await check("identity and precondition", async () => {
        if (!run.identityLabel?.trim() || !run.precondition?.trim())
            throw new Error("Declare --identity-label and --precondition in the dedicated capture entry after verifying the actual account prerequisites.");
        try {
            ...
            throw new Error(`Capture metadata is invalid or a referenced run is absent from ${out}/runs and ${out}/maps; check --continues, --precondition and --identity-label.`);
        }
    ...
    await check("sign-in configuration", async () => {
        if (run.authCmd?.trim())
            return "Auth helper configured; not executed.";
        if (!run.login)
            throw new Error("Signed-in capture needs --auth-cmd or --login in the selected entry.");
        if (!run.url)
            throw new Error("Saved-session capture needs a product URL in the selected entry.");
        const path = join(out, "sessions", `${new URL(run.url).hostname.replace(/[^a-z0-9]+/gi, "-")}.json`);
        try {
            await access(path, constants.R_OK);
            if (!(await stat(path)).isFile())
                throw new Error();
            ...
            throw new Error(`Saved session unavailable at ${path}; use the product's supported setup before capture.`);
        }
