const CLERK_TICKET_TTL_SECONDS = 60;
...
const CLERK_TICKET_EXCHANGE_FAILURE_CODES = Object.fromEntries(
...
export function isSafeClerkAuthFailureCode(value) {
  return SAFE_CLERK_AUTH_FAILURE_CODES.has(value);
}
...
export function safeClerkBoundedRouteFailure(stage, code, point = null) {
  ...
  return { stage, code, point: SAFE_CLERK_BOUNDED_ROUTE_FAILURE_POINTS.has(point) ? point : null };
}
...
export class OneShotClerkPageAuthenticator {
  ...
  async authenticate({ ticket, expectedUserId, expectedCurrentHref }) {
    if (this.attempted) return refusal("clerk_auth_locked", "Clerk authentication is already locked");
    this.attempted = true;
    if (
      ...
      if (!(await this.lockMethods())) return refusal("clerk_auth_failed", "Clerk authentication failed");
    ...
    if (!ambiguous) {
      ...
      try {
        ...
        if (Object.hasOwn(CLERK_PAGE_AUTH_FAILURE_CODES, failureStage)) {
    ...
    if (!(await this.lockMethods())) return refusal("clerk_auth_failed", "Clerk authentication failed");
    if (!validAuthState(state, expectedUserId)) {
      return refusal(
        ambiguous ? "clerk_auth_outcome_unknown" : "clerk_auth_failed",
        ambiguous ? "Clerk authentication timed out and could not be confirmed" : "Clerk authentication failed"
      );
    }
    return {
      ok: true,
...
export function isPermittedClerkMutation({ rawUrl, method, frontendApiOrigin, phase, activeSessionId = null }) {
  ...
  if (url.origin !== frontendApiOrigin || url.username || url.password || url.hash) return false;
  ...
  if (url.pathname === "/v1/environment" && url.searchParams.get("_method") === "PATCH" && url.searchParams.has("__clerk_db_jwt")) return true;
  if (phase === "bootstrap") {
    return url.pathname === "/v1/client/sign_ins";
  }
  if (["activating", "active"].includes(phase) && /^[A-Za-z0-9_-]{4,256}$/.test(activeSessionId ?? "")) {
...
export function provisionalClerkBootstrapTouchSessionId({ rawUrl, method, frontendApiOrigin, phase }) {
  if (phase !== "bootstrap" || String(method).toUpperCase() !== "POST") return null;
  let url;
  try {
    url = new URL(rawUrl);
  } catch {
  ...
  if (url.origin !== frontendApiOrigin || url.username || url.password || url.hash) return null;
  return /^\/v1\/client\/sessions\/([A-Za-z0-9_-]{4,256})\/touch$/.exec(url.pathname)?.[1] ?? null;
}

export { CLERK_TICKET_TTL_SECONDS };
