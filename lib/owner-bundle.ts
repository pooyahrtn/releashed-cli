...
} from "../supervisor/clerk-bootstrap.ts";
import { FIRST_OBSERVATION_AUTHORITY, FRESH_START_RECORD_KIND, validateFreshStartContract } from "./fresh-start-contract.ts";
...
type OwnedDirectory = { path: string; identity: FileIdentity };
type FileSnapshot = {
	path: string;
	bytes: Buffer;
	sha256: string;
...
type FileOwnership = Map<string, FileIdentity>;
type BundleStateDocument = {
	clerk_frontend_api_origin?: unknown;
	base_control_hashes?: Record<string, string>;
};
type ProviderRecovery = { state: unknown; external_id: unknown; user_id: unknown };

type InspectedBundle =
	| { complete: true; preProvision: false; pendingCompletion: false }
	| { complete: false; preProvision: true; pendingCompletion: false; root: OwnedDirectory; owned: FileOwnership }
	| {
		complete: false;
		preProvision: false;
		pendingCompletion: true;
		root: OwnedDirectory;
		state: BundleStateDocument;
		recovery: ProviderRecovery;
		account: { user_id: unknown; username: unknown; external_id: unknown };
...
type RecoveryMaterial = { state: string; external_id: string; user_id: string | null };
type IdentityCleanup = { cleanup_complete: boolean };
type ProvisionedIdentity = {
	userId: string;
	cleanup: (options: { browserStopped: boolean }) => Promise<IdentityCleanup>;
};
// provisionDisposableClerkIdentity is still untyped JavaScript whose inferred
// options type the recovery callback as () => Promise<void>, so a callback that
// reads its material is not assignable to the inferred signature. The structural
// type below states the callback this module actually passes; the fallback import
// is bound to it once, so the later call is checked against one signature.
type ProvisionIdentity = (options: {
	environment: { CLERK_SECRET_KEY: string | undefined };
	expectedFrontendOrigin: string;
	fetchImpl: typeof fetch;
	onRecoveryMaterial: (material?: RecoveryMaterial) => Promise<void>;
}) => Promise<ProvisionedIdentity>;
type OwnerBundleDependencies = {
	...
	now?: () => Date;
	beforeComplete?: (context: { bundle: string }) => Promise<void> | void;
};

const sha256 = (bytes: Buffer) => createHash("sha256").update(bytes).digest("hex");
const body = (value: unknown) => Buffer.from(`${JSON.stringify(value, null, 2)}\n`);
const identity = (stat: Stats): FileIdentity => ({ dev: stat.dev, ino: stat.ino, mode: stat.mode & 0o777 });
const sameIdentity = (stat: Stats | null, expected: FileIdentity): stat is Stats => Boolean(
...
const exactNames = (actual: string[], expected: string[]) => JSON.stringify([...actual].sort()) === JSON.stringify([...expected].sort());

function refused(): never { throw new Error("Owner bundle creation did not complete"); }
...
async function directory(path: string, { privateMode = false }: { privateMode?: boolean } = {}) {
...
async function assertDirectory(directory: OwnedDirectory) {
...
async function syncDirectory(directory: OwnedDirectory) {
...
async function privateSnapshot(path: string, boundary: () => Promise<void> = async () => {}) {
	await boundary();
...
async function assertSnapshot(snapshot: FileSnapshot, boundary: () => Promise<void>) {
...
async function writeOwned(path: string, bytes: Buffer, owned: FileOwnership, { boundary, parent, replace = false }: { boundary: () => Promise<void>; parent: OwnedDirectory; replace?: boolean }) {
	await boundary();
	if (replace) {
...
async function unlinkOwned(path: string, expected: FileIdentity, { boundary, parent }: { boundary: () => Promise<void>; parent: OwnedDirectory }) {
	await boundary();
...
async function removeOwned(bundle: string, bundleRoot: OwnedDirectory, parent: OwnedDirectory, owned: FileOwnership) {
...
function clerkOrigin(policy: { status?: unknown; browser_request_dispatch?: { allow?: Array<{ purpose?: unknown; origin?: unknown }> | null } | null }) {
...
function stateValue(bytes: Buffer, runId: string) {
	let value;
...
function recoveryValue(bytes: Buffer, { runId, stateSha256 }: { runId: string; stateSha256: string }) {
	let value;
	try { value = JSON.parse(bytes.toString("utf8")); } catch { refused(); }
	if (
...
function accountValue(value: { provisioned_at?: unknown }, { userId, username }: { userId: string; username: string }) {
...
function completionValue(bytes: Buffer, { runId, stateSha256 }: { runId: string; stateSha256: string }) {
	let value;
	try { value = JSON.parse(bytes.toString("utf8")); } catch { refused(); }
	if (
...
const providerRecovery = (value: ProviderRecovery) => ({ state: value.state, external_id: value.external_id, user_id: value.user_id });

async function inspectExisting(bundle: string, runId: string, parent: OwnedDirectory): Promise<InspectedBundle> {
	...
	if (final || finalWithRecovery) {
		...
		let contract;
		try {
			originPolicy = JSON.parse(snapshots.get("origin-allowlist.json")!.bytes.toString("utf8"));
			account = JSON.parse(snapshots.get("account.json")!.bytes.toString("utf8"));
			contract = JSON.parse(snapshots.get("fresh-start-contract.json")!.bytes.toString("utf8"));
		} catch { refused(); }
		...
		return { complete: true, preProvision: false, pendingCompletion: false };
	}
	...
	return {
		...
		owned: new Map([...snapshots.values()].map((snapshot): [string, FileIdentity] => [snapshot.path, snapshot.identity])),
	};
}

async function snapshotBase(base: OwnedDirectory) {
	...
	let policy;
	try { policy = JSON.parse(snapshots["origin-allowlist.json"].bytes.toString("utf8")); } catch { refused(); }
...
export async function createImmutableOwnerBundle({ baseOwnerDirectory, bundleParent, runId, secretKey, fetchImpl = fetch, dependencies = {} }: {
	baseOwnerDirectory: string;
	bundleParent: string;
	runId: string;
	secretKey: string;
	fetchImpl?: typeof fetch;
	dependencies?: OwnerBundleDependencies;
}) {
	...
	await syncDirectory(parent);
	const boundary = async () => { await assertDirectory(parent); await assertDirectory(bundleRoot); };
	const owned: FileOwnership = new Map();
	...
	const provision: ProvisionIdentity = dependencies.provisionIdentity ?? provisionDisposableClerkIdentity;
	let lifecycle: ProvisionedIdentity | null = null;
	try {
		for (const name of BUNDLE_CONTROL_FILES) await assertSnapshot(snapshots[name], () => assertDirectory(base));
		lifecycle = await provision({
		...
		if (lifecycle) {
			...
			if (error && typeof error === "object") {
				if ("cleanup" in error) cleanup = error.cleanup;
				const cleanupHandle = "cleanup_handle" in error ? error.cleanup_handle : null;
				if (typeof cleanupHandle === "function") cleanup = await cleanupHandle().catch(() => cleanup);
			}
			if (cleanup !== null && typeof cleanup === "object" && "cleanup_complete" in cleanup && cleanup.cleanup_complete === true) await removeOwned(bundle, bundleRoot, parent, owned).catch(() => {});
		}
		refused();
	}
}
