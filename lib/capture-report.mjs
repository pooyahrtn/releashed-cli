...
import { findCapture } from './capture-memory.mjs';
...
async function record(path, name) {
    ...
    if (!text.trim())
        throw new Error(`${name} record is empty: ${resolved}`);
    return { path: resolved, text };
}
function object(text, name) {
...
export async function captureReport(options) {
    const { mapsRoot, runId, inspection } = options;
    assertRunId(runId);
    if (!inspection?.trim())
        throw new Error('Inspect the selected original images, then supply --inspection describing what they show.');
    ...
    if (external.run_id !== runId)
        throw new Error(`phases.run_id must be ${runId}.`);
    ...
    if (times.some((time, index) => index > 0 && time < times[index - 1]))
        throw new Error('Recorded phase timestamps must be chronological.');
    const costs = external.costs;
    if (!costs || typeof costs !== 'object' || Array.isArray(costs))
        throw new Error('phases.costs needs caller, capture, grounding and product descriptions; write unknown when unmeasured.');
    const entries = costs;
    for (const key of ['caller', 'capture', 'grounding', 'product']) {
        if (typeof entries[key] !== 'string' || !entries[key].trim())
            throw new Error(`phases.costs.${key} needs an amount/basis or explicit unknown.`);
    }
...
async function blockedEvidence(candidateDir) {
    let manifest;
    try {
        manifest = JSON.parse(await readFile(join(candidateDir, 'manifest.json'), 'utf8'));
    ...
    if (!blocked || typeof blocked !== 'object')
        return null;
    if (typeof blocked.reason !== 'string' || !blocked.reason.trim())
        return null;
    if (typeof blocked.screenshot_path !== 'string' || !blocked.screenshot_path)
        return null;
    if (typeof blocked.screenshot_sha256 !== 'string' || !/^[0-9a-f]{64}$/.test(blocked.screenshot_sha256))
        return null;
    if (typeof blocked.observed_at !== 'string' || !Number.isFinite(Date.parse(blocked.observed_at)))
        return null;
    const entry = manifest.files.find((file) => file.path === blocked.screenshot_path);
    if (!entry || entry.sha256 !== blocked.screenshot_sha256)
        return null;
    const bytes = await sealedFile(candidateDir, manifest, blocked.screenshot_path);
    if (!bytes || !bytes.length || digest(bytes) !== blocked.screenshot_sha256)
        return null;
    return {
