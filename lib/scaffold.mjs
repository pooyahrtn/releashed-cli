...
export function normalizeOrigin(rawUrl) {
    ...
    if (url.username || url.password)
        throw new Error("URL userinfo is forbidden");
    return url.origin;
}
...
export function rollingCount(timestamps, now, seconds = 60) {
    ...
    while (timestamps.length > 0 && timestamps[0] <= floor)
        timestamps.shift();
...
export function modelCostEur(model, usage) {
    ...
    if (!price)
        return null;
    const kinds = ["input", "cacheWrite", "cacheRead", "output"];
    const usd = kinds.reduce((sum, kind) => sum + (usage?.[kind] ?? 0) * price[kind], 0) /
        1e6;
