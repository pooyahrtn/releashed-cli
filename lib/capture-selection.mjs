...
export function recordedSelection(events, paths) {
    if (!Array.isArray(paths) || paths.length === 0)
        throw new Error("goal_screenshots must contain at least one recorded screenshot");
    ...
    return paths.map((path) => {
        ...
        if (typeof path !== "string" || !observation)
            throw new Error(`screenshot ${String(path)} is not retained by the recorded trace`);
        if (observation.order <= previous)
            throw new Error("goal_screenshots must be unique and in recorded observation order");
...
export async function verifySelection(candidateDir, manifest, traceBytes, paths) {
    ...
    for (const item of selected) {
        ...
        if (!item.screenshot_path.startsWith("screenshots/") ||
            !bytes ||
...
export async function selectCapture({ mapsRoot, runId, screenshots, why, }) {
    ...
    if (!mapBytes || !trace)
        throw new Error(`map.json or observations.jsonl failed verification in ${candidateDir}`);
