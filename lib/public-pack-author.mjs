...
export function validatePublicBrief(output, sourceIds, sources) {
    ...
    try {
        ...
        throw new Error("Public pack author response is not JSON");
    }
    const pack = { schema_version: 1, product_summary: brief?.product_summary, claims: brief?.claims, sources };
    validatePublicPack(pack);
    const checked = pack;
    if (!checked.claims.every((claim) => claim.source_ids.every((sourceId) => sourceIds.has(sourceId))))
        throw new Error("Public pack claim cites an unknown source");
    return checked;
}
export async function authorPublicPack({ mountDirectory, outputDirectory, limits, requestModel }) {
    ...
    if (!response?.ok || !Number.isFinite(response.actual_eur) || !Number.isFinite(response.usage?.input_tokens) || !Number.isFinite(response.usage?.output_tokens) || response.model_identity?.approved !== true)
        throw new Error("Brokered public pack lacks reconciled usage or approved model identity");
