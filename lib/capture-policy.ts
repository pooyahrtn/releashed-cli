...
export type FreshnessOptions = { maxAgeDays?: number; fresh?: boolean; now?: Date; env?: NodeJS.ProcessEnv };
export type CapturePolicy = { max_age_days: number; fresh_requested: boolean; checked_at: string; config_path: string; source: 'request' | 'environment' | 'store' | 'default' };
const DAY_MS = 86_400_000;

function ageLimit(value: unknown): number {
...
export async function capturePolicy(mapsRoot: string, options: FreshnessOptions = {}): Promise<CapturePolicy> {
  ...
  try {
    ...
    stored = ageLimit((raw as { max_age_days?: unknown }).max_age_days);
  } catch (error) {
  ...
  if (rawFresh !== undefined && !['0', '1'].includes(rawFresh)) throw new Error('RELEASHED_FRESH must be 0 or 1.');
  if (options.fresh !== undefined && typeof options.fresh !== 'boolean') throw new Error('fresh must be boolean.');
  const now = options.now ?? new Date();
  if (!Number.isFinite(now.valueOf())) throw new Error('Invalid freshness clock.');
  return {
...
export function evidenceFreshness(dates: Array<string | null>, policy: CapturePolicy) {
