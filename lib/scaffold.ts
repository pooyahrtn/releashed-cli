...
export function sha256Text(value: BinaryLike): string {
...
export async function withFileLock<T>(
  ...
  for (;;) {
    try {
      ...
      if ((monotonicSeconds() - started) * 1000 >= timeoutMs)
        throw new Error("cap-state lock unavailable; fail closed");
...
// The state is an unversioned JSON document mutated in place by file-specific
// callers, so it stays dynamically typed at this boundary.
export async function mutateCapState<T>(
  path: string,
  mutation: (state: any) => Promise<T> | T,
): Promise<T> {
...
export function sanitizeUrl(rawUrl: string): string {
  ...
  const suspicious =
    /(^[A-Za-z0-9_-]{24,}$)|(@)|(^[0-9a-f]{8}-[0-9a-f-]{27,}$)|(^eyJ)/i;
...
export function isAuthFlowUrl(rawUrl: string): boolean {
  const url = new URL(rawUrl);
  return /\/(auth|oauth|sso|sign-?in|sign-?up|callback|verify|tokens?)(\/|$)/i.test(
    url.pathname,
  );
}
...
export type CapLimits = {
  ...
  browser_operations_total: number;
  browser_operations_per_rolling_minute: number;
  browser_requests_total: number;
  browser_requests_per_rolling_minute: number;
  listed_one_way_actions_total: number;
  app_side_cost_cap_eur: number;
  model_cost_cap_eur: number;
  combined_actual_plus_reserved_cap_eur: number;
};

export function freshCapState(
  limits: CapLimits,
  ...
  return {
    ...
    caps: {
      ...
      browser_operations_per_rolling_minute:
        limits.browser_operations_per_rolling_minute,
      browser_requests_total: limits.browser_requests_total,
      browser_requests_per_rolling_minute:
        limits.browser_requests_per_rolling_minute,
      ...
      combined_actual_plus_reserved_cap_eur:
        limits.combined_actual_plus_reserved_cap_eur,
    },
  };
}

export type CapState = ReturnType<typeof freshCapState>;

type EurLedger = {
  actual_eur: number;
  outstanding_reservations_eur: number;
};

export function combinedReservedAndActual(state: Record<string, unknown>): number {
  // Callers hand over parsed cap-state JSON (typed as Record<string, unknown> at
  // their boundary), so the ledgers are narrowed here, at the one consuming site.
  const app = state.app as EurLedger;
  const model = state.model as EurLedger;
  return (
    app.actual_eur +
    app.outstanding_reservations_eur +
    model.actual_eur +
    model.outstanding_reservations_eur
  );
}
...
export function priceSourceFor(model: unknown): typeof MODEL_PRICE_SOURCE {
  return typeof model === "string" && model.startsWith("claude-")
    ? MODEL_PRICE_SOURCE
    : TOGETHER_PRICE_SOURCE;
}
...
export function modelCostEur(
  ...
  const kinds = ["input", "cacheWrite", "cacheRead", "output"] as const;
  const usd =
    kinds.reduce((sum, kind) => sum + (usage?.[kind] ?? 0) * price[kind], 0) /
    1e6;
