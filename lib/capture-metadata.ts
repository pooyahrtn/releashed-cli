...
import type { CaptureMetadataInput, CaptureRoots } from "./capture-types.js";

const RUN_ID = /^[A-Za-z0-9][A-Za-z0-9._-]*$/;

export function assertRunId(id: unknown, option = "--continues"): string {
  if (typeof id !== "string" || !RUN_ID.test(id) || id === "." || id === "..")
    throw new Error(`${option} must name a safe run id`);
  return id;
}

export function preconditionRunIds(precondition: unknown): string[] {
...
async function hasRecord(
  ...
  try {
    ...
    const record: unknown = JSON.parse(await readFile(actual, "utf8"));
    if (!record || typeof record !== "object" || Array.isArray(record))
      return false;
    if (kind === "raw")
      return "status" in record && typeof record.status === "string";
    return (
      ("run_id" in record && typeof record.run_id === "string") ||
...
export async function validateRunReference(
  id: string,
  { runsRoot, mapsRoot }: CaptureRoots,
): Promise<string> {
  assertRunId(id);
  const raw = join(resolve(runsRoot), id, "target-session-1", "explorer-result.json");
  const packaged = join(resolve(mapsRoot), id, "map.json");
  if (
    !(await hasRecord(raw, runsRoot, "raw")) &&
    !(await hasRecord(packaged, mapsRoot, "map"))
  )
    throw new Error(
      `run ${id} was not found in ${resolve(runsRoot)} or ${resolve(mapsRoot)}`,
    );
  return id;
}

export async function validateCaptureMetadata({
  goal = null,
  policy = null,
  continues = null,
  precondition = null,
  ...
  runsRoot,
  mapsRoot,
}: CaptureMetadataInput & CaptureRoots): Promise<{
  ...
  if ((policy || continues || precondition || identityLabel) && !goal)
    throw new Error(
      "--policy, --continues, --precondition, and --identity-label need --goal",
    );
  if (
    identityLabel !== null &&
  ...
  const references = [continues, ...preconditionRunIds(precondition)].filter(
    (id): id is string => Boolean(id),
  );
  await Promise.all(
    references.map((id) => validateRunReference(id, { runsRoot, mapsRoot })),
  );
