...
const CLERK_TICKET_EXCHANGE_FAILURE_CODES: Record<string, string> =
  Object.fromEntries(
    Object.values(CLERK_TICKET_EXCHANGE_FAILURE_STAGES)
      .flatMap((stages) => Object.values(stages))
      .map((stage) => [stage, `clerk_auth_${stage}`]),
  );

export const CLERK_PAGE_AUTH_FAILURE_CODES: Record<string, string> =
  Object.freeze({
...
export function isSafeClerkAuthFailureCode(value: unknown): boolean {
  return typeof value === "string" && SAFE_CLERK_AUTH_FAILURE_CODES.has(value);
}

export const CLERK_BOUNDED_ROUTE_FAILURE_CODES: Record<string, string> =
  Object.freeze({
...
const SAFE_CLERK_BOUNDED_ROUTE_FAILURE_POINTS = new Set<string>(
  Object.values(CLERK_BOUNDED_ROUTE_FAILURE_POINTS),
);

export function safeClerkBoundedRouteFailure(
  stage: string,
  code: string,
  point: string | null = null,
): { stage: string; code: string; point: string | null } | null {
  if (!Object.hasOwn(CLERK_BOUNDED_ROUTE_FAILURE_CODES, stage)) return null;
  if (CLERK_BOUNDED_ROUTE_FAILURE_CODES[stage] !== code) return null;
  return {
    stage,
    code,
    point:
...
/** Whatever the browser SDK's start/inspect calls resolved to: success state, refusal, or nothing. */
export type ClerkPageAuthOutcome = {
  user_id?: unknown;
  session_id?: unknown;
...
export type ClerkPageStart = (args: {
  ticket: string;
  expectedUserId: string;
  expectedCurrentHref?: unknown;
}) => Promise<ClerkPageAuthOutcome>;

function validAuthState(
  state: ClerkPageAuthOutcome,
  expectedUserId: string,
): boolean {
  return Boolean(
    state &&
...
export class OneShotClerkPageAuthenticator {
  start: ClerkPageStart;
  inspect: () => Promise<ClerkPageAuthOutcome>;
  lock: () => unknown;
  timeoutMs: number;
  attempted = false;
  locked = false;
  ...
  async authenticate({
    ...
    if (
      ...
      return refusal(
        "clerk_auth_input_invalid",
        "Clerk authentication input is invalid",
      );
    }

    let timer: ReturnType<typeof setTimeout> | undefined;
    let outcome: ClerkPageAuthOutcome;
    try {
      outcome = await Promise.race([
        Promise.resolve().then(() =>
          this.start({ ticket, expectedUserId, expectedCurrentHref }),
        ),
        new Promise<ClerkPageAuthOutcome>((resolveTimeout) => {
          timer = setTimeout(
            () => resolveTimeout({ timed_out: true }),
            this.timeoutMs,
          );
        }),
      ]);
    ...
    let state: ClerkPageAuthOutcome = outcome;
    let ambiguous = false;
    ...
    if (!state || !validAuthState(state, expectedUserId)) {
      return refusal(
        ambiguous ? "clerk_auth_outcome_unknown" : "clerk_auth_failed",
        ambiguous
  ...
  async lockMethods(): Promise<boolean> {
    if (this.locked) return this.lockSucceeded;
    this.locked = true;
    try {
      await this.lock();
...
export function isPermittedClerkMutation({
  rawUrl,
  method,
  frontendApiOrigin,
  phase,
  ...
  if (url.pathname === "/v1/dev_browser") return true;
  ...
  if (
    ...
    return (
      url.pathname === `/v1/client/sessions/${encoded}/touch` ||
...
export function provisionalClerkBootstrapTouchSessionId({
  rawUrl,
  method,
  frontendApiOrigin,
  phase,
  ...
  if (url.origin !== frontendApiOrigin || url.username || url.password || url.hash)
    return null;
  return (
    /^\/v1\/client\/sessions\/([A-Za-z0-9_-]{4,256})\/touch$/.exec(url.pathname)?.[1] ??
    null
  );
}
