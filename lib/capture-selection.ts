...
export const digest = (bytes: BinaryLike): string =>
  createHash("sha256").update(bytes).digest("hex");

export async function sealedFile(
  candidateDir: string,
...
export function recordedSelection(
  ...
  if (!Array.isArray(paths) || paths.length === 0)
    throw new Error(
      "goal_screenshots must contain at least one recorded screenshot",
    );
  const observations = new Map<string, SelectedScreenshot & { order: number }>();
  for (const [index, event] of events.entries()) {
    for (const side of ["before", "after"] as const) {
      const path = event[side]?.screenshot_path;
      if (path && !observations.has(path))
        observations.set(path, {
          screenshot_path: path,
          event_id: event.event_id ?? null,
          captured_at: event.timestamp ?? null,
          screenshot_sha256: event[side]?.screenshot_sha256 ?? null,
  ...
  return paths.map((path) => {
    ...
    if (typeof path !== "string" || !observation)
      throw new Error(
        `screenshot ${String(path)} is not retained by the recorded trace`,
      );
    if (observation.order <= previous)
      throw new Error(
        "goal_screenshots must be unique and in recorded observation order",
      );
    previous = observation.order;
    const { order: _order, ...result } = observation;
    return result;
  });
}
...
export async function verifySelection(
  ...
  const events = String(traceBytes)
    .split("\n")
    .filter(Boolean)
    .map((line) => JSON.parse(line));
  const selected = recordedSelection(events, paths);
  for (const item of selected) {
    const bytes = await sealedFile(
      candidateDir,
      manifest,
      item.screenshot_path,
    );
    if (
      !item.screenshot_path.startsWith("screenshots/") ||
      !bytes ||
      (item.screenshot_sha256 && digest(bytes) !== item.screenshot_sha256)
    )
      throw new Error(
        `selected screenshot ${item.screenshot_path} is missing or does not match the sealed manifest`,
      );
...
export const selectionPath = (mapsRoot: string, runId: string): string =>
  join(dirname(resolve(mapsRoot)), "selections", `${assertRunId(runId)}.json`);
...
export async function selectCapture({
  ...
  if (typeof why !== "string" || !why.trim())
    throw new Error("select needs --why explaining the evidence selection");
  ...
  if (!mapBytes || !trace)
    throw new Error(
      `map.json or observations.jsonl failed verification in ${candidateDir}`,
    );
  ...
  if (
    ...
    throw new Error(
      "select requires a sealed directed capture with matching run IDs",
    );
  const evidence = await verifySelection(
    candidateDir,
    manifest,
    trace,
    screenshots,
