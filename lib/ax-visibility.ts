...
export async function isNodeOnScreen(send: CdpSend, backendNodeId: unknown): Promise<boolean> {
  ...
  try {
    ...
    const result = (await send("Runtime.callFunctionOn", {
      ...
      returnByValue: true,
    })) as { result?: { value?: unknown } } | null | undefined;
    return result?.result?.value === true;
  } catch {
    // No box model (display:none, detached, unlaid-out) -- not on screen.
    return false;
  } finally {
    if (objectId) await send("Runtime.releaseObject", { objectId }).catch(() => {});
  }
}
