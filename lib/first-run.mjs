 ...
 */
export function nodeRefusal(version = process.versions.node) {
    const major = Number(String(version).split(".")[0]);
    if (!Number.isFinite(major) || major >= MIN_NODE_MAJOR)
        return null;
    return (`releashed needs Node ${MIN_NODE_MAJOR} or newer, and this is Node ${version}. Install it, then re-check:\n` +
 ...
 */
export const BROWSER_REFUSAL = "Chromium is not installed. Run: releashed install-browser\n" +
...
export const EXPLORER_KEY_REFUSAL = "No explorer key. `releashed map` needs a model to look at each screenshot and decide what to try " +
...
// imminent.
export const GROUNDING_KEY_REFUSAL = "No grounding key. `releashed map` needs a second model that can point at a control on a " +
 ...
 */
export function missingModelKeys(env = process.env) {
    const problems = [];
 ...
 */
export function spendNotice(steps, model, budgetEur = null, subject = "this run") {
    const estimate = estimateEur(steps, model).toFixed(2);
    const basis = `${subject} spends your own model key: roughly EUR ${estimate} for ${steps} steps on ${model}, ` +
