import { chmod, link, lstat, mkdir, mkdtemp, readdir, readFile, realpath, rmdir, unlink, writeFile, } from "node:fs/promises";
...
async function assertOwnedDirectory(path, expected, label) {
    const metadata = await lstat(path);
...
function outputDirectory({ packsDirectory, runId }) {
...
async function validateStagedOutput({ directory, directoryIdentity, expectedSha256, }) {
    await assertOwnedDirectory(directory, directoryIdentity, "Staged public-pack output");
    ...
    const [publicPackIdentity, manifestIdentity, bytes, manifestBytes] = await Promise.all([
    ...
    if (!exactKeys(manifest, ["schema_version", "files"]) ||
        ...
        throw new Error("Staged public-pack manifest is not the required minimal shape");
    }
    return {
...
async function unlinkOwnedFile(file) {
    if (!file)
        return;
    try {
...
async function discardOwnedDirectory({ directory, directoryIdentity, files }) {
    if (!directory || !directoryIdentity)
        return false;
    try {
        await assertOwnedDirectory(directory, directoryIdentity, "Rebind temporary output");
    }
    catch {
        return false;
    }
    for (const file of files)
        await unlinkOwnedFile(file);
    try {
        await assertOwnedDirectory(directory, directoryIdentity, "Rebind temporary output");
 ...
 */
export async function rebindPublicPack({ repository, publicPackPath, expectedSha256, runId, finalize = finalizeExclusivePackOutput, beforePublish = async () => { }, afterPublicPackLinked = async () => { }, }) {
    const repositoryRoot = resolve(requireString(repository, "Repository root"));
    const repositoryDirectory = await ordinaryDirectory(repositoryRoot, "Repository root");
    const packs = await ordinaryDirectory(join(repositoryDirectory.requested, "packs"), "Packs directory");
    ...
    if (sha256Text(bytes) !== expectedSha256)
        throw new Error("Public-pack SHA-256 does not match the expected immutable bytes");
    validatePublicPack(JSON.parse(bytes.toString("utf8")));
    ...
    try {
        await assertOwnedDirectory(packsDirectory, packs.identity, "Packs directory");
        ...
        destinationIdentity = await directoryIdentity(output, "Public-pack destination");
        await assertOwnedDirectory(staging, stagingIdentity, "Staging output");
        await assertOwnedDirectory(output, destinationIdentity, "Public-pack destination");
