...
} from "./explorer-model-runtime.mjs";
...
async function assertDirectoryIdentity(directory, label) {
	...
	requireCondition(
		metadata?.isDirectory() && matchesIdentity(metadata, directory.identity),
...
async function ordinaryFile(path, label) {
	...
	try {
		...
		requireCondition(
			pathAfter?.isFile() && matchesSnapshotMetadata(pathAfter, after),
...
async function createExclusiveDirectory(path, label, assertBoundary, onCreated) {
	...
	requireCondition(
		beforeChmod?.isDirectory() && matchesIdentity(beforeChmod, createdIdentity),
...
async function removeOwnedDirectory(path, expected, files, assertTrustedRoots) {
	...
	const assertRollbackBoundary = async () => {
		...
		requireCondition(
			current?.isDirectory() && matchesIdentity(current, expected),
...
async function verifiedFileHashes(
	...
	return {
		hashes: Object.fromEntries(
			names.map((name) => [name, snapshots[name].sha256]),
		),
		snapshots,
	};
}
...
async function validateExistingPreparation({
	...
	let explorer;
	try {
		cap = JSON.parse(runtimeFiles.snapshots["cap-state.json"].bytes);
		target = JSON.parse(
			runtimeFiles.snapshots["target-session-config.json"].bytes,
		);
		explorer = JSON.parse(
			runtimeFiles.snapshots["explorer-model-config.json"].bytes,
		);
	} catch {
		throw new Error(
...
export async function prepareBoundedOnboardingRun({
	...
	const controls = Object.fromEntries(
		await Promise.all(
			PRIVATE_CONTROL_FILES.map(async (name) => [
				name,
	...
	const privateControlHashes = Object.fromEntries(
		PRIVATE_CONTROL_FILES.map((name) => [name, controls[name].sha256]),
	);
	const inputBinding = {
		run_id: runId,
	...
	try {
		...
		try {
			verifiedCap = JSON.parse(
				verifiedRuntime.snapshots["cap-state.json"].bytes,
			);
			verifiedExplorer = JSON.parse(
				verifiedRuntime.snapshots["explorer-model-config.json"].bytes,
			);
		} catch {
			throw new Error(
