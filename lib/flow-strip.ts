 ...
 */
export function stripHtml(screens: StripScreen[], { heading, subheading = null }: StripLabels = {} as StripLabels): string {
...
export async function renderStrip({ candidateDir, outputPath, heading, subheading = null, browser = null }: {
  ...
  heading: string;
  subheading?: string | null;
  browser?: StripBrowser | null;
}): Promise<string> {
  const map = JSON.parse(await readFile(join(candidateDir, "map.json"), "utf8"));
  const screens = await Promise.all(
    map.screens.map(async (screen: { title: string | null; url: string | null; screenshot: string | null }) => ({
      title: screen.title,
      url: screen.url,
  ...
  const { chromium }: { chromium: StripBrowser } = browser ? { chromium: browser } : await import("playwright");
