...
function show(value) {
    if (typeof value === "string")
        return value.length > 120 ? `${JSON.stringify(value.slice(0, 120))}…` : JSON.stringify(value);
...
function topLevelIssue(pack) {
    if (!exactKeys(pack, TOP_KEYS))
        return `top level ${shapeMismatch(pack, TOP_KEYS)}`;
    const p = pack;
    if (p.schema_version !== 1)
        return `schema_version must be 1, got ${show(p.schema_version)}`;
    ...
    if (!Array.isArray(p.claims))
        return `claims must be an array, got ${show(p.claims)}`;
    if (!Array.isArray(p.sources))
        return `sources must be an array, got ${show(p.sources)}`;
...
function sourceIssue(source, index, seenIds) {
    ...
    if (!exactKeys(source, SOURCE_KEYS))
        return `source ${label} ${shapeMismatch(source, SOURCE_KEYS)}`;
    if (typeof s.source_id !== "string")
        return `source ${label}'s source_id must be a string, got ${show(s.source_id)}`;
    if (seenIds.has(s.source_id))
        return `source_id ${show(s.source_id)} is used by more than one source`;
    if (!/^source-\d{3}$/.test(s.source_id))
        return `source ${label}'s source_id must match "source-NNN", got ${show(s.source_id)}`;
...
function claimIssue(claim, index, knownSourceIds) {
    ...
    if (!exactKeys(claim, CLAIM_KEYS))
        return `claim ${label} ${shapeMismatch(claim, CLAIM_KEYS)}`;
    if (typeof c.id !== "string")
        return `claim ${label}'s id must be a string, got ${show(c.id)}`;
    if (!/^[a-z][a-z0-9-]{0,63}$/.test(c.id))
        return `claim ${label}'s id must match "[a-z][a-z0-9-]{0,63}", got ${show(c.id)}`;
    if (!CLAIM_KINDS.has(c.kind))
        return `claim ${label}'s kind must be one of [${[...CLAIM_KINDS].join(", ")}], got ${show(c.kind)}`;
    if (typeof c.text !== "string" || !c.text.trim())
        return `claim ${label}'s text must be a nonempty string, got ${show(c.text)}`;
    ...
    const unknown = c.source_ids.find((sourceId) => !knownSourceIds.has(sourceId));
    if (unknown !== undefined)
        return `claim ${label} cites source_id ${show(unknown)}, which is not one of this pack's sources`;
    return null;
}
...
export function validatePublicPack(pack) {
    ...
    if (topIssue)
        throw new Error(`Public pack schema is invalid: ${topIssue}`);
    const checked = pack;
    const seenIds = new Set();
    checked.sources.forEach((source, index) => {
        const issue = sourceIssue(source, index, seenIds);
        if (issue)
            throw new Error(`Public pack sources are invalid: ${issue}`);
    ...
    checked.claims.forEach((claim, index) => {
        ...
        if (issue)
            throw new Error(`Public pack claims are not citable: ${issue}`);
        if (claimIds.has(claim.id))
            throw new Error(`Public pack claims are not citable: claim id ${show(claim.id)} is used by more than one claim`);
    ...
    for (const [pattern, label] of forbidden) {
        if (pattern.test(rendered))
            throw new Error(`Public pack contains a forbidden private artifact field: it matches ${label}`);
    }
    return checked;
}
