...
export type LocatorModelConfig = {
  baseUrl: string;
  modelName: string;
  modelFamily: string;
};
...
export type LocatorResolution =
  | { enabled: true; config: LocatorModelConfig }
...
export function resolveLocatorConfig(env: NodeJS.ProcessEnv): LocatorResolution {
  ...
  const missing = ["MIDSCENE_MODEL_BASE_URL", "MIDSCENE_MODEL_NAME", "MIDSCENE_MODEL_FAMILY"].find(
    (key) => !(env[key] ?? "").trim(),
  );
  if (missing)
    return {
...
export type ObservedShot = {
  // Exact bytes handed to the caller at observe time.
  png: Buffer;
  // Pixel dimensions those bytes must decode to (re-verified, not trusted blindly).
  width: number;
  height: number;
  // CSS viewport size the browser dispatches inputs in.
  cssWidth: number;
  cssHeight: number;
  // Sealed hash of the observation these bytes must still equal.
  sha256: string;
};

export type LocatedRect = { left: number; top: number; width: number; height: number } | null;

export type LocateOutcome =
  | { ok: true; x: number; y: number; rect: LocatedRect }
  | {
      ok: false;
      kind: "stale-shot" | "miss" | "out-of-bounds" | "timeout" | "error";
      detail: string;
    };

// Structural minimum of a Midscene agent for the locate-only path. The real one
// is built lazily (see defaultLocatorFactories); tests inject a stub.
export type LocatorAgent = {
  aiLocate: (
    prompt: string,
    opts: { uiContext: unknown; cacheable: false },
  ) => Promise<{ center?: unknown; rect?: unknown } | null | undefined>;
  destroy?: () => Promise<void> | void;
};

export type LocatorFactories = {
  createAgent: (page: unknown) => Promise<LocatorAgent> | LocatorAgent;
  // Required: the caller threads its factory so locate stays hermetic in tests
  // and the Midscene import lives in exactly one lazy place (see
  // defaultLocatorFactories). A missing factory is an explicit error, never a
  // silent import.
  createScreenshotItem?: (base64: string, capturedAt: number) => { base64: string };
};

// Lazily loaded so coordinate-mode runs never import the Midscene packages.
export async function defaultLocatorFactories(): Promise<LocatorFactories> {
  const [{ ScreenshotItem }, { PlaywrightAgent }] = await Promise.all([
    import("@midscene/core"),
    import("@midscene/web/playwright"),
  ]);
  return {
    createAgent: (page: unknown) =>
      // No report litter beside the run: grounding evidence stays in our own trace.
      new PlaywrightAgent(page as never, { generateReport: false }) as unknown as LocatorAgent,
    createScreenshotItem: (base64: string, capturedAt: number) =>
      ScreenshotItem.create(base64, capturedAt) as { base64: string },
  };
}
...
async function destroyGracefully(agent: LocatorAgent | undefined): Promise<void> {
  try {
...
function asPoint(center: unknown): { x: number; y: number } | null {
  const pair = Array.isArray(center)
    ? center
    : center && typeof center === "object"
      ? [(center as { x?: unknown }).x, (center as { y?: unknown }).y]
      : null;
...
function asRect(rect: unknown): LocatedRect {
  if (!rect || typeof rect !== "object") return null;
  const { left, top, width, height } = rect as Record<string, unknown>;
...
export async function locateOnShot(
  factories: LocatorFactories,
  page: unknown,
  shot: ObservedShot,
  target: string,
  ...
  const uiContext = {
    screenshot: createScreenshotItem(
  ...
  let agent: LocatorAgent | undefined;
  try {
