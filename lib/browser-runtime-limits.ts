...
export type InitialNavigationFailurePoint =
  (typeof INITIAL_NAVIGATION_FAILURE_POINTS)[keyof typeof INITIAL_NAVIGATION_FAILURE_POINTS];

const SAFE_INITIAL_NAVIGATION_FAILURE_POINTS = new Set<string>(Object.values(INITIAL_NAVIGATION_FAILURE_POINTS));

export function safeInitialNavigationFailurePoint(value: unknown): InitialNavigationFailurePoint | null {
  return typeof value === "string" && SAFE_INITIAL_NAVIGATION_FAILURE_POINTS.has(value)
    ? (value as InitialNavigationFailurePoint)
    : null;
}
