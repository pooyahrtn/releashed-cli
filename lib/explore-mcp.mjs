...
export function createExplorer({
  ...
  const locatorConfig = locatorResolution?.enabled ? locatorResolution.config : null;
  ...
  async function locateForDispatch(shot, target) {
    ...
    const outcome = await timing.span("grounding.locate", async () =>
      locateOnShot(await locatorFactories(shot), state.session.page, shot, target, {
        timeoutMs: budget,
      }),
    );
    if (!outcome.ok) {
  ...
  function stopAtScopeExit(decision) {
    if (!state.scopeExit) {
      state.scopeExit = "Observation went off the product's own site or its origin became uncertain. No out-of-scope screenshot retained; the final effect is unknown. No more input will be dispatched. Record any pending in-scope frames, then finish with goal_reached: false or close; never repeat this input.";
      state.blocked.push({
        instruction: decision?.instruction ?? "observe the product",
        target: decision?.target ?? null,
        transition_kind: "unknown-terminal", reason: state.scopeExit,
      });
    }
  ...
  async function snapshot(decision = null) {
    state.stateIndex += 1;
    const { page, cdp, boundary } = state.session;
    ...
    const checkScope = () => {
      ...
      try { sampleScope(page.url()); } catch { invalidScope = true; }
    ...
    const measure = captureTransitions ? (name, work) => timing.span(name, async () => {
      checkScope();
      try { return await work(); } finally { checkScope(); }
    }) : timing.span;
    const capturePage = captureTransitions ? {
      url: () => page.url(),
      screenshot: async () => (acquiredPng = await page.screenshot()),
    } : page;
    const listen = captureTransitions && typeof page.on === "function" &&
      typeof page.off === "function" && typeof page.mainFrame === "function";
    try {
      if (listen) page.on("framenavigated", navigation);
      let observed;
      try {
        observed = await measure("observation", () =>
          observeScreen(capturePage, cdp, runDir, state.stateIndex, true, measure),
        );
        checkScope();
      } catch (error) {
        checkScope();
        // A metadata failure may retain valid acquired pixels, but never origin-uncertain
        // bytes. A crossing during this fallback write is also rolled back below.
        if (acquiredPng)
          await measure("screenshot.preserve", () => writeFile(path, acquiredPng, { mode: 0o600 }));
        throw error;
      }
      // Immutable acquisition timestamp; runtime state commits only after guarded completion.
      observed.evidence.observed_at = new Date(now()).toISOString();
      checkScope();
      state.current = observed;
      return observed;
    } catch (error) {
      try { checkScope(); } catch { /* scope exit takes precedence over acquisition failure */ }
      if (invalidScope) {
        acquiredPng = null;
  ...
  async function observe(args = {}) {
    ...
    if (args.record_previous !== undefined && args.record_previous !== false) {
      ...
      const receipt = JSON.parse(kept.content[0].text);
  ...
  async function finish(args = {}) {
    ...
    if (goal && !(args.goal_reached === true && !state.walled && !state.scopeExit && !state.captureFailure)) {
      let origin = null;
      try {
        origin = new URL(state.entryUrl).origin;
      } catch {
        origin = null;
      }
