...
/** @param {unknown} id @param {string} [option] @returns {string} */
...
/** @param {unknown} precondition @returns {string[]} */
export function preconditionRunIds(precondition) {
  if (precondition === null || precondition === undefined) return [];
...
/** @param {string} path @param {string} root @param {'raw' | 'map'} kind */
async function hasRecord(path, root, kind) {
  try {
    ...
    const [actual, allowedRoot] = await Promise.all([realpath(path), realpath(root)]);
    const within = relative(allowedRoot, actual);
    if (within === "" || within.startsWith(`..${process.platform === "win32" ? "\\" : "/"}`) || isAbsolute(within)) return false;
    if (!(await stat(actual)).isFile()) return false;
    const record = JSON.parse(await readFile(actual, "utf8"));
    if (!record || typeof record !== "object" || Array.isArray(record)) return false;
    return kind === "raw"
      ? typeof record.status === "string"
      : typeof record.run_id === "string" || typeof record.schema_version === "number";
  } catch {
    return false;
  }
}

/** @param {string} id @param {import('./capture-types.js').CaptureRoots} roots */
export async function validateRunReference(id, { runsRoot, mapsRoot }) {
  ...
  if (!(await hasRecord(raw, runsRoot, "raw")) && !(await hasRecord(packaged, mapsRoot, "map")))
    throw new Error(`run ${id} was not found in ${resolve(runsRoot)} or ${resolve(mapsRoot)}`);
  return id;
}

/** @param {import('./capture-types.js').CaptureMetadataInput & import('./capture-types.js').CaptureRoots} metadata */
export async function validateCaptureMetadata({ goal = null, policy = null, continues = null, precondition = null, identityLabel = null, runsRoot, mapsRoot }) {
  ...
  if (identityLabel !== null && (typeof identityLabel !== "string" || !identityLabel.trim() || identityLabel.length > 128))
    throw new Error("--identity-label must be a short, non-empty opaque label");
  if (precondition !== null && precondition !== undefined && (typeof precondition !== "string" || !precondition.trim()))
    throw new Error("--precondition must be non-empty plain English");
  if (!goal) return { continues: null, precondition: null, identityLabel: null };
  const references = [continues, ...preconditionRunIds(precondition)].filter(/** @returns {id is string} */ (id) => Boolean(id));
