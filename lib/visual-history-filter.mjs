...
import { readImageReferences, } from "./image-references.mjs";
...
export const VISUAL_HISTORY_SCREENSHOT_ROOT_ENV = "RELEASHED_VISUAL_HISTORY_SCREENSHOT_ROOT";
...
function refuseIfUnsupportedImageMime(mime, where) {
    if (typeof mime === "string" &&
        mime.startsWith("image/") &&
        !isImageMime(mime))
        throw new VisualHistoryRefusal(`${where} uses unsupported image mime ${JSON.stringify(mime)}; refusing`);
}
...
function checkImagePayload(mime, url, where, screenshotRoot) {
    if (typeof mime !== "string" ||
        !MIME_TOKEN_RE.test(mime) ||
        !isImageMime(mime))
        throw new VisualHistoryRefusal(`${where} has a malformed image mime; refusing`);
    ...
    if (data) {
        if (data[1].toLowerCase() !== mime.toLowerCase())
            throw new VisualHistoryRefusal(`${where} declares ${mime} but carries ${data[1]}; refusing`);
        const b64 = data[2];
        if (b64.length === 0 ||
            b64.length % 4 !== 0 ||
            Buffer.from(b64, "base64").length === 0)
            throw new VisualHistoryRefusal(`${where} carries malformed base64; refusing`);
        return { path: null };
    }
    const scoped = asFilePath(url);
    if (scoped === null)
        throw new VisualHistoryRefusal(`${where} has an unfamiliar image url shape; refusing`);
    void screenshotRoot;
    return { path: scoped };
}
 ...
 */
function observeProvenance(output, screenshotRoot) {
    ...
    if (!isRecord(parsed))
        return undefined;
    if (typeof parsed["steps_used"] !== "number" &&
...
function memoryReferences(output) {
    let value;
    try {
        value = readImageReferences(output);
    }
    catch {
        throw new VisualHistoryRefusal("malformed saved-image references; refusing");
    }
    if (value === undefined)
        return undefined;
    if (!isRecord(value) ||
        ...
        throw new VisualHistoryRefusal("unfamiliar saved-image references; refusing");
    for (const ref of value["images"]) {
        if (!isRecord(ref) ||
            ...
            throw new VisualHistoryRefusal("invalid saved-image path or digest; refusing");
    }
    return value["images"];
}
/** Bind bytes, not attachment order; repeated images and prior pruning are safe. */
function memoryProvenance(refs, url, root) {
    if (refs === undefined || typeof url !== "string")
        return undefined;
    const data = DATA_URL_RE.exec(url);
    if (!data)
        return undefined;
    ...
    if (matches.length === 0)
        throw new VisualHistoryRefusal("saved-image bytes have no matching provenance digest; refusing");
...
function enumerateImages(messages, screenshotRoot) {
    const images = [];
    messages.forEach((message, messageIndex) => {
        if (!isRecord(message) ||
            !isRecord(message["info"]) ||
            !Array.isArray(message["parts"]))
            throw new VisualHistoryRefusal(`message at index ${messageIndex} is malformed; refusing`);
        message.parts.forEach((part, partIndex) => {
            if (!isRecord(part) || typeof part["type"] !== "string")
                throw new VisualHistoryRefusal(`part ${messageIndex}.${partIndex} is malformed; refusing`);
            const where = `part ${JSON.stringify(typeof part["id"] === "string" && part["id"] !== "" ? part["id"] : `${messageIndex}.${partIndex}`)}`;
            if (part["type"] === "file") {
                if (typeof part["id"] !== "string" || part["id"] === "")
                    throw new VisualHistoryRefusal(`user file ${where} has no stable occurrence id; refusing`);
                const mime = part["mime"];
                if (mime === "text/plain" || mime === "application/x-directory")
                    return;
                if (mime !== undefined && typeof mime !== "string")
                    throw new VisualHistoryRefusal(`user file ${where} has a non-string mime; refusing`);
                if (typeof mime !== "string" || !isImageMime(mime)) {
                    ...
                    if (typeof mime !== "string") {
                        ...
                        if ((typeof url === "string" &&
                            ...
                            throw new VisualHistoryRefusal(`user file ${where} is image-bearing but untyped; refusing`);
                    }
                    return;
                }
                const { path } = checkImagePayload(mime, part["url"], `user file ${where}`, screenshotRoot);
                if (path === null) {
                ...
                images.push({
                    prunable: true,
                    key: `file:${part["id"]}:${sha256Hex(`${mime}\n${part["url"]}`)}`,
                    messageIndex,
                    partIndex,
            ...
            if (part["type"] === "tool") {
                if (typeof part["id"] !== "string" || part["id"] === "")
                    throw new VisualHistoryRefusal(`tool ${where} has no stable occurrence id; refusing`);
                const state = part["state"];
                if (!isRecord(state) || typeof state["status"] !== "string")
                    throw new VisualHistoryRefusal(`tool ${where} has no status; refusing`);
                if (state["status"] !== "completed") {
                    if (state["status"] === "pending" ||
                        state["status"] === "running" ||
                        state["status"] === "error")
                        return;
                    throw new VisualHistoryRefusal(`tool ${where} has unfamiliar status ${JSON.stringify(state["status"])}; refusing`);
                }
                const attachments = state["attachments"];
                if (attachments === undefined)
                    return;
                if (!Array.isArray(attachments))
                    throw new VisualHistoryRefusal(`tool ${where} has non-array attachments; refusing`);
                ...
                attachments.forEach((entry, attachmentIndex) => {
                    ...
                    if (Array.isArray(entry))
                        throw new VisualHistoryRefusal(`${at} is a nested array; nested attachments are unsupported, refusing`);
                    if (!isRecord(entry))
                        throw new VisualHistoryRefusal(`${at} is not an object; refusing`);
                    if (entry["content"] !== undefined ||
                        entry["parts"] !== undefined ||
                        entry["items"] !== undefined)
                        throw new VisualHistoryRefusal(`${at} has nested content; nested attachments are unsupported, refusing`);
                    if (entry["type"] === "text" && typeof entry["text"] === "string")
                        return;
                    if (entry["type"] !== "file")
                        throw new VisualHistoryRefusal(`${at} has unfamiliar type ${JSON.stringify(entry["type"])}; refusing`);
                    const mime = entry["mime"] ?? entry["mimeType"];
                    if (mime !== undefined && typeof mime !== "string")
                        throw new VisualHistoryRefusal(`${at} has a non-string mime; refusing`);
                    if (typeof mime !== "string" || !isImageMime(mime)) {
                        refuseIfUnsupportedImageMime(mime, at);
                        return;
                    }
                    const { path } = checkImagePayload(mime, entry["url"] ?? entry["data"], at, screenshotRoot);
                    const own = path;
                    const parent = own ??
                        (() => {
                            ...
                            if (fromRead !== undefined) {
                                ...
                                if (scoped === null)
                                    throw new VisualHistoryRefusal(`${at} parent read filePath is not absolute; refusing`);
                                return scoped;
                            }
                            const fromObserve = observeProvenance(state["output"], screenshotRoot);
                            if (fromObserve !== undefined)
                                return fromObserve;
                            const fromMemory = memoryProvenance(savedImages, entry["url"] ?? entry["data"], screenshotRoot);
                            if (fromMemory !== undefined)
                                return fromMemory;
                            throw new VisualHistoryRefusal(`${at} has no screenshot provenance (no path, no read filePath, no capture output reference); refusing`);
                        })();
                    ...
                    images.push({
                        prunable: true,
                        key: `tool:${part["id"]}:${attachmentIndex}:${sha256Hex(`${mime}\n${String(entry["url"] ?? entry["data"])}`)}`,
                        messageIndex,
                        partIndex,
                        attachmentIndex,
                    });
 ...
 */
export function filterVisualHistory(messages, options) {
    const omitted = new Set(options.omittedKeys ?? []);
    const clones = structuredClone(messages);
    const images = enumerateImages(clones, options.screenshotRoot);
    const omitNow = new Set();
    for (const image of images) {
        if (image.prunable && omitted.has(image.key))
            omitNow.add(image.key);
    }
    const visible = images.filter((image) => !(image.prunable && (omitNow.has(image.key) || omitted.has(image.key))));
    const newOmissions = [];
    if (visible.length > MAX_IMAGES) {
        const prunableVisible = visible.filter((image) => image.prunable);
        const unprunableVisible = visible.length - prunableVisible.length;
        const keep = new Set(prunableVisible
            .slice(prunableVisible.length - RETAIN_LATEST)
            .map((image) => image.key));
        if (unprunableVisible + Math.min(RETAIN_LATEST, prunableVisible.length) >
            MAX_IMAGES)
            throw new VisualHistoryRefusal(`unprunable images (${unprunableVisible}) plus the retained latest leave no room under ${MAX_IMAGES}; refusing`);
        for (const image of prunableVisible) {
            if (keep.has(image.key))
                continue;
            if (omitNow.has(image.key))
                continue;
            omitNow.add(image.key);
            newOmissions.push(image.key);
        }
        for (const key of newOmissions)
            omitted.add(key);
    }
    ...
    const omittedAttachmentIndices = new Map();
    for (const image of images) {
        if (!image.prunable || !omitNow.has(image.key))
            continue;
        const message = clones[image.messageIndex];
        if (!message)
            continue;
        if (image.attachmentIndex === null) {
            const original = message.parts[image.partIndex];
    ...
    for (const [slot, indices] of omittedAttachmentIndices) {
        const [messageIndex, partIndex] = slot.split(".").map(Number);
        const part = clones[messageIndex]?.parts[partIndex];
        const state = part?.["state"];
        if (!Array.isArray(state?.["attachments"]))
            continue;
        state["attachments"] = state["attachments"].filter((_, index) => !indices.has(index));
    }
    ...
    const remaining = images.filter((image) => !(image.prunable && omitNow.has(image.key))).length;
    if (remaining > MAX_IMAGES)
        throw new VisualHistoryRefusal(`visible images (${remaining}) exceed the ceiling of ${MAX_IMAGES} after filtering; refusing`);
    return {
        messages: clones,
...
export function ledgerPath(dir, sessionID) {
...
export function ledgerLockPath(dir, sessionID) {
...
export function requireSessionID(messages) {
    const ids = new Set();
    for (const message of messages) {
        const sessionID = isRecord(message) && isRecord(message["info"])
            ? message["info"]["sessionID"]
            : undefined;
        if (typeof sessionID !== "string" || sessionID === "")
            throw new VisualHistoryRefusal("a message has a missing session id; refusing");
        ids.add(sessionID);
    }
    if (ids.size > 1)
        throw new VisualHistoryRefusal(`one transform holds ${ids.size} distinct sessions; refusing`);
    const only = ids.values().next();
    if (only.done)
        throw new VisualHistoryRefusal("no session id found; refusing");
    return only.value;
}
export async function loadOmittedKeys(dir, sessionID) {
    ...
    try {
        ...
        if (error.code === "ENOENT")
            return new Set();
        throw new VisualHistoryRefusal(`cannot read omission ledger at ${path}: ${error.message}`);
    }
    let parsed;
    try {
        ...
        throw new VisualHistoryRefusal(`omission ledger at ${path} is not valid JSON; refusing`);
    }
    if (!isRecord(parsed) ||
        parsed["version"] !== LEDGER_VERSION ||
        !Array.isArray(parsed["omitted"]))
        throw new VisualHistoryRefusal(`omission ledger at ${path} has an unfamiliar shape; refusing`);
    if (parsed["sessionID"] !== sessionID)
        throw new VisualHistoryRefusal(`omission ledger at ${path} belongs to another session; refusing`);
    const keys = new Set();
    for (const key of parsed["omitted"]) {
        if (typeof key !== "string" || key === "")
            throw new VisualHistoryRefusal(`omission ledger at ${path} holds a malformed key; refusing`);
        keys.add(key);
    }
    return keys;
}
/** Atomic private (0600) temp-file plus rename. Write failures refuse. */
export async function storeOmittedKeys(dir, sessionID, keys) {
    const path = ledgerPath(dir, sessionID);
    ...
    try {
        ...
        throw new VisualHistoryRefusal(`cannot write omission ledger at ${path}: ${error.message}`);
    }
}
async function acquireLock(dir, sessionID) {
    const path = lockPath(dir, sessionID);
    try {
        await mkdir(dir, { recursive: true });
    }
    catch (error) {
        throw new VisualHistoryRefusal(`cannot prepare omission ledger dir at ${dir}: ${error.message}`);
    }
    ...
    try {
        ...
        if (error.code !== "EEXIST")
            throw new VisualHistoryRefusal(`cannot claim ledger lock at ${path}: ${error.message}`);
    }
    ...
    throw new VisualHistoryRefusal(`omission ledger for this session is locked by a concurrent transform; refusing rather than risk resurrecting pruned images`);
}
async function releaseLock(dir, sessionID) {
    try {
...
function withSessionLock(sessionID, fn) {
    ...
    let release = () => { };
    const mine = new Promise((resolveLock) => {
        release = resolveLock;
    });
    sessionLocks.set(sessionID, prev.then(() => mine));
...
function readEnv() {
    ...
    if (dir === "" && root === "")
        return null;
    if (dir === "" || root === "")
        throw new VisualHistoryRefusal(`partial visual-history config: ${VISUAL_HISTORY_DIR_ENV} is ${dir === "" ? "missing" : "set"} but ${VISUAL_HISTORY_SCREENSHOT_ROOT_ENV} is ${root === "" ? "missing" : "set"}; refusing instead of silently disabling`);
    if (!isAbsolute(dir))
        throw new VisualHistoryRefusal(`${VISUAL_HISTORY_DIR_ENV} must be absolute, got ${JSON.stringify(dir)}`);
    if (!isAbsolute(root))
        throw new VisualHistoryRefusal(`${VISUAL_HISTORY_SCREENSHOT_ROOT_ENV} must be absolute, got ${JSON.stringify(root)}`);
    return { dir, root };
}
/**
 ...
 */
export async function visualHistoryTransform(_input, output) {
    const env = readEnv();
    if (env === null)
        return;
    if (!isRecord(output) || !Array.isArray(output["messages"]))
        throw new VisualHistoryRefusal(`unfamiliar transform payload (output.messages is not an array); refusing (tested OpenCode ${SUPPORTED_OPENCODE_VERSION} only)`);
