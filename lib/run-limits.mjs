...
const OWNER_BLOCKED_DESTINATION = /\b(billing|checkout|subscri(?:be|ption)|upgrade[- ]?to[- ]?paid|cancel[- ]?account|delete[- ]?account)\b/i;
...
export function inScope(currentUrl, allowedOrigins, { include = [], exclude = [] } = {}) {
    ...
    if (!allowedOrigins.has(url.origin))
        return false;
    if (exclude.some((pattern) => matchesGlob(url.pathname, pattern)))
        return false;
    if (include.length > 0 && !include.some((pattern) => matchesGlob(url.pathname, pattern)))
        return false;
    return true;
}
...
export function verbIsTheAction(verb, { instruction = "", target = "" } = {}) {
    ...
    if (label) {
        ...
        if (new RegExp(`^${verb}(s|ed|ing)?\\b`, "i").test(afterConfirm))
            return true;
        if (label.split(/\s+/).filter(Boolean).length <= 3 && wholeWord.test(label))
            return true;
    }
...
const BOT_WALL = /\b(captcha|are you a (human|robot)|unusual traffic|verify you are human|access denied|请完成安全验证|checking your browser|enable javascript and cookies)\b/i;
...
export function noEffectNote(streak) {
    if (!(streak >= NO_EFFECT_STREAK_LIMIT))
        return null;
...
export function onBoundOrigin(currentUrl, allowedOrigins, pathScope) {
