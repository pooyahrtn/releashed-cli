...
export function validateFrozenPublicFetchPolicy(policy, { allowInsecureFixture = false } = {}) {
  ...
  if (!exactKeys(policy, expected) || policy.schema_version !== 1 || policy.record_kind !== "spike-a-frozen-public-pack-fetch-policy" || typeof policy.policy_id !== "string" || !policy.policy_id || policy.content_classification !== "public-only" || typeof policy.note !== "string") throw new Error("Frozen public-source policy schema is invalid");
  if (!Array.isArray(policy.initial_urls) || !Array.isArray(policy.permitted_final_urls) || policy.initial_urls.length !== 1 || policy.permitted_final_urls.length !== 2 || !Number.isSafeInteger(policy.maximum_redirects) || !Number.isSafeInteger(policy.timeout_ms) || !Number.isSafeInteger(policy.maximum_raw_bytes_per_source) || !Number.isSafeInteger(policy.maximum_derived_text_bytes_total) || !Array.isArray(policy.allowed_content_types)) throw new Error("Frozen public-source policy values are invalid");
  const initialUrls = policy.initial_urls.map((url) => normalizedExactUrl(url, allowInsecureFixture));
  const finalUrls = policy.permitted_final_urls.map((url) => normalizedExactUrl(url, allowInsecureFixture));
  if (new Set(finalUrls).size !== 2 || new Set(policy.allowed_content_types).size !== CONTENT_TYPES.length || !CONTENT_TYPES.every((type) => policy.allowed_content_types.includes(type))) throw new Error("Frozen public-source policy list values are invalid");
  if (policy.maximum_redirects !== 3 || policy.timeout_ms !== 20_000 || policy.maximum_raw_bytes_per_source !== 524_288 || policy.maximum_derived_text_bytes_total !== 28_672) throw new Error("Frozen public-source policy limits are not exact");
  ...
  return { policy_id: policy.policy_id, initial_urls: initialUrls, permitted_final_urls: finalUrls, permitted_origins: [...new Set(finalUrls.map((url) => new URL(url).origin))], maximum_redirects: policy.maximum_redirects, timeout_ms: policy.timeout_ms, maximum_raw_bytes_per_source: policy.maximum_raw_bytes_per_source, allowed_content_types: [...CONTENT_TYPES], maximum_derived_text_bytes_total: policy.maximum_derived_text_bytes_total, policy_sha256: sha256Text(JSON.stringify(policy)) };
}
