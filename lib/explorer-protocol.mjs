...
export class SequentialRpcTransport {
    ...
    constructor({ write, timeoutMs = EXPLORER_RPC_TIMEOUT_MS, }) {
        if (typeof write !== "function" || !Number.isSafeInteger(timeoutMs) || timeoutMs <= 0)
            fail("invalid_rpc", "RPC transport requires a writer and positive deadline");
        this.write = write;
    ...
    request(broker, payload) {
        ...
        if (this.pending)
            return Promise.reject(new ExplorerProtocolError("concurrent_rpc", "Explorer RPC must be strictly sequential"));
    ...
    receiveLine(line) {
        ...
        if (!this.pending || !exactKeys(record, ["rpc_id", "response"]) || record.rpc_id !== this.pending.rpcId || !response || typeof response !== "object" || Array.isArray(response)) {
...
function cleanText(value, name, maximumBytes, { multiline = false, synthetic = false } = {}) {
    if (typeof value !== "string" || !value.trim() || Buffer.byteLength(value, "utf8") > maximumBytes)
        fail("invalid_text", `${name} must be bounded non-empty text`);
    const controlPattern = multiline ? /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/ : /[\u0000-\u001f\u007f]/;
    if (controlPattern.test(value))
        fail("invalid_text", `${name} contains unsupported control characters`);
...
function cleanUrl(value) {
    if (typeof value !== "string" || value.length > 2_048)
        fail("invalid_url", "Current URL is missing or too long");
    let url;
    try {
        url = new URL(value);
    }
    ...
    if ((!localFixture && !boundLocalOrigin && url.protocol !== "https:") || url.username || url.password || url.search || url.hash)
        fail("invalid_url", "Current URL is not a sanitized HTTPS URL");
    if (/\/(?:auth|oauth|sso|sign-?in|sign-?up|callback|verify|tokens?)(?:\/|$)/i.test(url.pathname))
        fail("invalid_url", "Authentication-flow URLs are not explorer input");
    return url.href;
}
function parseVisibleRefs(accessibility) {
    ...
    for (const line of accessibility.split("\n")) {
        ...
        if (refs.has(ref))
            fail("malformed_observation", "Accessibility state repeats a visible reference");
        refs.set(ref, { role: match[2].toLowerCase(), name: match[3] ?? "" });
    }
    return refs;
}
export function validateBoundPublicPack(publicPackBytes, expectedSha256) {
    if (!Buffer.isBuffer(publicPackBytes) && !(publicPackBytes instanceof Uint8Array))
        fail("invalid_public_pack", "Public pack must be supplied as exact bytes");
    if (!/^[0-9a-f]{64}$/.test((expectedSha256 ?? "")))
        fail("invalid_public_pack_binding", "Public pack binding must be a SHA-256 digest");
    if (publicPackBytes.byteLength === 0 || publicPackBytes.byteLength > MAX_PUBLIC_PACK_BYTES)
        fail("invalid_public_pack", "Public pack exceeds the explorer boundary");
    const actualSha256 = createHash("sha256").update(publicPackBytes).digest("hex");
    if (actualSha256 !== expectedSha256)
        fail("public_pack_binding_mismatch", "Public pack bytes do not match their supervisor binding");
    let pack;
    try {
        ...
        if (error instanceof ExplorerProtocolError)
            throw error;
...
export function validateObservation(response) {
    ...
    if (!exactKeys(response, ["ok", "observation"]) || record.ok !== true || !exactKeys(observation, ["observation_id", "url", "accessibility"])) {
    ...
    if (typeof observationId !== "string" || !/^[a-z0-9][a-z0-9._-]{0,79}$/i.test(observationId))
        fail("malformed_observation", "Observation ID is invalid");
    const accessibility = cleanText(observation.accessibility, "Accessibility state", MAX_ACCESSIBILITY_BYTES, { multiline: true });
    return {
...
export function validateDecisionOutput(output, observation) {
    if (typeof output !== "string" || Buffer.byteLength(output, "utf8") > 4_096)
        fail("malformed_output", "Decision output must be bounded JSON text");
    let decision;
    try {
    ...
    if (/https?:\/\//i.test(output) || Object.hasOwn(decision ?? {}, "url") || Object.hasOwn((decision?.action ?? {}), "url"))
        fail("invalid_url", "Explorer decisions may not contain direct URLs");
    if (decision?.schema_version !== 1 || !["act", "done"].includes(decision?.kind))
        fail("malformed_output", "Decision kind or schema version is invalid");
    if (decision.kind === "done") {
        if (!exactKeys(decision, ["schema_version", "kind", "reason"]))
            fail("malformed_output", "Done decision contains unexpected fields");
        return { schema_version: 1, kind: "done", reason: validateReason(decision.reason) };
    }
    const action = decision.action;
    if (!exactKeys(decision, ["schema_version", "kind", "reason", "action"]) || !action || !ACTION_TYPES.has(action.type))
        fail("malformed_output", "Action decision contains unexpected fields");
    const reason = validateReason(decision.reason);
    if (action.type === "click") {
        if (!exactKeys(action, ["type", "ref"]) || !/^e\d+$/.test((action.ref ?? "")))
            fail("invalid_ref", "Click must name exactly one visible reference");
        const visible = observation.refs.get(action.ref);
        if (!visible || !CLICKABLE_ROLES.has(visible.role))
            fail("invalid_ref", "Click reference is stale, absent, or not clickable");
        return { schema_version: 1, kind: "act", reason, action: { type: "click", ref: action.ref } };
    }
    if (action.type === "type") {
        if (!exactKeys(action, ["type", "ref", "text", "replace"]) || !/^e\d+$/.test((action.ref ?? "")) || typeof action.replace !== "boolean")
            fail("invalid_ref", "Type must name one visible input reference and replace policy");
        const visible = observation.refs.get(action.ref);
        if (!visible || !TYPEABLE_ROLES.has(visible.role))
            fail("invalid_ref", "Type reference is stale, absent, or not an input");
    ...
    if (!exactKeys(action, ["type", "direction"]) || !["up", "down"].includes(action.direction))
        fail("malformed_output", "Scroll direction is invalid");
...
export function validateActionOutcome(response) {
    ...
    const outcomeKeys = outcome?.protocol === undefined ? ["outcome_id", "status", "summary"] : ["outcome_id", "status", "summary", "protocol"];
    if (!exactKeys(response, ["ok", "outcome"]) || record.ok !== true || !exactKeys(outcome, outcomeKeys)) {
        fail("malformed_action_outcome", "Supervisor action outcome has an unexpected shape");
    }
    if (typeof outcome.outcome_id !== "string" || !/^[a-z0-9][a-z0-9._-]{0,79}$/i.test(outcome.outcome_id))
        fail("malformed_action_outcome", "Action outcome ID is invalid");
    if (!["completed", "rejected", "unknown"].includes(outcome.status))
        fail("malformed_action_outcome", "Action outcome status is invalid");
    if (outcome.protocol !== undefined && (outcome.status !== "completed" || outcome.protocol !== "scope_complete"))
        fail("malformed_action_outcome", "Action outcome protocol is invalid");
    return {
        outcome_id: outcome.outcome_id,
        status: outcome.status,
        summary: cleanText(outcome.summary, "Action outcome", 1_000, { multiline: true }),
...
{"schema_version":1,"kind":"done","reason":"why useful reachable exploration is exhausted"}`;
    if (Buffer.byteLength(prompt, "utf8") > MAX_MODEL_CONTENT_BYTES)
        fail("prompt_too_large", "Bound inputs do not fit the declared explorer model request");
    return prompt;
}
function brokerStop(response) {
    const refusal = response?.refusal;
    const code = refusal?.code;
    if (typeof code === "string" && /cap|deadline|budget/.test(code))
        return "cap";
    return "broker_abort";
}
...
export async function runExplorerLoop({ publicPack, rpc, mission = GENERIC_EXPLORER_MISSION, }) {
    if (!publicPack || typeof publicPack !== "object")
        fail("invalid_public_pack", "Validated public pack is required");
    if (typeof rpc !== "function")
        fail("invalid_rpc", "A supervised RPC transport is required");
    ...
    const oneAtATime = async (broker, payload) => {
        if (rpcOutstanding)
            fail("concurrent_rpc", "Explorer attempted overlapping broker requests");
        rpcOutstanding = true;
        try {
    ...
    if (!observeResponse?.ok)
        return halted(brokerStop(observeResponse), decisions, runHistory);
    let observation;
    try {
        observation = validateObservation(observeResponse);
    }
    catch (error) {
        return halted(thrownCode(error) ?? "malformed_observation", decisions, runHistory);
    }
    ...
    while (decisions < EXPLORER_DECISION_LIMIT) {
        ...
        try {
            ...
            return halted(thrownCode(error) ?? "protocol_error", decisions, runHistory);
        }
        let modelResponse;
        try {
        ...
        decisions += 1;
        if (!modelResponse?.ok)
            return halted(brokerStop(modelResponse), decisions, runHistory);
        let decision;
        try {
            decision = validateDecisionOutput(modelResponse.output, observation);
        }
        catch (error) {
            return halted(thrownCode(error) ?? "malformed_output", decisions, runHistory);
        }
        if (decision.kind === "done") {
        ...
        if (!actionResponse?.ok)
            return halted(brokerStop(actionResponse), decisions, runHistory);
        let outcome;
        try {
            outcome = validateActionOutcome(actionResponse);
        }
        catch (error) {
            return halted(thrownCode(error) ?? "malformed_action_outcome", decisions, runHistory);
        }
        ...
        if (!nextResponse?.ok)
            return halted(brokerStop(nextResponse), decisions, runHistory);
        let nextObservation;
        try {
        ...
        if (existing)
            ...
            runHistory.states.push(nextState);
