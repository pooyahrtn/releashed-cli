...
export type ClaimState = "current" | "uncertain" | "retired";
...
export interface RefIntegrity extends BoundReference {
  integrity: "ok" | "unavailable" | "origin_mismatch";
  detail: string;
  screenshot_absolute_path: string;
}
...
export interface NoteRead {
  ...
  claims: Array<{
    ...
    superseded_by?: string;
    retired_reason?: string;
    references: RefIntegrity[];
  }>;
  supports_ok: boolean;
  unavailable_supports: string[];
  match: { matched_terms: number; of_terms: number };
}
...
function rejectExtra(value: Record<string, unknown>, allowed: string[], label: string): void {
  for (const key of Object.keys(value)) {
    if (!allowed.includes(key)) fail(`${label} has unsupported field ${JSON.stringify(key)}`);
  }
}

function cleanText(value: unknown, label: string, max: number, { allowEmpty = false }: { allowEmpty?: boolean } = {}): string {
  if (typeof value !== "string") fail(`${label} must be a string`);
  const text = value.trim();
  if (!allowEmpty && !text) fail(`${label} must be non-empty`);
  if (text.length > max) fail(`${label} is too long (${text.length} chars; limit ${max})`);
  if (CONTROL_CHARS.test(text)) fail(`${label} contains control characters`);
  for (const shape of SECRET_SHAPES) {
    if (shape.pattern.test(text)) fail(`${label} looks like it contains ${shape.name}; only generalized product behavior belongs in notes`);
  }
  return text;
}

function cleanId(value: unknown, label: string, pattern: RegExp): string {
...
export function assertOrigin(url: unknown): string {
  if (typeof url !== "string") fail("url must be a string");
  let parsed: URL;
  try {
...
const originKey = (origin: string): string => createHash("sha256").update(origin).digest("hex");
const storeRoot = (mapsRoot: string): string => dirname(resolve(mapsRoot));

function knowledgeDir(mapsRoot: string, origin: string): string {
...
async function containedUnder(base: string, segments: string[]): Promise<string> {
...
async function notePathFor(mapsRoot: string, origin: string, flowId: string, suffix = ".json"): Promise<string> {
...
async function readStoredNote(notePath: string): Promise<StoredNote | null> {
  let raw: string;
  try {
  ...
  return parsed as unknown as StoredNote;
}
...
function assertStoredNote(note: StoredNote, notePath: string): void {
  if (!note || typeof note !== "object" || Array.isArray(note)) fail(`stored note is invalid: ${notePath}`);
  if (note.schema_version !== 1) fail(`stored note has unsupported schema: ${notePath}`);
  if (typeof note.flow_id !== "string" || !FLOW_ID.test(note.flow_id) || note.flow_id.length > MAX_ID) fail(`stored note flow_id is invalid: ${notePath}`);
  if (typeof note.origin !== "string") fail(`stored note origin is invalid: ${notePath}`);
  try {
    assertOrigin(note.origin);
  } catch {
    fail(`stored note origin is invalid: ${notePath}`);
  }
  if (typeof note.title !== "string" || !note.title.trim() || note.title.length > MAX_TITLE) fail(`stored note title is invalid: ${notePath}`);
  if (!Number.isInteger(note.revision) || note.revision < 1) fail(`stored note revision is invalid: ${notePath}`);
  if (typeof note.modified_at !== "string" || Number.isNaN(Date.parse(note.modified_at))) fail(`stored note modified_at is invalid: ${notePath}`);
  ...
  if (note.author_context_verified !== false) fail(`stored note author_context_verified is invalid: ${notePath}`);
  if (!Array.isArray(note.claims) || note.claims.length > MAX_CLAIMS) fail(`stored note claims are invalid: ${notePath}`);
  const ids = new Set<string>();
  for (const [index, claim] of note.claims.entries()) {
    const label = `stored note claims[${index}]`;
    if (!claim || typeof claim !== "object" || Array.isArray(claim)) fail(`${label} is invalid: ${notePath}`);
    if (typeof claim.id !== "string" || !CLAIM_ID.test(claim.id) || claim.id.length > MAX_ID) fail(`${label} id is invalid: ${notePath}`);
    if (ids.has(claim.id)) fail(`${label} duplicates id ${JSON.stringify(claim.id)}: ${notePath}`);
    ids.add(claim.id);
    if (typeof claim.text !== "string" || !claim.text.trim() || claim.text.length > MAX_TEXT) fail(`${label} text is invalid: ${notePath}`);
    if (typeof claim.conditions !== "string" || !claim.conditions.trim() || claim.conditions.length > MAX_CONDITIONS) fail(`${label} conditions are invalid: ${notePath}`);
    if (claim.state !== "current" && claim.state !== "uncertain" && claim.state !== "retired") fail(`${label} state is invalid: ${notePath}`);
    ...
    for (const [refIndex, ref] of claim.references.entries()) {
      ...
      if (!ref || typeof ref !== "object" || Array.isArray(ref)) fail(`${refLabel} is invalid: ${notePath}`);
      try {
        assertRunId(ref.run_id, "stored reference run_id");
      } catch {
        fail(`${refLabel} run_id is invalid: ${notePath}`);
      }
      if (typeof ref.manifest_sha256 !== "string" || !HEX64.test(ref.manifest_sha256)) fail(`${refLabel} manifest digest is invalid: ${notePath}`);
      if (typeof ref.screenshot_path !== "string" || !ref.screenshot_path.startsWith("screenshots/") || isAbsolute(ref.screenshot_path) || ref.screenshot_path.split(/[\\/]/).includes("..")) fail(`${refLabel} path is invalid: ${notePath}`);
      if (typeof ref.screenshot_sha256 !== "string" || !HEX64.test(ref.screenshot_sha256)) fail(`${refLabel} image digest is invalid: ${notePath}`);
      if (ref.event_id !== null && typeof ref.event_id !== "string") fail(`${refLabel} event_id is invalid: ${notePath}`);
      if (ref.side !== "before" && ref.side !== "after") fail(`${refLabel} side is invalid: ${notePath}`);
      if (ref.captured_at !== null && (typeof ref.captured_at !== "string" || Number.isNaN(Date.parse(ref.captured_at)))) fail(`${refLabel} captured_at is invalid: ${notePath}`);
      if (typeof ref.cited_url !== "string" || !ref.cited_url) fail(`${refLabel} cited_url is invalid: ${notePath}`);
    }
    for (const key of ["supersedes", "superseded_by", "retired_reason"] as const) {
      const value = claim[key];
      if (value !== undefined && (typeof value !== "string" || !value)) fail(`${label} ${key} is invalid: ${notePath}`);
    }
  }
}
...
async function containedRunDir(mapsRoot: string, runId: string): Promise<string | null> {
...
async function bindReference(
  mapsRoot: string,
  origin: string,
  ref: { run_id: string; screenshot_path: string },
): Promise<BoundReference> {
  assertRunId(ref.run_id, "reference run_id");
  if (typeof ref.screenshot_path !== "string" || !ref.screenshot_path) fail("reference screenshot_path must be a string");
  ...
  if (isAbsolute(ref.screenshot_path)) {
    ...
    if (!file) fail(`reference image is not retained: ${ref.run_id} ${ref.screenshot_path}`);
    ...
    if (ref.screenshot_path.split(/[\\/]/).includes("..")) fail(`reference image escapes run ${ref.run_id}`);
    rel = ref.screenshot_path;
  }
  const found = await findCapture({ mapsRoot, runId: ref.run_id }).catch(() => null);
  if (!found) fail(`reference run is not a retained sealed capture: ${ref.run_id}`);
  if (!found.selection) fail(`reference run ${ref.run_id} has no verified selected originals; a stop-image fallback cannot support a note`);
  ...
  if (!manifestBytes) fail(`reference manifest is unavailable: ${ref.run_id}`);
  ...
  if (!imageBytes) fail(`reference image is missing or changed after sealing: ${ref.run_id} ${rel}`);
  const imageSha = digest(imageBytes);
  const traceBytes = await sealedFile(candidateDir, manifest, "observations.jsonl");
  if (!traceBytes) fail(`reference trace is unavailable: ${ref.run_id}`);
  const matches: Array<{ event_id: string | null; side: "before" | "after"; url: unknown; timestamp: unknown }> = [];
  for (const line of String(traceBytes).split("\n").filter(Boolean)) {
    let event: { event_id?: unknown; timestamp?: unknown; before?: { screenshot_path?: unknown; url?: unknown }; after?: { screenshot_path?: unknown; url?: unknown } };
    try {
  ...
  if (matches.length === 0) fail(`reference image has no retained observation event: ${ref.run_id} ${rel}`);
  for (const match of matches) {
    if (typeof match.url !== "string" || !match.url) fail(`reference observation has no usable origin: ${ref.run_id} ${rel}`);
  }
  const origins = new Set(
    matches.map((match) => {
      try {
        return new URL(match.url as string).origin;
      } catch {
        return null;
      }
    }),
  );
  if (origins.size !== 1 || origins.has(null)) fail(`reference observation origin is missing or ambiguous: ${ref.run_id} ${rel}`);
  const citedUrl = matches[0]!.url as string;
...
const refIdentity = (ref: BoundReference): string =>
  `${ref.run_id}|${ref.manifest_sha256}|${ref.screenshot_path}|${ref.screenshot_sha256}`;

/** Revalidate a retained bound reference. Reports integrity; never throws. */
async function revalidateReference(mapsRoot: string, origin: string, ref: BoundReference): Promise<RefIntegrity> {
  const absolute = resolve(resolve(mapsRoot), ref.run_id, ref.screenshot_path);
  const unavailable = (detail: string): RefIntegrity => ({
    ...ref, integrity: "unavailable", detail, screenshot_absolute_path: absolute,
  });
  const candidateDir = join(resolve(mapsRoot), ref.run_id);
  if (!await containedRunDir(mapsRoot, ref.run_id)) return unavailable("run directory is outside the product store");
  const manifestBytes = await readFile(join(candidateDir, "manifest.json")).catch(() => null);
  if (!manifestBytes) return unavailable("manifest is missing");
  if (digest(manifestBytes) !== ref.manifest_sha256) return unavailable("manifest changed after the note cited it");
  const manifest = JSON.parse(manifestBytes.toString("utf8")) as { files: Array<{ path: string; sha256: string }> };
  const imageBytes = await sealedFile(candidateDir, manifest, ref.screenshot_path);
  if (!imageBytes) return unavailable("image is missing or no longer matches the sealed manifest");
  if (digest(imageBytes) !== ref.screenshot_sha256) return unavailable("image bytes changed after the note cited them");
  const traceBytes = await sealedFile(candidateDir, manifest, "observations.jsonl");
  if (!traceBytes) return unavailable("observation trace is unavailable");
  ...
  if (exact.length === 0) return unavailable("cited observation event and side are no longer retained");
  const observed = exact[0] as { event_id: string | null; side: "before" | "after"; url: unknown; timestamp: unknown };
  let eventOrigin: string | null = null;
  try {
  ...
  if (observed.url !== ref.cited_url) return unavailable("cited url does not match the retained observation");
  const stamp = observed.timestamp;
  const derived = typeof stamp === "string" && stamp.trim() && !Number.isNaN(Date.parse(stamp))
    ? new Date(stamp).toISOString()
    : null;
  if (derived !== ref.captured_at) return unavailable("cited capture date does not match the retained observation");
  return { ...ref, integrity: "ok", detail: "matches the sealed evidence", screenshot_absolute_path: absolute };
}

function validateProposal(note: unknown): ResolvedProposal {
  const inputBytes = Buffer.byteLength(JSON.stringify(note ?? null), "utf8");
  if (inputBytes > MAX_NOTE_INPUT_BYTES) fail(`note proposal is too large (${inputBytes} bytes; limit ${MAX_NOTE_INPUT_BYTES})`);
  ...
  if (!Array.isArray(claimsRaw)) fail("claims must be a list");
  const retireRaw = note.retire ?? [];
  if (!Array.isArray(retireRaw)) fail("retire must be a list");
  if (claimsRaw.length === 0 && retireRaw.length === 0) fail("note must add, support or retire at least one claim");
  if (claimsRaw.length > MAX_CLAIMS) fail(`too many proposed claims (${claimsRaw.length}; limit ${MAX_CLAIMS})`);
  if (retireRaw.length > MAX_RETIRE) fail(`too many retirements (${retireRaw.length}; limit ${MAX_RETIRE})`);
  const claims = claimsRaw.map((item, index) => {
    ...
    if (state !== "current" && state !== "uncertain") fail(`claims[${index}].state must be current or uncertain`);
    ...
    const references = (item.references as unknown[]).map((entry, refIndex) => {
      ...
      if (typeof record.screenshot_path !== "string" || !record.screenshot_path) fail(`claims[${index}].references[${refIndex}].screenshot_path must be a string`);
      return { run_id: record.run_id as string, screenshot_path: record.screenshot_path as string };
    });
    let supersedes: string | undefined;
    let reason: string | undefined;
    if (item.supersedes !== undefined) {
      supersedes = cleanId(item.supersedes, `claims[${index}].supersedes`, CLAIM_ID);
      if (supersedes === id) fail(`claims[${index}] cannot supersede itself`);
      if (item.reason === undefined) fail(`claims[${index}].reason is required when superseding ${supersedes}`);
    }
    if (item.reason !== undefined) reason = cleanText(item.reason, `claims[${index}].reason`, MAX_REASON);
    return { id, text, conditions, state: state as "current" | "uncertain", references, supersedes, reason };
  });
  const seen = new Set<string>();
  for (const claim of claims) {
    if (seen.has(claim.id)) fail(`duplicate claim id ${JSON.stringify(claim.id)} in one proposal`);
    seen.add(claim.id);
  }
...
export async function rememberFlow({ mapsRoot, url, expected_revision, note }: {
  ...
  if (!Number.isInteger(expected_revision) || expected_revision < 0) fail("expected_revision must be a non-negative whole number");
  ...
  try {
    ...
    if (stored) assertStoredNote(stored, notePath);
    if (expected_revision === 0) {
      if (stored) fail(`note ${flowId} already exists at revision ${stored.revision}; reread it and retry with expected_revision ${stored.revision}`);
    } else {
      if (!stored) fail(`note ${flowId} does not exist; create it with expected_revision 0`);
      if (stored.revision !== expected_revision)
        fail(`note ${flowId} is at revision ${stored.revision}, not ${expected_revision}; reread it and retry`);
      if (stored.origin !== origin || stored.flow_id !== flowId) fail(`stored note identity changed under ${flowId}; refusing to write`);
    ...
    for (const entry of proposal.retire ?? []) {
      ...
      if (!old) fail(`cannot retire unknown claim ${JSON.stringify(entry.id)}`);
      if (old.state === "retired") fail(`claim ${JSON.stringify(entry.id)} is already retired; terminal retirement never revives`);
    ...
    for (const claim of proposal.claims ?? []) {
      ...
      if (old) {
        if (old.state === "retired") fail(`claim ${JSON.stringify(claim.id)} is retired; terminal retirement never revives under the same id`);
        if (retiring.has(claim.id)) fail(`claim ${JSON.stringify(claim.id)} is retired by this note and cannot gain new supports in the same write`);
        if (old.text !== claim.text) fail(`claim ${JSON.stringify(claim.id)} text is immutable; propose a correction that supersedes it instead`);
        if (old.conditions !== claim.conditions) fail(`claim ${JSON.stringify(claim.id)} conditions are immutable; propose a correction that supersedes it instead`);
        if (claim.supersedes !== undefined) fail(`existing claim ${JSON.stringify(claim.id)} cannot supersede; a correction needs a new claim id`);
        if (claim.reason !== undefined) fail(`claims[${JSON.stringify(claim.id)}].reason needs supersedes; plain support updates carry no reason`);
      ...
      if ((proposal.retire ?? []).some((entry) => entry.id === claim.id)) fail(`claim ${JSON.stringify(claim.id)} is retired by this note and cannot be re-added`);
      let supersedes: string | undefined;
      if (claim.supersedes !== undefined) {
        const target = existing.get(claim.supersedes);
        if (!target) fail(`correction target ${JSON.stringify(claim.supersedes)} does not exist`);
        if (target.state === "retired") fail(`correction target ${JSON.stringify(claim.supersedes)} is already retired`);
        if (retiring.has(claim.supersedes)) fail(`correction target ${JSON.stringify(claim.supersedes)} is retired by this note; reference it instead of superseding`);
      ...
      if (claim.reason !== undefined && supersedes === undefined) fail(`claims reason needs supersedes; plain claims carry no reason`);
    ...
    for (const claim of existing.values()) {
      if (claim.state === "retired" || retiring.has(claim.id)) continue;
      for (const ref of claim.references) {
        const check = await revalidateReference(mapsRoot, origin, ref);
        if (check.integrity !== "ok") unavailable.push(`${claim.id}:${ref.run_id}:${ref.screenshot_path} (${check.detail})`);
      }
    }
    ...
    if (next.claims.length > MAX_CLAIMS) fail(`note would hold ${next.claims.length} claims; limit ${MAX_CLAIMS}`);
    for (const claim of next.claims) {
      if (claim.references.length === 0) fail(`claim ${JSON.stringify(claim.id)} has no supports`);
      if (claim.references.length > MAX_REFS_PER_CLAIM) fail(`claim ${JSON.stringify(claim.id)} has too many supports`);
    }
...
const normalTerms = (value: unknown): string[] =>
  String(value ?? "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim().split(" ").filter((term) => term.length > 1);

/** Reads return the full already-bounded title/text/conditions in JSON;
 *  only the human-readable summary truncates for display. */
async function readNote(mapsRoot: string, origin: string, flowId: string, queryTerms: string[]): Promise<NoteRead | null> {
  const notePath = await notePathFor(mapsRoot, origin, flowId);
  const stored = await readStoredNote(notePath);
  if (!stored) return null;
  assertStoredNote(stored, notePath);
  if (stored.origin !== origin || stored.flow_id !== flowId) return null;
  ...
  if (queryTerms.length > 0 && matched === 0) return null;
  const unavailable: string[] = [];
  const claims = [];
  for (const claim of stored.claims) {
    const references: RefIntegrity[] = [];
    for (const ref of claim.references) {
      const check = await revalidateReference(mapsRoot, stored.origin, ref);
      if (check.integrity !== "ok" && claim.state !== "retired") unavailable.push(`${claim.id}:${ref.run_id}:${ref.screenshot_path} (${check.detail})`);
...
/** Read-only memory lookup: the existing findCaptures result, unchanged,
 *  plus separately labelled product notes. Never creates, locks or writes.
 *  Notes never promote candidates, freshness or next_step. A malformed
 *  note never loses the capture lookup: per-note failures are isolated in
 *  notes_errors with an explicit reason. */
export type MemoryLookupResult = CaptureMemoryResult & {
  product_notes: NoteRead[];
  notes_guidance: string;
  notes_errors: string[];
  notes_scope: { origins_scanned: number; origins_capped: boolean; flows_capped: boolean };
};
...
export async function findMemory({ mapsRoot, url = null, query = "", ...options }: FreshnessOptions & {
  mapsRoot: string;
  url?: string | null;
  query?: string;
  ...
  if (trimmedQuery.length > MAX_QUERY) fail(`query is too long (${trimmedQuery.length} chars; limit ${MAX_QUERY})`);
  ...
  const roots: Array<{ dir: string; key: string }> = [];
  ...
  if (origin) {
    ...
    for (const entry of sliced) roots.push({ dir: join(realKnowledge, entry), key: entry });
  }
  const productNotes: NoteRead[] = [];
  ...
  for (const root of roots) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (read) productNotes.push(read);
      } catch (error) {
        notesErrors.push(`${flowId}: ${(error as Error).message}`);
      }
    }
  }
...
export interface CompactNoteReference {
  run_id: string;
  screenshot_path: string;
  screenshot_absolute_path: string;
  screenshot_sha256: string;
...
export interface CompactNoteClaim {
  id: string;
  text: string;
  conditions: string;
  state: "current" | "uncertain";
  supersedes?: string;
  superseded_by?: string;
  references: CompactNoteReference[];
}
...
export interface CompactNote {
  ...
  revision: number;
  modified_at: string;
  claims: CompactNoteClaim[];
  supports_ok: boolean;
  unavailable_supports: string[];
  match: { matched_terms: number; of_terms: number };
}

export interface NotesLookupResult {
  product_notes: CompactNote[];
  notes_guidance: string;
  notes_errors: string[];
  notes_scope: { origins_scanned: number; origins_capped: boolean; flows_capped: boolean };
}
...
export async function findNotes({ mapsRoot, url, query }: {
  ...
  url: string;
  query: string;
}): Promise<NotesLookupResult> {
  if (typeof url !== "string" || !url.trim()) fail("notes needs a product url");
  const origin = assertOrigin(url);
  if (typeof query !== "string") fail("notes needs --goal '<customer evidence question>' as text; objects and other payloads are not questions");
  ...
  if (question.length > MAX_QUERY) fail(`query is too long (${question.length} chars; limit ${MAX_QUERY})`);
  const full = await findMemory({ mapsRoot, url: origin, query: question });
  const productNotes: CompactNote[] = [];
  for (const note of full.product_notes) {
    ...
    productNotes.push({
      ...
      claims: active.map((claim) => ({
        ...
        state: claim.state as "current" | "uncertain",
        ...(claim.supersedes ? { supersedes: claim.supersedes } : {}),
        ...(claim.superseded_by ? { superseded_by: claim.superseded_by } : {}),
...
async function originOfNote(path: string): Promise<string | null> {
...
export function formatCompactNotes(notes: CompactNote[]): string {
  if (notes.length === 0) return "No product notes for this question. This is a compact active-claim view; full history, including retired claims, remains available through memory.";
  const lines: string[] = [];
  for (const note of notes) {
    ...
    for (const claim of note.claims) {
      ...
      if (claim.supersedes) lines.push(`    corrects: ${claim.supersedes}`);
      for (const ref of claim.references) {
    ...
    if (!note.supports_ok) lines.push(`  Unavailable supports: ${note.unavailable_supports.join("; ")}`);
  }
...
export function formatNotesNotices(result: NotesLookupResult): string {
  const lines: string[] = [];
  for (const error of result.notes_errors) lines.push(`  note error: ${error}`);
...
export function formatRememberResult(result: RememberResult): string {
  ...
  if (result.added_claims.length > 0) lines.push(`New claims: ${result.added_claims.join(", ")}.`);
  if (result.retired_claims.length > 0) lines.push(`Retired claims: ${result.retired_claims.join(", ")}.`);
  if (result.added_references.length > 0) lines.push(`New supports: ${result.added_references.join("; ")}.`);
  if (result.duplicate_references.length > 0) lines.push(`Already cited (not duplicated): ${result.duplicate_references.join("; ")}.`);
...
const summarize = (text: string): string =>
  text.length <= DISPLAY_TRUNCATE ? text : `${text.slice(0, DISPLAY_TRUNCATE)} [truncated; full text in JSON]`;

export function formatNotes(notes: NoteRead[]): string {
  if (notes.length === 0) return "No product notes for this query. Only sealed capture candidates above (if any) apply.";
  const lines: string[] = [];
  for (const note of notes) {
    lines.push(`Note ${note.flow_id} (revision ${note.revision}, modified ${note.modified_at}): ${summarize(note.title)}`);
    ...
    for (const claim of note.claims) {
      lines.push(`  [${claim.state}] ${claim.id}: ${summarize(claim.text)}`);
      lines.push(`    conditions: ${summarize(claim.conditions)}`);
      if (claim.retired_reason) lines.push(`    retired: ${claim.retired_reason}${claim.superseded_by ? ` (superseded by ${claim.superseded_by})` : ""}`);
      if (claim.supersedes) lines.push(`    corrects: ${claim.supersedes}`);
      for (const ref of claim.references) {
        lines.push(`    - ${ref.integrity === "ok" ? "supported" : ref.integrity} ${ref.screenshot_absolute_path} (run ${ref.run_id}, captured ${ref.captured_at ?? "date unknown"}): ${ref.detail}`);
      }
