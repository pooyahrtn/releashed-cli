...
export async function stopProcessTree(child, graceMs = 500) {
    if (!child || !child.pid || !Number.isSafeInteger(child.pid))
        throw new Error("Process tree has no owned group identifier");
...
export function attachBoundedJsonLineReader(stream, { onMessage, onError }) {
    ...
    stream.on("data", (chunk) => {
        ...
        for (;;) {
            ...
            routing = routing.then(async () => {
                if (terminal)
                    throw new Error("Child returned a message after completion");
    ...
    stream.on("end", () => {
        if (!failed && buffer.trim())
            fail(new Error("Child returned truncated JSONL"));
        endResolve();
    });
...
export async function runBoundedJsonlChild({ command, args, options, timeoutMs, label, onMessage, graceMs = 250 }) {
    ...
    if (!stdout || !stderrStream || !stdin)
        throw new Error(`${label} child stdio is unavailable`);
    let stderr = "";
    ...
    const reader = attachBoundedJsonLineReader(stdout, {
        onMessage: (message) => onMessage(message, {
            send(value) {
                if (stdin.destroyed)
                    throw new Error(`${label} input closed`);
