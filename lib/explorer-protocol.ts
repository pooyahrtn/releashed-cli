...
export type ExplorerDecision =
  | { schema_version: 1; kind: "done"; reason: string }
  | { schema_version: 1; kind: "act"; reason: string; action: ExplorerAction };
...
export type CompactHistory = {
  visited_states: Omit<StateRecord, "fingerprint">[];
  action_attempts: ActionAttempt[];
  decision_budget: { used: number; remaining: number };
};

export type ExplorerLoopResult =
  | {
      status: "halted";
      stop_reason: string;
      decisions: number;
      detail: string | null;
      run_history: CompactHistory;
    }
  | {
      status: "done";
      stop_reason: string;
...
type PendingRpc = {
  ...
  reject: (error: ExplorerProtocolError) => void;
...
export class SequentialRpcTransport {
  ...
  timeoutMs: number;
  nextRpcId: number;
  pending: PendingRpc | null;
  terminalError: ExplorerProtocolError | null;
  constructor({
    write,
    timeoutMs = EXPLORER_RPC_TIMEOUT_MS,
  }: {
    write: (message: Record<string, unknown>) => void;
    timeoutMs?: number;
  }) {
...
function cleanText(
  ...
  name: string,
  maximumBytes: number,
  { multiline = false, synthetic = false }: { multiline?: boolean; synthetic?: boolean } = {},
): string {
...
function parseVisibleRefs(accessibility: string): Map<string, { role: string; name: string }> {
...
export function validateBoundPublicPack(
  publicPackBytes: unknown,
  expectedSha256: unknown,
): { pack: unknown; sha256: string } {
  ...
  if (!/^[0-9a-f]{64}$/.test((expectedSha256 ?? "") as string)) fail("invalid_public_pack_binding", "Public pack binding must be a SHA-256 digest");
...
export function validateObservation(response: unknown): ValidatedObservation {
...
export function observationFingerprint(observation: { url: string; accessibility: string }): string {
...
export function validateDecisionOutput(output: unknown, observation: ValidatedObservation): ExplorerDecision {
  ...
  if (/https?:\/\//i.test(output) || Object.hasOwn(decision ?? {}, "url") || Object.hasOwn((decision?.action ?? {}) as object, "url")) fail("invalid_url", "Explorer decisions may not contain direct URLs");
  if (decision?.schema_version !== 1 || !["act", "done"].includes(decision?.kind as string)) fail("malformed_output", "Decision kind or schema version is invalid");
  ...
  if (!exactKeys(decision, ["schema_version", "kind", "reason", "action"]) || !action || !ACTION_TYPES.has(action.type as string)) fail("malformed_output", "Action decision contains unexpected fields");
  const reason = validateReason(decision.reason);
  if (action.type === "click") {
    if (!exactKeys(action, ["type", "ref"]) || !/^e\d+$/.test((action.ref ?? "") as string)) fail("invalid_ref", "Click must name exactly one visible reference");
    const visible = observation.refs.get(action.ref as string);
    if (!visible || !CLICKABLE_ROLES.has(visible.role)) fail("invalid_ref", "Click reference is stale, absent, or not clickable");
    return { schema_version: 1, kind: "act", reason, action: { type: "click", ref: action.ref as string } };
  }
  if (action.type === "type") {
    if (!exactKeys(action, ["type", "ref", "text", "replace"]) || !/^e\d+$/.test((action.ref ?? "") as string) || typeof action.replace !== "boolean") fail("invalid_ref", "Type must name one visible input reference and replace policy");
    const visible = observation.refs.get(action.ref as string);
    if (!visible || !TYPEABLE_ROLES.has(visible.role)) fail("invalid_ref", "Type reference is stale, absent, or not an input");
    return { schema_version: 1, kind: "act", reason, action: { type: "type", ref: action.ref as string, text: cleanText(action.text, "Synthetic input", 160, { synthetic: true }), replace: action.replace as boolean } };
  }
  if (!exactKeys(action, ["type", "direction"]) || !["up", "down"].includes(action.direction as string)) fail("malformed_output", "Scroll direction is invalid");
  return { schema_version: 1, kind: "act", reason, action: { type: "scroll", direction: action.direction as "up" | "down" } };
}

export function validateActionOutcome(response: unknown): ActionOutcome {
  const record = (response ?? {}) as Record<string, unknown>;
  ...
  if (typeof outcome.outcome_id !== "string" || !/^[a-z0-9][a-z0-9._-]{0,79}$/i.test(outcome.outcome_id)) fail("malformed_action_outcome", "Action outcome ID is invalid");
  if (!["completed", "rejected", "unknown"].includes(outcome.status as string)) fail("malformed_action_outcome", "Action outcome status is invalid");
  if (outcome.protocol !== undefined && (outcome.status !== "completed" || outcome.protocol !== "scope_complete")) fail("malformed_action_outcome", "Action outcome protocol is invalid");
  return {
    outcome_id: outcome.outcome_id,
    status: outcome.status as ActionOutcome["status"],
    summary: cleanText(outcome.summary, "Action outcome", 1_000, { multiline: true }),
    ...(outcome.protocol ? { protocol: outcome.protocol as string } : {})
  };
}

function stateRecord(observation: ValidatedObservation): StateRecord {
...
function compactRunHistory(runHistory: RunHistory, decisionsUsed: number): CompactHistory {
  return {
...
export function buildDecisionPrompt({
  publicPack,
  mission = GENERIC_EXPLORER_MISSION,
  observation,
  runHistory,
  decisionsUsed,
}: {
...
function brokerStop(response: unknown): string {
  const refusal = (response as { refusal?: unknown } | null | undefined)?.refusal as
...
function actionKey(stateId: string, action: ExplorerAction): string {
...
export async function runExplorerLoop({
  publicPack,
  rpc,
  mission = GENERIC_EXPLORER_MISSION,
}: {
  publicPack: unknown;
  rpc: (broker: string, payload: Record<string, unknown>) => Promise<Record<string, unknown>>;
  mission?: string;
}): Promise<ExplorerLoopResult> {
  ...
  const runHistory: RunHistory = { states: [], actionAttempts: [] };
  const attempted = new Set<string>();
  let decisions = 0;
  let rpcOutstanding = false;
  const oneAtATime = async (broker: string, payload: Record<string, unknown>): Promise<Record<string, unknown>> => {
  ...
  let observeResponse: Record<string, unknown> | undefined;
  try {
  ...
  while (decisions < EXPLORER_DECISION_LIMIT) {
    ...
    let nextResponse: Record<string, unknown> | undefined;
    try {
    ...
    let nextObservation: ValidatedObservation;
    try {
