...
function exactOrigin(raw) {
    ...
    requireCondition(["http:", "https:"].includes(url.protocol) &&
        !url.username &&
        !url.password &&
        url.pathname === "/" &&
        !url.search &&
        !url.hash, "Target origin must be a normalized HTTP(S) origin");
    return url.origin;
}
function clerkFrontendApiOrigin(originPolicy) {
    const allow = isRecord(originPolicy) &&
        isRecord(originPolicy.browser_request_dispatch)
        ? originPolicy.browser_request_dispatch.allow
        : undefined;
    const candidates = (Array.isArray(allow) ? allow : []).filter((entry) => typeof entry?.purpose === "string" &&
...
function ownerCaps(authority) {
    ...
    requireCondition(isRecord(limits) &&
        Object.entries(FROZEN_SPIKE_CAPS).every(([key, value]) => limits[key] === value), "Owner authority does not contain the exact frozen Spike A caps");
...
export function validateActionLedger(ledger, authority, { targetOrigin = null, ignoreAuthBootstrapAdmissions = false, } = {}) {
    requireCondition(isRecord(ledger) &&
        ledger.schema_version === 1 &&
        ledger.record_kind === "spike-a-action-and-app-cost-ledger" &&
        ledger.status === "pass", "Private app-action ledger is invalid");
    const caps = ownerCaps(authority);
    const ledgerCaps = ledger.caps;
    requireCondition(isRecord(ledgerCaps) &&
        ...
        ledgerCaps.combined_model_and_app_actual_plus_outstanding_reservations_eur ===
            caps.combined_actual_plus_reserved_cap_eur, "Private app-action ledger caps do not match owner authority");
    requireCondition(Array.isArray(ledger.allowed_one_way_classes) &&
        ledger.allowed_one_way_classes.length > 0, "Private app-action ledger has no allowed classes");
    ...
    for (const entry of ledger.allowed_one_way_classes) {
        ...
        requireCondition(entry.action_matrix_row === "Listed own-account progress" &&
            Number.isSafeInteger(entry.maximum_count) &&
            entry.maximum_count >= 0 &&
            finiteNonnegative(entry.worst_case_eur_per_action), "Private app-action ledger contains an invalid allowed class");
    ...
    if (!ignoreAuthBootstrapAdmissions &&
...
export function validateBoundedOnboardingLedger(ledger, authority, { targetOrigin = null, ignoreAuthBootstrapAdmissions = false, } = {}) {
    ...
    requireCondition(isRecord(ledger) && Array.isArray(ledger.allowed_one_way_classes), "Private app-action ledger has no allowed classes");
...
export function classifyBoundedOnboardingControl({ scopeHref, requested, observed, }) {
    if (!exactKeys(requested, ["ref", "observation_hash"]) ||
    ...
    if (typeof observed.role !== "string" ||
...
export function validateTargetActionRegistry(registry, { targetOrigin, actionLedger, authority, }) {
    ...
    requireCondition(exactKeys(registry, [
        "schema_version",
        "record_kind",
        "status",
        "target_origin",
        ...
        Array.isArray(registry.entries), "Private target-action registry schema is invalid");
    ...
    for (const entry of registry.entries) {
        requireCondition(exactKeys(entry, [
            ...
            entry.visible_control.required_visible_text.every((text) => typeof text === "string" &&
                text.trim() === text &&
                text.length > 0 &&
                text.length <= 160) &&
            ...
            /^[a-z][a-z0-9-]{0,95}$/.test(entry.effect_class_id), "Private target-action registry contains an invalid entry");
        ...
        if (entry.action_class === "Reversible own-account") {
            requireCondition(entry.expected_request === null &&
                entry.effect_class_id === "reversible-own-account", "Reversible registry entries must not authorize a mutation request");
        }
        else {
            ...
            requireCondition(typeof entry.expected_request.url === "string", "Listed registry entry mutation must be an exact same-origin URL without a fragment");
            const requestUrl = new URL(entry.expected_request.url);
            requireCondition(requestUrl.origin === origin &&
                requestUrl.href === entry.expected_request.url &&
                !requestUrl.hash, "Listed registry entry mutation must be an exact same-origin URL without a fragment");
...
export function classifyObservedControl({ registry, requested, observed, }) {
    ...
    if (!isRecord(observed) ||
        ...
        return {
            ...
            refusal: {
                code: "control_observation_stale",
                message: "The requested control is not bound to the current retained observation",
            },
        };
    }
    ...
    observedUrl.search = "";
    observedUrl.hash = "";
    const visibleSummary = typeof observed.visible_state_summary === "string"
        ? observed.visible_state_summary
        : "";
    const entries = registry.entries.filter((candidate) => candidate.visible_control.url === observedUrl.href &&
        candidate.visible_control.role === observed.role &&
        candidate.visible_control.accessible_name === observed.name &&
        candidate.visible_control.required_visible_text.every((text) => visibleSummary.includes(text)));
    if (entries.length !== 1)
        return {
            ok: false,
            refusal: {
                code: entries.length > 1 ? "ambiguous_visible_control" : "unknown_visible_control",
                message: "This visible control does not have one exact private safety classification",
            },
        };
    const [entry] = entries;
    return {
...
function validateCurrentCap({ cap, authority, ledgerRows, runId, }) {
    const caps = ownerCaps(authority);
    requireCondition(isRecord(cap) && cap.run_id === runId && cap.abort === null, "Current cap state is not usable for this run");
    const workingDayDeadline = cap.working_day_deadline;
    requireCondition(typeof workingDayDeadline === "string" &&
        Number.isFinite(Date.parse(workingDayDeadline)) &&
        Date.parse(workingDayDeadline) > Date.now(), "Original working-day deadline has expired");
    const capCaps = cap.caps;
    requireCondition(isRecord(capCaps) &&
        Object.entries(caps).every(([key, value]) => capCaps[key] === value), "Current cap state does not retain every original owner cap");
    ...
    requireCondition(reconciliations.every((row) => finiteNonnegative(row.actual_eur) &&
        row.reservation_within_bound === true &&
        row.credential_exposed === false), "Model ledger has an unsafe reconciliation");
    const model = cap.model;
    const app = cap.app;
    requireCondition(isRecord(model) &&
        reservations.length === reconciliations.length &&
        model.calls === reconciliations.length &&
        model.refused_before_dispatch === refused.length &&
        model.outstanding_reservations_eur === 0, "Current model call ledger is not fully reconciled");
    requireCondition(typeof model.actual_eur === "number" &&
        Math.abs(model.actual_eur - actual) < 1e-12 &&
        finiteNonnegative(model.actual_eur) &&
        model.actual_eur <= caps.model_cost_cap_eur, "Current model cost does not match its ledger");
    const modelActualEur = model.actual_eur;
    requireCondition(isRecord(app) &&
        app.actual_eur === 0 &&
        app.outstanding_reservations_eur === 0 &&
        app.one_way_actions === 0, "Target app budget was already consumed");
    requireCondition(modelActualEur + app.actual_eur <= caps.combined_actual_plus_reserved_cap_eur, "Current combined cost exceeds the original cap");
    return {
...
export async function buildTargetRuntimeConfig({ repository, ownerDirectory, runId, registryPath = null, mode = LEGACY_TARGET_RUNTIME_MODE, targetOriginOverride = null, testOnlyLocalFixture = false, anonymous = false, savedSessionPath = null, }) {
    ...
    const savedSession = savedSessionPath
        ...
        : null;
    const ownerSnapshots = Object.fromEntries(await Promise.all(PRIVATE_OWNER_FILES.map(async (file) => [
        file,
    ...
    requireCondition(authority.status === "pass" &&
        ...
        account.status === "pass" &&
        retention.status === "pass", "A frozen private owner input is not approved");
    ...
    const approvedTargetOrigins = (originPolicy.explorer_navigation?.allow ?? [
    ...
    const fixtureAllowed = testOnlyLocalFixture &&
        process.env.SPIKE_A_TEST_ONLY_TARGET_FIXTURE === "1";
    requireCondition(approvedTargetOrigins.includes(targetOrigin) ||
        (fixtureAllowed && targetOrigin.startsWith("http://127.0.0.1:")), "Target origin is not an owner-approved explorer navigation origin");
    if (boundedOnboarding)
        ...
        if (!fixtureAllowed)
            requireCondition(registry.entries.length > 0, "Production target exploration requires a nonempty frozen action registry");
    }
    validatePublicPack(publicPack);
    const contentFiles = contentManifest.files;
    requireCondition(contentManifest?.schema_version === 1 && Array.isArray(contentFiles), "Public-pack content manifest is invalid");
    const publicPackHash = publicPackSnapshot.sha256;
    const packRows = contentFiles.filter((entry) => entry?.path === "public-pack.json");
    ...
    return {
        ...
        private_owner_inputs: Object.fromEntries(PRIVATE_OWNER_FILES.map((file) => [
            file,
        ...
        target: {
            ...
            allowed_request_origins: fixtureAllowed
                ? [targetOrigin]
                : originPolicy.browser_request_dispatch.allow.map((entry) => entry.origin),
            suppressed_request_origins: fixtureAllowed
                ? []
                : originPolicy.browser_request_dispatch.recognized_but_suppress_before_dispatch.map((entry) => entry.origin),
        },
        outputs: generated,
        ...(fixtureAllowed
            ? {
...
export async function preflightTargetRuntime({ repository, configPath, }) {
    ...
    const anonymousBounded = boundedOnboarding &&
    ...
    requireCondition(hasKeys(config, expectedKeys) &&
        config.schema_version === 1 &&
        config.runtime_identity === TARGET_RUNTIME_ID &&
        [
            LEGACY_TARGET_RUNTIME_MODE,
            BOUNDED_ONBOARDING_RUNTIME_MODE,
        ].includes(config.mode), "Target runtime configuration schema is invalid");
    if (savedSessionBounded)
        requireCondition(hasKeys(config.saved_session, ["path", "sha256"]) &&
            /^[0-9a-f]{64}$/.test(config.saved_session.sha256), "Saved-session binding is invalid");
    const fixtureAllowed = config.test_only_local_fixture === true &&
        process.env.SPIKE_A_TEST_ONLY_TARGET_FIXTURE === "1";
    requireCondition(config.test_only_local_fixture !== true || fixtureAllowed, "Local target fixture mode is test-only");
    requireCondition(hasKeys(config.public_pack, ["path", "sha256"]) &&
        /^[0-9a-f]{64}$/.test(config.public_pack.sha256), "Target public-pack binding is invalid");
    requireCondition(hasKeys(config.private_owner_inputs, PRIVATE_OWNER_FILES), "Target private-owner binding set is invalid");
    for (const file of PRIVATE_OWNER_FILES)
        requireCondition(hasKeys(config.private_owner_inputs[file], ["path", "sha256"]) &&
            /^[0-9a-f]{64}$/.test(config.private_owner_inputs[file].sha256), `Target private-owner binding for ${file} is invalid`);
    if (!boundedOnboarding)
        requireCondition(hasKeys(config.target_action_registry, ["path", "sha256"]) &&
            /^[0-9a-f]{64}$/.test(config.target_action_registry.sha256), "Target registry binding is invalid");
    requireCondition(hasKeys(config.cap_state, ["path", "sha256", "binding"]) &&
        /^[0-9a-f]{64}$/.test(config.cap_state.sha256), "Target cap binding is invalid");
    requireCondition(hasKeys(config.model_ledger, ["path", "sha256"]) &&
        /^[0-9a-f]{64}$/.test(config.model_ledger.sha256), "Target model-ledger binding is invalid");
    requireCondition(hasKeys(config.target, [
        "origin",
        "initial_url",
    ...
    requireCondition(hasKeys(config.outputs, [
        "browser_config_path",
        "sandbox_input_path",
    ...
    if (fixtureAllowed)
        requireCondition(hasKeys(config.transient_auth, ["mode", "header_name", "origin"]) &&
            config.transient_auth.mode === "header" &&
            config.transient_auth.header_name === "x-flow-map-fixture-auth" &&
            config.transient_auth.origin === config.target.origin, "Synthetic fixture auth policy is invalid");
    if (!fixtureAllowed && !anonymousBounded) {
        ...
        requireCondition(hasKeys(config.clerk_auth, clerkKeys) &&
            config.clerk_auth.mode === "one-use-ticket", "Production Clerk auth policy is invalid");
    }
    const packPath = resolve(config.public_pack.path);
    requireCondition(relative(join(repo, "packs"), packPath) ===
        join(`public-pack-${config.run_id}`, "public-pack.json"), "Target runtime is not bound to the exact run public-pack.json");
    const [publicPackSnapshot, capSnapshot, modelLedgerSnapshot] = await Promise.all([
    ...
    const privateSnapshots = await Promise.all([
        ...
        ...(boundedOnboarding
            ? []
            : [
                readJsonSnapshot(config.target_action_registry.path, "Private target-action registry"),
            ]),
    ]);
    ...
    for (let index = 0; index < PRIVATE_OWNER_FILES.length; index += 1)
        requireCondition(privateSnapshots[index].sha256 ===
            config.private_owner_inputs[PRIVATE_OWNER_FILES[index]].sha256, `Private owner input ${PRIVATE_OWNER_FILES[index]} changed after target preparation`);
    const registrySnapshot = boundedOnboarding
        ? null
        : privateSnapshots.at(-1);
    if (!boundedOnboarding)
        requireCondition(registrySnapshot?.sha256 === config.target_action_registry.sha256, "Private target-action registry changed after preparation");
    const registry = registrySnapshot?.value ?? null;
    const privateValues = Object.fromEntries(PRIVATE_OWNER_FILES.map((file, index) => [
        file,
    ...
    requireCondition(JSON.stringify(currentCapBinding) ===
        JSON.stringify(config.cap_state.binding), "Current deadline, caps, or model cost no longer match target preparation");
    if (boundedOnboarding)
        validateBoundedOnboardingLedger(actionLedger, authority, {
    ...
    requireCondition(approvedTargetOrigins.includes(config.target.origin) ||
        (fixtureAllowed && config.target.origin.startsWith("http://127.0.0.1:")), "Target origin no longer matches the private owner policy");
    requireCondition(config.target.initial_url === `${config.target.origin}/` &&
        JSON.stringify(config.target.allowed_navigation_origins) ===
            JSON.stringify([config.target.origin]), "Target navigation policy is not exact");
    ...
    const expectedSuppressedOrigins = fixtureAllowed
        ? []
        : originPolicy.browser_request_dispatch.recognized_but_suppress_before_dispatch.map((entry) => entry.origin);
    requireCondition(JSON.stringify(config.target.allowed_request_origins) ===
        JSON.stringify(expectedRequestOrigins) &&
        JSON.stringify(config.target.suppressed_request_origins) ===
            JSON.stringify(expectedSuppressedOrigins), "Target request policy no longer matches the private owner policy");
    if (!fixtureAllowed && !anonymousBounded) {
        requireCondition(account?.identity?.provider === "Clerk" &&
            typeof account.identity.provider_user_id === "string", "Private account is not an approved Clerk identity");
        requireCondition(config.clerk_auth.frontend_api_origin ===
            clerkFrontendApiOrigin(originPolicy) &&
            config.target.allowed_request_origins.includes(config.clerk_auth.frontend_api_origin), "Production Clerk origin is not bound to the owner request policy");
        if (boundedOnboarding) {
            const identity = config.clerk_auth.approved_disposable_identity;
            requireCondition(hasKeys(identity, ["provider_user_id", "username"]) &&
                identity.provider_user_id === account.identity.provider_user_id &&
                identity.username === account.identity.username &&
                typeof identity.username === "string", "Bounded Clerk identity is not bound to the approved disposable account");
        }
    }
    const runtimeDirectory = join(repo, ".runtime", config.run_id);
    const sourceRunDirectory = join(repo, "runs", config.run_id);
    requireCondition(config.cap_state.path === join(runtimeDirectory, "cap-state.json") &&
        config.model_ledger.path ===
            join(sourceRunDirectory, "model-ledger.jsonl"), "Target cost bindings are not the exact source-run artifacts");
    requireCondition(JSON.stringify(config.outputs) ===
        JSON.stringify({
            browser_config_path: join(runtimeDirectory, "target-browser-config.json"),
            sandbox_input_path: join(runtimeDirectory, "target-sandbox-input.json"),
    ...
    return {
        ...
        clerkUserId: fixtureAllowed || anonymousBounded
            ? null
