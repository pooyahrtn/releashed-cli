...
import { provisionDisposableClerkIdentity, reconcileDisposableClerkRecovery, verifyDisposableClerkIdentity, } from "../supervisor/clerk-bootstrap.mjs";
...
const sameIdentity = (stat, expected) => Boolean(stat && !stat.isSymbolicLink() && stat.dev === expected.dev && stat.ino === expected.ino && (stat.mode & 0o777) === expected.mode);
...
async function directory(path, { privateMode = false } = {}) {
    ...
    if (!stat?.isDirectory() || stat.isSymbolicLink() || (privateMode && (stat.mode & 0o077) !== 0) || await realpath(path).catch(() => null) !== path)
        refused();
    return { path, identity: identity(stat) };
}
async function assertDirectory(directory) {
    const stat = await lstat(directory.path).catch(() => null);
    if (!stat?.isDirectory() || !sameIdentity(stat, directory.identity) || await realpath(directory.path).catch(() => null) !== directory.path)
        refused();
}
async function syncDirectory(directory) {
    ...
    try {
        ...
        if (!sameIdentity(await handle.stat(), directory.identity))
            refused();
        await handle.sync();
    }
    finally {
...
async function privateSnapshot(path, boundary = async () => { }) {
    ...
    if (!pathBefore?.isFile() || pathBefore.isSymbolicLink() || (pathBefore.mode & 0o777) !== 0o600 || await realpath(path).catch(() => null) !== path)
        refused();
    let handle;
    let bytes;
    let opened;
    try {
        ...
        if (!opened.isFile() || !sameIdentity(opened, identity(pathBefore)))
            refused();
        bytes = await handle.readFile();
        const afterRead = await handle.stat();
        if (!sameIdentity(afterRead, identity(opened)) || afterRead.size !== opened.size || afterRead.mtimeMs !== opened.mtimeMs || afterRead.ctimeMs !== opened.ctimeMs)
            refused();
    }
    finally {
        await handle?.close();
    ...
    if (!pathAfter?.isFile() || !sameIdentity(pathAfter, identity(opened)) || pathAfter.size !== opened.size || pathAfter.mtimeMs !== opened.mtimeMs || pathAfter.ctimeMs !== opened.ctimeMs)
        refused();
    await boundary();
...
async function assertSnapshot(snapshot, boundary) {
    ...
    if (current.identity.dev !== snapshot.identity.dev || current.identity.ino !== snapshot.identity.ino || current.identity.mode !== snapshot.identity.mode ||
...
async function writeOwned(path, bytes, owned, { boundary, parent, replace = false }) {
    ...
    await boundary();
    if (!sameIdentity(await lstat(path).catch(() => null), owned.get(path)))
        refused();
...
async function unlinkOwned(path, expected, { boundary, parent }) {
    await boundary();
    if (!sameIdentity(await lstat(path).catch(() => null), expected))
        refused();
    await unlink(path);
    await boundary();
    await syncDirectory(parent);
...
async function removeOwned(bundle, bundleRoot, parent, owned) {
    ...
    if ((await readdir(bundle)).length !== 0)
        return false;
    await rmdir(bundle);
    await syncDirectory(parent);
...
function clerkOrigin(policy) {
    const rows = policy?.browser_request_dispatch?.allow?.filter((row) => typeof row?.purpose === "string" && /Clerk frontend\/auth session transport only/i.test(row.purpose)) ?? [];
    if (policy?.status !== "pass" || rows.length !== 1)
        refused();
...
function stateValue(bytes, runId) {
    ...
    if (!exactKeys(value, ["schema_version", "kind", "run_id", "clerk_frontend_api_origin", "base_control_hashes"]) ||
...
function recoveryValue(bytes, { runId, stateSha256 }) {
    ...
    if (!exactKeys(value, ["schema_version", "kind", "run_id", "bundle_state_sha256", "state", "external_id", "user_id"]) ||
...
function accountValue(value, { userId, username }) {
    ...
    if (!USER_ID.test(userId ?? "") || !/^flowmap_[0-9a-f]{32}$/.test(username ?? "") ||
...
function completionValue(bytes, { runId, stateSha256 }) {
    ...
    if (!exactKeys(value, ["schema_version", "kind", "run_id", "bundle_state_sha256", "account_sha256", "fresh_start_contract_sha256"]) ||
...
async function inspectExisting(bundle, runId, parent) {
    ...
    for (const name of OWNED_NAMES) {
        if (entries.includes(name))
            snapshots.set(name, await privateSnapshot(join(bundle, name), boundary));
    }
    ...
    if (final || finalWithRecovery) {
        for (const name of BUNDLE_CONTROL_FILES)
            if (snapshots.get(name)?.sha256 !== state.base_control_hashes[name])
                refused();
        let originPolicy;
        let account;
        let contract;
        ...
        if (clerkOrigin(originPolicy) !== state.clerk_frontend_api_origin)
            refused();
        ...
        if (completion.account_sha256 !== snapshots.get("account.json").sha256 || completion.fresh_start_contract_sha256 !== snapshots.get("fresh-start-contract.json").sha256)
            refused();
        if (finalWithRecovery) {
            ...
            if (recovery.state !== "identity-bound" || recovery.user_id !== account?.identity?.provider_user_id || username !== account?.identity?.username)
                refused();
            return {
                complete: false,
                preProvision: false,
        ...
        await boundary();
        if (!exactNames(await readdir(bundle), FINAL_NAMES))
            refused();
    ...
    if (FINAL_NAMES.every((name) => entries.includes(name)))
        refused();
    if (!snapshots.has("recovery.json"))
        refused();
    const recovery = recoveryValue(snapshots.get("recovery.json").bytes, { runId, stateSha256: stateFile.sha256 });
    ...
    if (snapshots.has("recovery.json.next")) {
        ...
        if (recovery.state !== "creation-pending" || next.state !== "identity-bound" || next.external_id !== recovery.external_id)
            refused();
...
async function snapshotBase(base) {
    ...
    let policy;
    try {
        policy = JSON.parse(snapshots["origin-allowlist.json"].bytes.toString("utf8"));
    }
    catch {
        refused();
    }
...
export async function createImmutableOwnerBundle({ baseOwnerDirectory, bundleParent, runId, secretKey, fetchImpl = fetch, dependencies = {} }) {
    ...
    if (await lstat(bundle).catch(() => null)) {
        ...
        if (!await removeOwned(bundle, existing.root, parent, existing.owned))
            refused();
    }
    ...
    if (bundleRoot.identity.mode !== 0o700)
        refused();
    await syncDirectory(parent);
    const boundary = async () => { await assertDirectory(parent); await assertDirectory(bundleRoot); };
    const owned = new Map();
    ...
    const provision = dependencies.provisionIdentity ?? provisionDisposableClerkIdentity;
    let lifecycle = null;
    try {
        for (const name of BUNDLE_CONTROL_FILES)
            await assertSnapshot(snapshots[name], () => assertDirectory(base));
        lifecycle = await provision({
            environment: { CLERK_SECRET_KEY: secretKey }, expectedFrontendOrigin: origin, fetchImpl,
            onRecoveryMaterial: async (material) => writeOwned(join(bundle, "recovery.json"), body({ schema_version: 1, kind: RECOVERY_KIND, run_id: runId, bundle_state_sha256: stateSha256, ...material }), owned, { boundary, parent: bundleRoot, replace: material?.state === "identity-bound" }),
        });
        const recovery = recoveryValue((await privateSnapshot(join(bundle, "recovery.json"), boundary)).bytes, { runId, stateSha256 });
        if (recovery.state !== "identity-bound" || recovery.user_id !== lifecycle.userId)
            refused();
        const username = `flowmap_${recovery.external_id.slice("flowmap_cal_".length)}`;
        ...
        for (const name of BUNDLE_CONTROL_FILES)
            await writeOwned(join(bundle, name), snapshots[name].bytes, owned, { boundary, parent: bundleRoot });
        ...
        for (const name of BUNDLE_CONTROL_FILES)
            await assertSnapshot(snapshots[name], () => assertDirectory(base));
        await boundary();
        if (!exactNames(await readdir(bundle), [...FINAL_NAMES, "recovery.json"]))
            refused();
        ...
        if (lifecycle) {
            ...
            if (cleanup?.cleanup_complete === true)
                await removeOwned(bundle, bundleRoot, parent, owned).catch(() => { });
        }
        else {
            let cleanup = null;
            if (error && typeof error === "object") {
                ...
                if (typeof cleanupHandle === "function")
                    cleanup = await cleanupHandle().catch(() => cleanup);
            }
            if (cleanup !== null && typeof cleanup === "object" && "cleanup_complete" in cleanup && cleanup.cleanup_complete === true)
                await removeOwned(bundle, bundleRoot, parent, owned).catch(() => { });
        }
        refused();
    }
