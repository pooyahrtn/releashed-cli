...
import { retireDisposableClerkIdentity } from "../supervisor/clerk-bootstrap.mjs";
import { validateFreshStartContract } from "./fresh-start-contract.mjs";
export const OWNER_BUNDLE_RETIREMENT_RECEIPT = "account-retirement-receipt.json";
...
const sameIdentity = (stat, expected) => Boolean(stat &&
    !stat.isSymbolicLink() &&
...
async function captureDirectoryChain(path) {
    ...
    if (chain[0].identity.mode !== 0o700)
        refused();
    return chain;
}
...
async function privateSnapshot(path, chain) {
    await assertDirectoryChain(chain);
    ...
    try {
        ...
        if (!sameIdentity(afterRead, fileIdentity(opened)) ||
    ...
    if (!sameIdentity(after, fileIdentity(opened)) ||
...
async function assertSnapshot(snapshot, chain) {
...
function accountRecord({ userId, username, provisionedAt }) {
    return {
        ...
        browser_broker_handoff: {
            ...
            instruction: "Authenticate only through the supervisor's one-use Clerk ticket boundary.",
            explorer_disclosure: "Identity and authentication material remain supervisor-private.",
        },
        secret_storage: {
...
function expectedReceipt(hashes) {
    return {
        schema_version: 1,
...
async function inspectCertificate({ bundleDirectory, runId }) {
    ...
    if (exactNames(entries, [...CERTIFICATE_FILES, OWNER_BUNDLE_RETIREMENT_RECEIPT]))
        receiptState = "published";
    else if (exactNames(entries, [...CERTIFICATE_FILES, STAGED_RECEIPT]))
        receiptState = "staged";
    else if (!exactNames(entries, CERTIFICATE_FILES))
        refused();
    const snapshots = Object.fromEntries(await Promise.all(CERTIFICATE_FILES.map(async (name) => [
        name,
    ...
    if (!exactKeys(state, [
        ...
        !CONTROL_FILES.every((name) => HASH.test(state.base_control_hashes[name] ?? "") &&
            state.base_control_hashes[name] === snapshots[name].sha256))
        refused();
    const originPolicy = parseJson(snapshots["origin-allowlist.json"].bytes);
    const clerkRows = originPolicy?.browser_request_dispatch?.allow?.filter((row) => typeof row?.purpose === "string" &&
        /Clerk frontend\/auth session transport only/i.test(row.purpose)) ?? [];
    if (originPolicy?.status !== "pass" ||
        clerkRows.length !== 1 ||
    ...
    if (receiptState !== "absent") {
        const receiptName = receiptState === "published"
            ? OWNER_BUNDLE_RETIREMENT_RECEIPT
            : STAGED_RECEIPT;
        receiptSnapshot = await privateSnapshot(join(bundleDirectory, receiptName), chain);
        if (!receiptSnapshot.bytes.equals(bytesFor(receipt)))
            refused();
    }
    const expectedEntries = receiptState === "published"
    ...
    const assertUnchanged = async () => {
        await assertDirectoryChain(chain);
        if (!exactNames(await readdir(bundleDirectory), expectedEntries))
            refused();
        await Promise.all(Object.values(snapshots).map((snapshot) => assertSnapshot(snapshot, chain)));
        if (receiptSnapshot)
            await assertSnapshot(receiptSnapshot, chain);
        await assertDirectoryChain(chain);
    };
...
async function writeReceipt(certificate, dependencies) {
    ...
    const finalPath = join(certificate.bundleDirectory, OWNER_BUNDLE_RETIREMENT_RECEIPT);
    ...
    if (certificate.receiptState === "absent") {
        ...
        let handle;
        try {
            handle = await open(stagedPath, constants.O_WRONLY |
                constants.O_CREAT |
                constants.O_EXCL |
    ...
    if (!sameIdentity(await lstat(stagedPath).catch(() => null), stagedIdentity) ||
    ...
    if (!published.bytes.equals(bytesFor(certificate.receipt)))
        refused();
    await syncDirectory(certificate.bundleDirectory, certificate.chain);
    if (!exactNames(await readdir(certificate.bundleDirectory), [
        ...CERTIFICATE_FILES,
        OWNER_BUNDLE_RETIREMENT_RECEIPT,
    ]))
        refused();
}
export async function retireCompletedOwnerBundle({ bundleDirectory, runId, environment = process.env, fetchImpl = fetch, workingDayDeadline = null, dependencies = {}, }) {
    ...
    try {
        ...
        const providerResult = await (dependencies.retireIdentity ?? retireDisposableClerkIdentity)({
        ...
        if (providerResult?.retirement_confirmed !== true)
            refused();
        secretKey = null;
        await certificate.assertUnchanged();
