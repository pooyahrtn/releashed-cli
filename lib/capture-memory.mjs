...
export const REQUEST_FRAMING_WORDS = new Set("a an the me my we our you your it its this that these those of for from in on at to and or with please show see find give current currently latest flow flows screen screens screenshot screenshots image images".split(" "));
...
function scoredObservations(map, traceBytes, goal, origin, goalPaths) {
    const wanted = [...new Set(terms(goal))].filter((term) => !REQUEST_FRAMING_WORDS.has(term));
...
async function candidate(candidateDir, goal, origin) {
    ...
    const claimed_screen = claimedScreenRecord
        ? {
            title: typeof claimedScreenRecord.title === "string"
                ? claimedScreenRecord.title.slice(0, 200)
                : null,
            url: typeof claimedScreenRecord.url === "string"
