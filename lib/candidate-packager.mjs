 ...
 */
export function rejectPrivatePath(path, label) {
    const segments = resolve(path).split(sep).filter(Boolean);
    if (segments.some((segment, index) => (index > 0 && PRIVATE_SEGMENTS.has(segment)) ||
 ...
 */
export async function inspectProspectiveOutput(path, label) {
    ...
    for (const piece of resolved
        ...
        if (metadata.isSymbolicLink())
            fail(`${label} passes through ${current}, which is a symlink; point ${label} at the canonical directory path with symlinks resolved`);
        if (!metadata.isDirectory())
            fail(`${label} passes through ${current}, which is not a directory; point ${label} at a canonical directory path`);
    }
    return resolved;
}
...
async function snapshotFile(source, stage, rel, { text = true, allowExactStrings = [] } = {}) {
    ...
    if (text)
        scanText(bytes, rel, allowExactStrings);
...
export async function packageCandidate({ runId, runPath, outputPath, publicPackPath, publicPackSha256, titleOverrides = {}, captions = {}, allowExactStrings = [], }) {
    ...
    const output = resolve(outputParent, outputSegments[outputSegments.length - 1]);
    ...
    if (typeof publicPackPath !== "string" ||
    ...
    try {
        ...
        if (report.schema_version !== 1 ||
            ...
            fail("production report schema, run id, or public-pack binding does not match the exact requested run");
        ...
        let blockedRef = null;
        const stagedResultBytes = await readFile(join(stage, "explorer-result.json"), "utf8").catch(() => "null");
        const stagedResult = JSON.parse(stagedResultBytes);
        const stopUnreached = stagedResult?.stop_reason === "wall" ||
        ...
        if (stagedEvents.length === 0) {
            ...
            const exactNoTransition = report.candidate_eligible === false &&
                Array.isArray(reasons) &&
                reasons.length === 1 &&
                reasons[0] === "no transition was recorded";
            if (exactNoTransition && stopUnreached && unclaimed && validBlocked) {
                const rel = assertContained(run, resolve(run, blocked.screenshot_path), "blocked wall screenshot");
        ...
        else if (report.candidate_eligible === true &&
            Array.isArray(report.non_publishable_reasons) &&
            report.non_publishable_reasons.length === 0 &&
            stopUnreached &&
            unclaimed &&
            validBlocked &&
            stagedEvents.some((event) => event.before?.screenshot_path === blocked.screenshot_path ||
        ...
        if ((report.candidate_eligible !== true && !blockedRef) ||
        ...
        if (screenshotDirInfo) {
            if (!screenshotDirInfo.isDirectory() ||
            ...
            for (const entry of await readdir(screenshotSourceDirectory, {
                ...
                if (!screenshotRefs.has(rel))
                    fail(`unreferenced retained screenshot ${entry.name} -- no event in observations.jsonl points to it. ` +
        ...
        if (dashed) {
            for (const event of stagedTrace.events.filter((item) => item.transition_kind === "dashed")) {
                ...
                const source = boundPack.sources.find((item) => item.source_id === citation.source_id);
                if (!source ||
                    source.final_url !== (citation.source_url ?? citation.url))
                    fail(`dashed citation does not match public pack for ${event.event_id}`);
                const statement = boundPack.claims.find((claim) => claim.id === citation.claim_id);
                ...
                if (!statement ||
                    ...
                    fail(`dashed citation does not match a bounded public-pack claim for ${event.event_id}`);
            }
        }
        ...
        for (const path of [
            ...
            const info = await lstat(join(stage, path)).catch(() => null);
            if (info?.isFile())
                await chmod(join(stage, path), 0o444);
        }
