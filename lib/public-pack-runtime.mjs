...
export function validatePublicPackAuthorAllocation(authority) {
  const allocation = authority?.model_broker_approval?.public_pack_author;
  ...
  if (!exactKeys(allocation, expected) || allocation.content_classification !== "public-only" || allocation.broker !== "spike-a-openai-model-broker-v1" || allocation.provider !== "OpenAI" || allocation.api !== "Responses API" || allocation.model !== "gpt-5.6-terra" || allocation.maximum_calls !== 1 || allocation.maximum_input_tokens !== 32_000 || allocation.maximum_output_tokens !== 2_000 || allocation.worst_case_reserved_eur !== 0.104 || allocation.status !== "approved-not-called" || !exactKeys(allocation.reservation_breakdown_eur, ["input", "output"]) || allocation.reservation_breakdown_eur.input !== 0.08 || allocation.reservation_breakdown_eur.output !== 0.024) throw new Error("Frozen public-author allocation is invalid");
  if (authority.limits?.model_cost_cap_eur !== 18 || authority.limits?.combined_actual_plus_reserved_cap_eur !== 20 || allocation.worst_case_reserved_eur > authority.limits.model_cost_cap_eur || allocation.worst_case_reserved_eur > authority.limits.combined_actual_plus_reserved_cap_eur) throw new Error("Public-author allocation does not fit the frozen caps");
  return allocation;
}

export function validateOwnerPublicPackPolicies({ authority, originAllowlist, allowInsecureFixture = false }) {
  if (authority?.status !== "pass" || originAllowlist?.status !== "pass") throw new Error("Owner policy status is not pass");
  const allocation = validatePublicPackAuthorAllocation(authority);
  const fetchPolicy = validateFrozenPublicFetchPolicy(originAllowlist.public_pack_fetch_policy, { allowInsecureFixture });
  return { allocation, fetch_policy: fetchPolicy, fetch_policy_raw: originAllowlist.public_pack_fetch_policy, authority_sha256: sha256Text(JSON.stringify(authority)), origin_policy_sha256: sha256Text(JSON.stringify(originAllowlist.public_pack_fetch_policy)) };
}
...
export function validateForwardedAuthorRpc({ message, allocation, alreadyForwarded, expectedPublicInputManifestSha256 }) {
  ...
  if (message?.kind !== "rpc" || message?.broker !== "model" || message?.rpc_id !== FROZEN_AUTHOR_RPC_ID) return { ok: false, reason: "Author sandbox RPC is malformed" };
  if (message.public_input_manifest_sha256 !== expectedPublicInputManifestSha256) return { ok: false, reason: "Author sandbox RPC is not bound to the frozen public input" };
...
export function validatePublicPackAuthorCompletion({ message, expectedPublicInputManifestSha256 }) {
  if (message?.kind !== "public_pack_author_complete" || message.clean_environment !== true || message.canary_blocked !== true || message.public_input_manifest_sha256 !== expectedPublicInputManifestSha256) return { ok: false, reason: "Public-pack author completion is not bound to the frozen public input" };
  return { ok: true };
}
