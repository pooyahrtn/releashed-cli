...
export function explorerModel(modelId, env = process.env) {
    ...
    if (!apiKey)
        throw new Error("SPIKE_EXPLORER_API_KEY is required when SPIKE_EXPLORER_BASE_URL is set");
...
export const explorerMaxOutputTokens = (fallback, env = process.env) => Number(env.SPIKE_EXPLORER_MAX_TOKENS) || fallback;
