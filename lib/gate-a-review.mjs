...
function validateTrace(candidate, manifestFiles, runId) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!event.intended_action || typeof event.intended_action !== "object" || Array.isArray(event.intended_action) || !["navigate", "observe", "click", "type", "scroll"].includes(event.intended_action.method)) fail(`candidate trace action schema is invalid at ${event.event_id}`);
    ...
    if (event.transition_kind === "solid" && (!before || !after || (event.before.observation_hash === event.after.observation_hash && event.before.screenshot_sha256 === event.after.screenshot_sha256) || !["navigate", "click", "type", "scroll"].includes(method) || event.observed_outcome === "unknown-terminal" || event.evidence_provenance !== "direct-browser-observation")) fail(`solid event is invalid at ${event.event_id}`);
    ...
    if (event.transition_kind === "none" && (!before || !after || event.before.observation_hash !== event.after.observation_hash || event.before.screenshot_sha256 !== event.after.screenshot_sha256 || !["observe", "scroll", "click", "type"].includes(method) || event.evidence_provenance !== "direct-browser-observation")) fail(`no-transition event is invalid at ${event.event_id}`);
