...
import { findCaptures, REQUEST_FRAMING_WORDS } from "./capture-memory.mjs";
...
const contentTerms = (goal) => [...new Set(normal(goal).split(" "))].filter((term) => term.length > 1 && !REQUEST_FRAMING_WORDS.has(term));
...
function nameScreen(candidate) {
    if (candidate.claimed_screen?.title || candidate.claimed_screen?.url)
        return {
            ...
            captured_at: candidate.goal_screenshots[0]?.captured_at ?? candidate.captured_at,
 ...
 */
export function rankAlternatives(candidates, { goal, excludeRunId = null, limit = DEFAULT_ALTERNATIVE_LIMIT, }) {
    ...
    scored.sort((a, b) => b.matched - a.matched ||
        ...
        String(b.candidate.captured_at ?? "").localeCompare(String(a.candidate.captured_at ?? "")));
 ...
 */
export async function alternativesFromStore({ mapsRoot, goal, origin = null, excludeRunId = null, limit = DEFAULT_ALTERNATIVE_LIMIT, ...options }) {
    const empty = {
        goal,
        origin,
        considered: 0,
    ...
    try {
        ...
        return {
            goal,
            origin: found.origin,
            considered: found.candidates.filter((candidate) => candidate.run_id !== excludeRunId).length,
...
const dateOnly = (value) => typeof value === "string" && value.length >= 10
    ? value.slice(0, 10)
    : "date unknown";
/**
...
export function formatAlternatives(result) {
    ...
    if (result.lookup_error) {
        lines.push(`I could not search what has already been captured: ${result.lookup_error}. ` +
            `Run "releashed memory <url> --goal "${result.goal}"" to search the store yourself.`);
        return lines;
    }
    if (result.alternatives.length === 0) {
        lines.push("Nothing already in the store matched those words. That is a statement about this search, " +
            "not about the product: a screen can exist under a name these words do not touch.");
        return lines;
    }
    const count = result.alternatives.length;
    lines.push(`${count} ${count === 1 ? "screen" : "screens"} already in the store may be what you meant ` +
        `— candidates to inspect, not proof:`);
    for (const item of result.alternatives) {
        lines.push(`  • ${item.title ?? "(untitled screen)"} — ${item.url ?? "(url not recorded)"} — captured ${dateOnly(item.captured_at)}`);
        lines.push(`      run ${item.run_id}${item.screenshot_path ? ` — ${item.screenshot_path}` : ""}`);
    }
    lines.push("These matched the goals those earlier runs were given, not this one. Open the images before " +
