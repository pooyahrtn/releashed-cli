...
import { isNodeOnScreen } from "./ax-visibility.ts";
import type { CdpSend } from "./ax-visibility.ts";
import { formatAxLine } from "./ax-line-format.ts";
...
type Evaluate = {
  ...
  <A, T>(pageFunction: (arg: A) => T | Promise<T>, arg: A): Promise<T>;
};
...
type Measure = <T>(name: string, work: () => Promise<T>) => Promise<T>;

const defaultMeasure: Measure = (_name, work) => work();
...
export async function accessibilitySummary(cdp: CdpClient): Promise<{
  visible_state_summary: string;
  refs: AxRef[];
}> {
  ...
  const refs: AxRef[] = [];
  const candidates: Array<AxRef & { backendDOMNodeId: unknown }> = [];
  ...
  const send: CdpSend = (method, params) => cdp.send(method, params);
  const onScreenFlags = await Promise.all(
    candidates.map((candidate) =>
      isNodeOnScreen(send, candidate.backendDOMNodeId),
    ),
  );
...
): Promise<{ evidence: ScreenEvidence; refs: AxRef[]; png: Buffer }> {
  const url = page.url();
...
export function buildTransitionEvent({
  ...
  method: string;
  identified: { ref?: string | null; tier: string };
  mutated: unknown;
  ...
  return {
    ...
    observed_outcome: changed
      ? ({ click: "clicked", type: "typed", scroll: "scrolled", wait: "waited" } as Record<string, string>)[method]
      : "no-visible-effect",
...
export async function focusedTarget(
  page: ExplorerPage,
  refs: AxRef[],
): Promise<{ tier: string; ref: string | null }> {
  const focused = await page
    .evaluate(() => {
      ...
      const labelText = (
        label: { textContent?: string | null } | null | undefined,
      ): string => String(label?.textContent ?? "").replace(/\s+/g, " ").trim();
    ...
    .catch(() => null);
  if (!focused) return { tier: "positional", ref: null };
  ...
  if (matches.length === 1) return { tier: NAMED_ROLES.has(matches[0].role) ? "named" : "described", ref: matches[0].ref };
  if (names.size > 0) return { tier: "positional", ref: null };
...
export async function identifyTarget(
  ...
  for (const candidate of candidates) {
    ...
    const match =
      refs.find((ref) => ref.name === candidate.name && ref.role === candidate.role) ??
      refs.find((ref) => ref.name === candidate.name);
    if (match) return { tier: NAMED_ROLES.has(match.role) ? "named" : "described", ref: match.ref };
  }
