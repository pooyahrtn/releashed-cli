...
import { isAuthFlowUrl } from "./scaffold.mjs";
...
const runShell = async (command) => {
    try {
        ...
        throw new Error(`--auth-cmd ran in ${process.cwd()}: ${error instanceof Error ? error.message : String(error)}`);
    }
};
// What the command printed, and nothing else: chatter belongs on stderr.
export function classifyAuthOutput(stdout) {
    ...
    if (text.startsWith("{")) {
        ...
        if (!parsed ||
            ...
            throw new Error(`--auth-cmd printed JSON that is not a schema_version-1 session file: "${text.slice(0, 80)}"`);
...
const badOutput = (text) => new Error(`--auth-cmd must print a sign-in URL or a schema_version-1 session file, not: "${text.slice(0, 80)}"`);
...
async function redeemSignInUrl(ticketUrl, { name, outputDir, timeoutMs = 60_000 }) {
    const { launchVisibleBrowser, saveSession } = await import("../scripts/capture-session.mjs");
    const browser = await launchVisibleBrowser(ticketUrl, { headless: true });
    try {
        const deadline = Date.now() + timeoutMs;
        let href = ticketUrl;
        for (;;) {
            const evaluated = await browser.cdp.send("Runtime.evaluate", { expression: "location.href", returnByValue: true }, browser.sessionId);
            ...
            if (typeof evaluatedValue !== "string")
                throw new Error(`--auth-cmd's sign-in link never resolved to a location (still at ${ticketUrl}): a refusal is a finding, not something to work around`);
            href = evaluatedValue;
            if (href.startsWith("http") && !isAuthFlowUrl(href))
                break;
            if (Date.now() > deadline)
                throw new Error(`--auth-cmd's sign-in link never left the sign-in flow (still at ${new URL(href).pathname} after ${Math.round(timeoutMs / 1000)}s): a refusal is a finding, not something to work around`);
...
export async function sessionFromAuthCmd({ command, sessionDir, name }, { shell = runShell, redeem = redeemSignInUrl } = {}) {
    const result = classifyAuthOutput(await shell(command));
    await mkdir(sessionDir, { recursive: true, mode: 0o700 });
    if (result.kind === "url")
        return redeem(result.url, { name, outputDir: sessionDir });
