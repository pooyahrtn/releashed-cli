...
function requireCondition(condition, message) {
    if (!condition)
        throw new Error(`Gate A evaluation refused: ${message}`);
}
function nonemptyStrings(value) {
    return (Array.isArray(value) &&
        value.length > 0 &&
        value.every((item) => typeof item === "string" && !!item.trim() && item.length <= 2_000));
}
...
export function validateGateAEvaluation(evaluation) {
    requireCondition(exactKeys(evaluation, [
        "schema",
        "schemaVersion",
        "evaluationVersion",
        "sealed",
    ...
    requireCondition(evaluation.schema === "external-flow-map.hidden-artifact-evaluation" &&
        evaluation.schemaVersion === 1 &&
        evaluation.sealed === true &&
        typeof evaluation.evaluationVersion === "string" &&
        evaluation.evaluationVersion.length > 0, "schema identity or seal is invalid");
    const seal = evaluation.sealingContract;
    requireCondition(isRecord(seal) &&
        ...
        seal.mustNotBeShownTo.includes("map builder"), "sealing contract is not the precommitted Gate A boundary");
    const questions = evaluation.questions;
    requireCondition(Array.isArray(questions) && questions.length >= 3, "at least three sealed questions are required");
    const questionIds = new Set();
    for (const question of questions) {
        requireCondition(exactKeys(question, [
            "id",
            "kind",
            "distinctLearnerGoal",
            "prompt",
        ...
        requireCondition(QUESTION_ID.test(questionId) &&
            !questionIds.has(questionId) &&
            typeof question.distinctLearnerGoal === "string" &&
            question.distinctLearnerGoal.trim() &&
            typeof question.prompt === "string" &&
            question.prompt.trim(), "question schema is invalid");
        questionIds.add(questionId);
        requireCondition(Array.isArray(question.expectedAnswerElements) &&
            question.expectedAnswerElements.length > 0 &&
            question.expectedAnswerElements.every((element) => isRecord(element) &&
                typeof element.id === "string" &&
                element.required === true &&
                typeof element.answer === "string" &&
                element.answer.trim() &&
                nonemptyStrings(element.sourceRefs)), "expected answer elements are invalid");
        const evidence = question.evidenceRequirements;
        requireCondition(isRecord(evidence) &&
            typeof evidence.minimumDirectObservedTransitions === "number" &&
            Number.isSafeInteger(evidence.minimumDirectObservedTransitions) &&
            evidence.minimumDirectObservedTransitions >= 0, "question evidence requirements are invalid");
        const evidencePlan = evidence.requiredSequence ?? evidence.requiredInspection;
        requireCondition(nonemptyStrings(evidencePlan) &&
            (evidence.requiredSequence === undefined ||
                evidence.requiredInspection === undefined) &&
            nonemptyStrings(evidence.requiredArtifacts) &&
            nonemptyStrings(evidence.notAccepted), "question evidence requirements are invalid");
        requireCondition(exactKeys(question.scoring, ["0", "1", "2"]) &&
            ...
            question.scoring["2"].trim(), "question scoring is invalid");
    }
    const rubric = evaluation.globalScoringRubric;
    requireCondition(isRecord(rubric) &&
        ...
        nonemptyStrings(rubric.qualitativeReview), "global all-or-nothing scoring rubric is invalid");
    const sourceSnapshot = evaluation.sourceSnapshot;
    requireCondition(Array.isArray(sourceSnapshot) &&
        sourceSnapshot.length > 0 &&
        sourceSnapshot.every((source) => isRecord(source) &&
            SOURCE_ID.test(String(source.id ?? "")) &&
            HASH.test(String(source.sha256 ?? ""))), "source snapshot is invalid");
    return {
