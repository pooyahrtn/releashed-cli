...
export async function observeScreen(page, cdp, runDir, index, readOnly = false) {
  const url = page.url();
  const png = await page.screenshot();
  const summary = await accessibilitySummary(cdp);
  ...
  await writeFile(join(runDir, screenshotPath), png, { mode: 0o600 });
  return {
    evidence: {
      url,
...
export function buildTransitionEvent({
  ...
  // it carries no effect evidence.
  const observing = method === "scroll";
  return {
    run_id: runId,
    ...
    observed_outcome: changed
      ? { click: "clicked", type: "typed", scroll: "scrolled" }[method]
      : "no-visible-effect",
