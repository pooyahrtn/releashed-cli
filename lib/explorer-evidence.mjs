...
export async function accessibilitySummary(cdp) {
  ...
  const onScreenFlags = await Promise.all(
    candidates.map((candidate) =>
      isNodeOnScreen((method, params) => cdp.send(method, params), candidate.backendDOMNodeId),
    ),
  );
...
export async function observeScreen(page, cdp, runDir, index, readOnly = false, measure = (_name, work) => work()) {
  const url = page.url();
