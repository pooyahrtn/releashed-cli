...
export function stripHtml(screens, { heading, subheading = null } = {}) {
    const figures = screens
        .map((screen, index) => `<figure><div class="shot">${screen.dataUri ? `<img src="${screen.dataUri}" alt="">` : '<div class="missing">no screenshot retained</div>'}</div><figcaption><b>${index + 1}. ${esc(screen.title)}</b>${screen.url ? `<span>${esc(screen.url)}</span>` : ""}</figcaption></figure>`)
        .join("");
...
export async function renderStrip({ candidateDir, outputPath, heading, subheading = null, browser = null }) {
    ...
    const screens = await Promise.all(map.screens.map(async (screen) => ({
        title: screen.title,
        url: screen.url,
