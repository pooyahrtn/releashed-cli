...
export const FORBIDDEN_VERBS: string[] = ["send", "post", "pay", "delete"];
...
export const OWNER_FORBIDDEN_VERBS: string[] = ["pay", "delete"];
...
export type PathScope = { include?: string[]; exclude?: string[] };
...
export function verbIsTheAction(
  verb: string,
  { instruction = "", target = "" }: { instruction?: unknown; target?: unknown } = {},
): boolean {
...
export function forbiddenAction(
  decision: { instruction?: unknown; target?: unknown },
  verbs: string[] = FORBIDDEN_VERBS,
): string | null {
...
export function typedValueAllowed(text: unknown, seedValues: unknown[] = []): boolean {
...
export type WallFinding = {
  kind: "bot-wall" | "login-wall";
  reason: string;
};
...
export type BudgetFinding = {
  reason: "time_budget_exhausted" | "cost_budget_exhausted";
  detail: string;
};
...
export function budgetExhausted({ elapsedSeconds, minutes, costEur, maxEur }: BudgetState): BudgetFinding | null {
...
export function noEffectNote(streak: number): string | null {
...
export function waitAllowance(waitedMs: number): number {
...
export type Authorization = { authorized: boolean; reason: string | null };
...
export function checkActionAuthorized(
