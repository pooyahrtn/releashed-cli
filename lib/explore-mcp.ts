...
import { authorTargetPack } from "../scripts/author-target-pack.ts";
import { packageCandidate } from "./candidate-packager.ts";
import { ACTION_TYPES } from "./explorer-protocol.ts";
import {
...
import type { ActionDecision } from "./run-limits.ts";
import {
  buildTransitionEvent,
  identifyTarget,
  observeScreen,
} from "./explorer-evidence.ts";
import type {
  AxRef,
  ExplorerPage,
  ScreenEvidence,
...
import { digest, recordedSelection } from "./capture-selection.ts";
import { validateCaptureMetadata } from "./capture-metadata.ts";
import { createLocalTiming } from "./local-timing.ts";
import type { LocalTiming } from "./local-timing.ts";
import {
  alternativesFromStore,
  formatAlternatives,
} from "./goal-alternatives.ts";
import type { AlternativesResult } from "./goal-alternatives.ts";
import type { SelectedScreenshot } from "./capture-types.js";

// Tool-call envelope. observe declares NO output schema on purpose (see the header note), so
// the image part below is structural, never a declared schema.
...
// Every tool's arguments arrive as JSON. One shape with all-optional fields; validateAct and
// the per-tool runtime checks below stay the authority on what is actually required.
// Owner-directed capture hooks (see the createExplorer docstring): locateTarget resolves one
// noun phrase against a detached observed screenshot and returns the point in CSS pixels
// (or null); captureTransitions retains immediate and settled frames per dispatched input.
// Neither is a tool payload: the MCP schemas below never mention them.
export type LocateTargetShot = {
  png: Buffer;
  sha256: string;
  width: number;
  height: number;
...
export type LocateTargetResult = {
...
export type LocateTarget = (args: {
  page: unknown;
  target: string;
  screenshot: LocateTargetShot;
}) => Promise<LocateTargetResult> | LocateTargetResult;

export type ExplorerToolArgs = {
  url?: string;
  record_previous?: boolean;
  action?: string;
  instruction?: string;
  target?: string;
  direction?: string;
  coordinate?: { x?: unknown; y?: unknown } | null;
  text?: string;
  drop_target?: string;
  drop_coordinate?: { x?: unknown; y?: unknown } | null;
  refresh?: boolean;
  why?: string;
...
type PointInput = { x?: unknown; y?: unknown } | null | undefined;

function toPoint(value: PointInput): Point | null {
...
function requirePoint(value: Point | null | undefined, action: string): Point {
  // Unreachable when validateAct passed: tap/type/drag always carry finite points by then.
  if (!value) throw new Error(`unsupported action ${action}`);
  return value;
}

// The per-run counters and log the action boundary writes into, matching the BoundaryRecord
// shape scripts/vision-explorer-spike.ts installs the gate with.
export type SessionBoundary = {
  allowed: number;
  refused: Array<{ method: string; url: string; reason: string }>;
  events: Array<{ method: string; path: string; verdict: string }>;
  total: number;
  sameOriginMutations: number;
  origin: string;
  origins: Set<string>;
  readOnly: boolean;
  mine: boolean;
...
// Evidence sealed with its immutable acquisition stamp (snapshot() sets observed_at;
// trace-bound shots read it back in writeSidecars, where a missing stamp voids the marker).
type SealedEvidence = ScreenEvidence & { observed_at?: string };
...
type TransitionEvent = ReturnType<typeof buildTransitionEvent>;

type BlockedAction = {
  instruction: string;
  target: string | null;
  transition_kind: string;
  // checkActionAuthorized's type leaves reason nullable even when it refuses; the run
  // records whatever it said verbatim.
  reason: string | null;
};
...
type FinishedRun = {
  ...
  preparation_record?: string;
  map: string;
  strip?: string;
  goal_screenshots?: Array<SelectedScreenshot & { screenshot_path: string }>;
  transitions: number;
  doors_not_followed: number;
  already_captured?: AlternativesResult;
  already_captured_markdown?: string;
  evidence_markdown?: string;
  timing_receipt?: Awaited<ReturnType<LocalTiming["flush"]>> | null;
};
...
type ExplorerState = {
  ...
  pending: TransitionEvent | null;
  pendingTail: TransitionEvent[];
  captureFailure: string | null;
  scopeExit: string | null;
  wallObservation: SealedEvidence | null;
  events: TransitionEvent[];
  blocked: BlockedAction[];
  stateIndex: number;
  stepsUsed: number;
  inputsDispatched: number;
  noEffectStreak: number;
  waitedMs: number;
  staleArmed: boolean;
  staleRefreshSpent: boolean;
  startedAt: number;
  finished: FinishedRun | null;
  walled: string | null;
};

type ActDecision = ActionDecision & {
  target: string | null;
  text: string | null;
  coordinate: PointInput | null;
  direction: string | null;
  drop_target: string | null;
  drop_coordinate: PointInput | null;
};

export type LaunchMeasure = <T>(
  name: string,
  work: () => Promise<T>,
) => Promise<T>;
...
export type CreateExplorerOptions = {
  ...
  acquireUntil?: number | null;
  locator?: LocatorResolution | undefined;
  locateTarget?: LocateTarget | undefined;
  captureTransitions?: boolean;
  createLocatorAgent?: LocatorFactories["createAgent"] | null;
  createScreenshotItem?: LocatorFactories["createScreenshotItem"] | null;
  locatorTimeoutMs?: number;
  launch?: typeof launchBoundBrowser;
  resolveSession?: (() => Promise<string | null>) | null;
  cleanupSession?: () => Promise<void>;
  timing?: LocalTiming;
  now?: () => number;
  authorPack?: (url: string, fetchImpl?: typeof fetch) => Promise<unknown>;
  packageRun?: (args: {
    runId: string;
    runPath: string;
    outputPath: string;
...
export function validateAct(
  args: ExplorerToolArgs,
  { locatorEnabled = false }: { locatorEnabled?: boolean } = {},
): string | null {
  const method =
    typeof args?.action === "string" ? RECORDED_METHOD[args.action] : undefined;
...
function coordinateError(
  ...
  if (
    (typeof x === "number" && (x < 0 || x >= MAX_VIEWPORT_WIDTH)) ||
...
export async function launchBoundBrowser(
  entryUrl: string,
  {
    ...
    mine = false,
    measure = defaultLaunchMeasure,
  }: {
    viewport?: { width: number; height: number };
    savedSessionPath?: string | null;
    mine?: boolean;
    measure?: LaunchMeasure;
  } = {},
): Promise<ExplorerSession> {
  const { chromium } = await import("playwright");
  const { installActionBoundary } = await import(
    "../scripts/vision-explorer-spike.ts"
  );
...
export function createExplorer({
  ...
  class LocateMissError extends Error {
    kind: string;
    target: string;
    constructor(kind: string, target: string, detail: string) {
  ...
  const locatorResolution: LocatorResolution | undefined = locatorFromEnv
  ...
  if (locatorFromEnv && (process.env.RELEASHED_LOCATOR ?? "").trim()) {
    ...
    if (resolution && !resolution.enabled)
      throw new Error(
        `target locator misconfigured: ${resolution.reason}`,
      );
  }
  ...
  const locatorEnabled =
    Boolean(locateTarget) || locatorResolution?.enabled === true;
  const locatorConfig: LocatorModelConfig | null =
    locatorResolution?.enabled === true ? locatorResolution.config : null;
  ...
  let locatorFactoriesPromise: Promise<LocatorFactories> | null = null;
  async function locatorFactories(shot: ObservedShot): Promise<LocatorFactories> {
    ...
    if (locateTarget)
      return {
        createScreenshotItem: (base64: string) => ({ base64 }),
        createAgent: (_page: unknown) => ({
          aiLocate: (target: string) =>
            Promise.resolve(
              locateTarget({
                page: _page,
                target,
  ...
  function observedShot(): ObservedShot {
    ...
    const viewport =
      typeof session.page.viewportSize === "function"
  ...
  async function locateForDispatch(
    shot: ObservedShot,
    target: string,
  ): Promise<Extract<Awaited<ReturnType<typeof locateOnShot>>, { ok: true }>> {
    ...
    const outcome = await timing.span("grounding.locate", async () =>
      locateOnShot(await locatorFactories(shot), session.page, shot, target, {
        timeoutMs: budget,
      }),
    );
    if (!outcome.ok) {
  ...
  async function reverifyObservation(
    beforeUrl: string,
    beforeHash: string,
  ): Promise<void> {
    const session = state.session;
    ...
    const reverified = await timing.span("grounding.reverify", () =>
      session.page.screenshot().catch(() => null),
    );
  ...
  let authenticationFailure: unknown = null;
  let browserStartupFailure: Error | null = null;
  let callQueue: Promise<unknown> = Promise.resolve();
  let closing: Promise<void> | null = null;
  let declarationWritten = false;
  let browserClosed = false;
  let browserCloseFailure: string | null = null;
  let removeScopeListener: (() => void) | null = null;
  ...
  async function open(firstUrl: string | undefined): Promise<void> {
    ...
    try {
      await timing.span("browser.startup", async () => {
        ...
        if (captureTransitions) {
          ...
          if (typeof page.on === "function" && typeof page.off === "function" &&
            ...
            const navigation = (frame: Frame): void => {
              try {
                if (frame === page.mainFrame()) sampleCaptureScope(frame.url());
              } catch {
      ...
      if (state.session) await closeCaptureBrowser().catch(() => {});
      removeScopeListener?.();
  ...
  function stopAtScopeExit(decision: ActDecision | null): Error {
    if (!state.scopeExit) {
      state.scopeExit =
  ...
  function sampleCaptureScope(url?: string): void {
    if (!captureTransitions || !state.session || browserClosed) return;
    try {
  ...
  function requireCaptureScope(decision: ActDecision | null = null): void {
    sampleCaptureScope();
    if (captureTransitions && state.scopeExit) throw stopAtScopeExit(decision);
  }

  async function closeCaptureBrowser(): Promise<void> {
    const session = state.session;
    if (!session || browserClosed) return;
    sampleCaptureScope();
    try {
  ...
  async function snapshot(
    decision: ActDecision | null = null,
    ...
    const path = join(
      runDir,
      "screenshots",
      `state-${String(state.stateIndex).padStart(4, "0")}.png`,
    );
    let acquiredPng: Buffer | null = null;
    ...
    const sampleScope = (url: string): void => {
      try {
        if (!onBoundOrigin(url, boundary.origins)) invalidScope = true;
      } catch {
        invalidScope = true;
      }
    };
    const navigation = (frame: Frame): void => {
      try {
        if (frame === page.mainFrame()) sampleScope(frame.url());
      } catch {
        invalidScope = true;
      }
    };
    const checkScope = (): void => {
      ...
      sampleCaptureScope();
      if (state.scopeExit) invalidScope = true;
      if (invalidScope) throw stopAtScopeExit(decision);
    };
    ...
    const measure: LaunchMeasure = captureTransitions
      ? (name, work) =>
          timing.span(name, async () => {
            checkScope();
            try {
              return await work();
            } finally {
    ...
    const listen =
      ...
      typeof page.mainFrame === "function";
    try {
      if (listen) page.on("framenavigated", navigation);
      let observed: CurrentObservation;
      try {
        observed = await measure("observation", () =>
          observeScreen(
            capturePage,
            cdp,
            runDir,
        ...
        if (preserved)
          await measure("screenshot.preserve", () =>
            writeFile(path, preserved, { mode: 0o600 }),
          );
        throw error;
      }
      ...
      if (listen) page.off("framenavigated", navigation);
    }
  }

  async function observe(args: ExplorerToolArgs = {}): Promise<ToolResult> {
    if (state.finished)
    ...
    if (state.scopeExit && !state.current) return toolError(state.scopeExit);
    ...
    let recordedEvents: unknown = null;
    if (args.record_previous !== undefined && args.record_previous !== false) {
      ...
      const receipt: { recorded?: unknown; recorded_events?: unknown } =
        keptFirst && keptFirst.type === "text"
    ...
    if (!state.session) {
      ...
      const entryWall = detectWall(
        opened.evidence.url,
        opened.evidence.visible_state_summary,
        { entry: true },
      );
      if (entryWall) {
        ...
        text: JSON.stringify(
          {
            ...
            ...(imageOutput === "paths"
              ? {
                  image_path: join(
                    runDir,
                    current.evidence.screenshot_path,
                  ),
                }
              : {}),
  ...
  async function act(args: ExplorerToolArgs = {}): Promise<ToolResult> {
    if (state.finished)
      return toolError("this run is finished; its map is already packaged");
    sampleCaptureScope();
    if (state.scopeExit) return toolError(state.scopeExit);
    if (browserClosed || browserCloseFailure)
      return toolError(browserCloseFailure ?? "the browser is closed; finish packaging or close this capture");
    if (!state.session || !state.current)
      return toolError("call observe first: nothing has been looked at yet");
    if (state.captureFailure) return toolError(state.captureFailure);
    ...
    let identified: { tier: string; ref: string | null } = {
      tier: "positional",
      ref: null,
    };
    ...
    if (waiting) {
      ...
      try {
        await timing.span("action.dispatch", async () => {
          ...
          let point: Point | undefined = toPoint(decision.coordinate) ?? undefined;
          let dropPoint: Point | undefined =
            toPoint(decision.drop_coordinate) ?? undefined;
          if (
            locatorEnabled &&
          ...
          if (args.action === "tap") {
            ...
            await ourDragMotion(
              page,
              { x, y },
              requirePoint(dropPoint, args.action),
            );
          } else {
    ...
    if (!onBoundOrigin(page.url(), boundary.origins)) {
      if (captureTransitions) return toolError(stopAtScopeExit(decision).message);
  ...
  function completedAct(
    before: SealedEvidence,
    after: SealedEvidence,
    identified: { tier: string; ref: string | null },
    waiting: boolean,
    wall: { reason: string } | null,
  ): ToolResult {
  ...
  async function captureInputFrames(
    before: SealedEvidence,
    ...
    const checkFrame = (frame: SealedEvidence): void => {
  ...
  async function record(): Promise<ToolResult> {
    if (state.finished)
      return toolError("this run is finished; its map is already packaged");
    sampleCaptureScope();
  ...
  function selectNonzeroBlockedShot({
    ...
    why: unknown;
    unreached: boolean;
    selected: SelectedScreenshot[] | null;
  }): { shot: SealedEvidence } | { error: string } | null {
    ...
    if (state.scopeExit && !state.wallObservation) return null;
  ...
  async function writeSidecars(
    ...
    selected: SelectedScreenshot[] | null = null,
    unreached = false,
  ): Promise<void> {
    ...
    const nonzeroBlockedShot =
      blockedSelection && "shot" in blockedSelection
    ...
    const blockedShot: SealedEvidence | null | undefined =
  ...
  async function finish(args: ExplorerToolArgs = {}): Promise<ToolResult> {
    if (state.finished) return text(state.finished);
    if (browserStartupFailure) throw browserStartupFailure;
    sampleCaptureScope();
    if (browserCloseFailure) return toolError(browserCloseFailure);
    if (captureTransitions && state.pending)
    ...
    if (args.goal_screenshots !== undefined) {
      ...
      try {
        ...
        return toolError(
          `${thrownMessage(error)} The run is retained with its recorded trace intact: ` +
    ...
    if (!captureTransitions) await timing.span("evidence.sweep", () =>
    ...
    try {
      ...
      // five retries.
      throw new Error(
        `packaging failed and was not retried: ${thrownMessage(error)}. ` +
    ...
    if (goal) {
      ...
      const { renderStrip } = await import("./flow-strip.ts");
      await timing
        .span("render.strip", () =>
          renderStrip({
    ...
    let alternatives: AlternativesResult | null = null;
    if (
      goal &&
    ...
    const markdownLink = (label: string, path: string): string =>
    ...
    if (selected) {
      ...
      const shot =
        blockedShotSelection && "shot" in blockedShotSelection
  ...
  async function cleanup(): Promise<void> {
    try {
      if (captureTransitions) await closeCaptureBrowser();
      else await timing.span("browser.cleanup", async () => {
        await state.session?.close();
      });
    } finally {
      removeScopeListener?.();
  ...
  function handleCall(
    ...
    const result: Promise<ToolResult> = callQueue.then(async () => {
      let response: ToolResult;
      let timingReceipt: Awaited<ReturnType<LocalTiming["flush"]>>;
      try {
...
): Promise<ReturnType<typeof createExplorer>> {
