...
import { CLERK_PAGE_AUTH_FAILURE_CODES, CLERK_TICKET_TTL_SECONDS, isSafeClerkAuthFailureCode, safeClerkBoundedRouteFailure } from "../lib/clerk-auth.mjs";
...
function isDeferredCleanup(value) {
    return typeof value === "function";
}
function isSettlingCleanup(value) {
    return (typeof value === "function" &&
        "settlesByIdentityDeletion" in value &&
        value.settlesByIdentityDeletion === true);
}
function isAmbiguityObservedAt(value) {
...
function isMintOutcomeUnknown(error) {
...
async function verifyClerkFrontendOrigin({ secretKey, expectedFrontendOrigin, fetchImpl, deadlineAt }) {
    ...
    const totalCount = domains.total_count;
...
export async function provisionDisposableClerkIdentity({ environment = process.env, fetchImpl = fetch, expectedFrontendOrigin, workingDayDeadline = null, onRecoveryMaterial = async () => { }, markerFactory = () => `flowmap_cal_${randomUUID().replaceAll("-", "")}` }) {
    let secretKey = environment.CLERK_SECRET_KEY ?? null;
    ...
    return {
        ...
        cleanup({ browserStopped = false } = {}) {
            cleanupPromise ??= (async () => {
                ...
                if (attachedFailureCode === CLERK_MINT_OUTCOME_UNKNOWN_CODE &&
                    ...
                    try {
                        if (auth?.cleanup)
                            authCleanup = await auth.cleanup();
                        else if (isDeferredCleanup(attachedCleanup))
                            authCleanup = await attachedCleanup({ browserStopped: true });
                    }
                    catch { }
                    try {
                        await deleteDisposableUser({ secretKey, userId, fetchImpl });
...
export async function reconcileDisposableClerkRecovery({ recovery, expectedFrontendOrigin, workingDayDeadline = null, environment = process.env, fetchImpl = fetch }) {
    let secretKey = environment.CLERK_SECRET_KEY ?? null;
    delete environment.CLERK_SECRET_KEY;
    try {
        if (typeof secretKey !== "string" || secretKey.length < 8 || secretKey.length > 4_096 || /[\r\n]/.test(secretKey)) {
        ...
        if (userId !== null) {
            ...
            if (exact !== null) {
                if (exact.id !== userId || exact.external_id !== externalId)
                    throw new Error("mismatched recovery");
                await deleteDisposableUser({ secretKey, userId, fetchImpl });
                ...
                if (remaining !== null)
                    throw new Error("unconfirmed recovery");
            }
        }
        else {
            ...
            if (users.length > 1)
                throw new Error("ambiguous recovery");
            if (users.length === 1) {
...
export async function verifyDisposableClerkIdentity({ identity, expectedFrontendOrigin, workingDayDeadline = null, environment = process.env, fetchImpl = fetch }) {
    let secretKey = environment.CLERK_SECRET_KEY ?? null;
    delete environment.CLERK_SECRET_KEY;
    try {
        if (typeof secretKey !== "string" || secretKey.length < 8 || secretKey.length > 4_096 || /[\r\n]/.test(secretKey)) {
        ...
        if (markerMatches.length !== 1 || markerMatches[0] !== userId)
            throw new Error("identity mismatch");
 ...
 */
export async function retireDisposableClerkIdentity({ identity, expectedFrontendOrigin, workingDayDeadline = null, environment = process.env, fetchImpl = fetch, beforeDelete = async () => { }, beforeConfirmation = async () => { } }) {
    let secretKey = environment.CLERK_SECRET_KEY ?? null;
    delete environment.CLERK_SECRET_KEY;
    try {
        if (typeof secretKey !== "string" || secretKey.length < 8 || secretKey.length > 4_096 || /[\r\n]/.test(secretKey)) {
...
async function listDisposableUsersByMarker({ secretKey, marker, fetchImpl, deadlineAt }) {
    ...
    const rawRows = Array.isArray(body) ? body : isRecord(body) ? body.data : undefined;
    if (!Array.isArray(rawRows) || rawRows.length > 2)
        throw new Error("Clerk disposable identity lookup was invalid");
    const rows = rawRows;
    const totalCount = isRecord(body) && !Array.isArray(body) ? body.total_count : undefined;
    ...
    return rows.map((row) => {
        ...
        if (fields?.external_id !== marker)
            throw new Error("Clerk returned a mismatched disposable identity marker");
...
async function verifyExactDisposableIdentity({ secretKey, expectedUserId, expectedUsername, fetchImpl, deadlineAt }) {
    const body = await boundedFetchJson({
        fetchImpl,
        secretKey,
    ...
    const ownerDeadline = workingDayDeadline === null
        ...
        : typeof workingDayDeadline === "number"
            ...
            : typeof workingDayDeadline === "string"
                ...
                : NaN;
    if (!Number.isFinite(ownerDeadline) || ownerDeadline <= now)
        throw new Error("The original working-day deadline has expired");
...
async function boundedFetchJsonResult({ fetchImpl, secretKey, url, init, deadlineAt, failure, onDispatch = null, acceptedStatuses = [] }) {
    ...
    try {
        ...
        if (error instanceof Error && error.message.startsWith(failure))
            throw error;
        throw new Error(failure);
    }
    finally {
...
function sessionRows(body) {
    if (Array.isArray(body))
        return { rows: body, totalCount: null };
...
async function listSessionIds({ secretKey, expectedUserId, fetchImpl, deadlineAt }) {
    ...
    for (let page = 0; page < MAX_SESSION_PAGES; page += 1) {
        ...
        for (const row of rows) {
            ...
            if (found.has(id))
                throw new Error("Clerk session inventory was ambiguous");
            found.add(id);
        }
        if (totalCount !== null) {
 ...
 */
export async function preflightFreshClerkAuthentication({ expectedUserId: preflightUserId, expectedFrontendOrigin, workingDayDeadline, environment = process.env, fetchImpl = fetch, approvedDisposableIdentity }) {
    let secretKey = environment.CLERK_SECRET_KEY ?? null;
    let sessionIds = null;
    try {
        if (typeof secretKey !== "string" || secretKey.length < 8 || secretKey.length > 4_096 || /[\r\n]/.test(secretKey)) {
...
async function stopBrowserBeforeReconciliation(browser) {
    ...
    if (typeof stopBrowser !== "function")
        return false;
    let timer = undefined;
    try {
        return await Promise.race([
...
function bootstrapFailure(message, cleanup, diagnosticCode = "clerk_auth_failed", failureCode = null) {
    ...
    if (failureCode !== null)
        Object.defineProperty(error, "code", { value: failureCode });
    return error;
}
function boundedRouteBootstrapFailure(cleanup, route) {
    const error = bootstrapFailure("The browser could not confirm Clerk authentication", cleanup, "clerk_auth_landing_unconfirmed");
...
function deferUnknownMintCleanup({ ambiguityObservedAt, timing, secretKey: initialSecretKey, expectedUserId: initialUserId, beforeSessionIds: initialSessionIds, fetchImpl }) {
...
function deferFailedAuthenticationCleanup({ secretKey, expectedUserId, beforeSessionIds, signInTokenId, fetchImpl, outcomeUnknown }) {
    ...
    return function cleanup({ browserStopped = false, cleanupDeadlineAt = null } = {}) {
        ...
        if (activeSecretKey === null ||
            ...
            activeTokenId === null) {
            return Promise.resolve(safeCleanupResult({
                complete: false,
...
async function cleanupKnownUnusedToken({ secretKey, fetchImpl, signInTokenId, cleanupDeadlineAt = null }) {
    let unusedTokenRevoked = false;
    try {
        if (secretKey === null || signInTokenId === null)
            throw new Error("Clerk unused sign-in token cleanup did not complete");
        await revokeExact({
            secretKey,
            fetchImpl,
            path: `/v1/sign_in_tokens/${encodeURIComponent(signInTokenId)}/revoke`,
 ...
 */
export async function bootstrapClerkAuthentication({ browser, expectedUserId: preflightUserId, expectedFrontendOrigin, workingDayDeadline, environment = process.env, fetchImpl = fetch, deferFailureCleanup = false, requireFreshAuth = false, approvedDisposableIdentity = null, timing: timingDependencies = {} }) {
    let secretKey = environment.CLERK_SECRET_KEY ?? null;
    delete environment.CLERK_SECRET_KEY;
    if (typeof secretKey !== "string" || secretKey.length < 8 || secretKey.length > 4_096 || /[\r\n]/.test(secretKey)) {
        secretKey = null;
    ...
    if (requireFreshAuth) {
        const cleanBrowserPreAuth = (preAuth) => isRecord(preAuth) &&
            preAuth.ok === true &&
    ...
    try {
        ...
        secretKey = null;
        beforeSessionIds.clear();
        throw bootstrapFailure("Clerk authentication preparation failed", cleanup, CLERK_MINT_OUTCOME_UNKNOWN_CODE, CLERK_MINT_OUTCOME_UNKNOWN_CODE);
    }
    let response;
    try {
        ...
        try {
            response = await Promise.race([
                browser.call({ method: "authenticate_clerk_ticket", ticket: signInToken.ticket, expected_user_id: expectedUserId }, "supervisor"),
                new Promise((_, rejectTimeout) => {
