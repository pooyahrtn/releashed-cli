...
} from "../lib/browser-runtime-limits.mjs";
...
function privateIdentityExposureError() {
  const error = new Error("Private identity exposure prevented evidence retention");
  error.privateIdentityExposure = true;
  return error;
}

function publicCapState(state) {
  if (!state?.abort?.operator_diagnostic) return state;
  const clone = structuredClone(state);
  delete clone.abort.operator_diagnostic;
  return clone;
}
...
export function evaluateCalibrationRestoration({ baseline, open, final, guard = {} }) {
  ...
  proof.safe = Object.entries(proof).every(([key, value]) => {
...
export async function waitForDevToolsPort(devtoolsFile, {
...
export class BrowserBroker {
  constructor(config, {
    ...
    this.absoluteDeadlineMs = Number.isFinite(config.operation_deadline_ms) ? config.operation_deadline_ms : Date.now() + 60_000;
  ...
  async start() {
    ...
    const socket = new this.WebSocket(version.webSocketDebuggerUrl);
    ...
    const attached = await this.cdp.send("Target.attachToTarget", { targetId: page.targetId, flatten: true });
    this.pageTargetId = page.targetId;
    this.pageSessionId = attached.sessionId;
    this.sessionTypes.set(attached.sessionId, { targetId: page.targetId, type: "page" });
    await this.configureSession(attached.sessionId, "page", false);
  ...
  async loadSavedSession() {
    ...
    if (!session || typeof session !== "object" || !Array.isArray(session.cookies) || (session.origin_storage !== undefined && !Array.isArray(session.origin_storage))) {
    ...
    const originStorage = session.origin_storage ?? [];
    ...
    const hasLiveCookie = cookies.some((cookie) => !Number.isFinite(cookie.expires) || cookie.expires < 0 || cookie.expires > nowSeconds);
    ...
    for (const entry of originStorage) {
      if (typeof entry?.origin !== "string") throw new Error("saved_session_invalid: an origin-storage entry is missing its origin");
      let origin;
      try {
  ...
  async callPageFunction(functionDeclaration, values = [], effectful = false) {
    ...
    if (!root.result?.objectId) throw new Error("The page JavaScript context is unavailable");
    if (effectful) this.assertEffectfulAdmissionOpen();
  ...
  async startClerkAuthentication(ticket, expectedUserId, expectedCurrentHref) {
    ...
    if (Object.hasOwn(exchange ?? {}, "failure_stage")) return exchange;
    const createdSessionId = exchange?.created_session_id;
    if (!/^[A-Za-z0-9_-]{4,256}$/.test(createdSessionId ?? "")) return { failure_stage: "session_missing" };
  ...
  inspectClerkAuthentication() {
    return this.callPageFunction(CLERK_PAGE_FUNCTIONS.inspect);
  }

  assertEffectfulAdmissionOpen() {
    if (!this.backgroundAbortLatched) return;
    const error = new Error("The browser run is not live");
    error.backgroundAbortLatched = true;
    throw error;
  }

  async lockClerkAuthenticationMethods() {
    const result = await this.callPageFunction(CLERK_PAGE_FUNCTIONS.lock);
    if (result?.locked !== true) throw new Error("The page Clerk methods could not be locked");
  }
  ...
  async privateNavigationSnapshot() {
    ...
    const current = Array.isArray(entries) && Number.isSafeInteger(index) ? entries[index] : null;
    if (
      typeof current?.url !== "string" ||
      !current.url ||
      !Number.isSafeInteger(current.id) ||
      typeof this.topFrameId !== "string" ||
      !this.topFrameId ||
      typeof this.topFrameLoaderId !== "string" ||
    ...
    const predecessor = index > 0 ? entries[index - 1] : null;
    const twoBack = index > 1 ? entries[index - 2] : null;
    return {
      exactUrl: current.url,
      entryId: current.id,
      entryIndex: index,
      entryCount: entries.length,
      predecessorId: Number.isSafeInteger(predecessor?.id) ? predecessor.id : null,
      predecessorUrl: typeof predecessor?.url === "string" ? predecessor.url : null,
      twoBackId: Number.isSafeInteger(twoBack?.id) ? twoBack.id : null,
      twoBackUrl: typeof twoBack?.url === "string" ? twoBack.url : null,
  ...
  noteBoundedRouteFailure(point) {
    if (Object.hasOwn(CLERK_BOUNDED_ROUTE_FAILURE_POINTS, point)) {
      this.boundedRouteFailurePoint = CLERK_BOUNDED_ROUTE_FAILURE_POINTS[point];
    }
    return null;
  }
  ...
  privateInitialAuthHandoff(snapshot) {
    try {
      if (!isAuthFlowUrl(snapshot?.exactUrl)) return false;
  ...
  routeBindingRequestRelevant(rawUrl) {
    try {
      return !this.suppressedOrigins.has(normalizeOrigin(rawUrl));
    } catch {
      return true;
    }
  }
  ...
  async admitPrivateRouteState(deadline, { sealNetwork = false } = {}) {
    ...
    if (state?.abort) return refusal("run_aborted", "The browser run is not live");
    const workingDayDeadline = Date.parse(state?.working_day_deadline);
    if (
  ...
  async confirmBoundedScopeExit() {
    ...
    if (!this.boundedOnboarding || first.exactUrl === this.boundedScopeHref || isAuthFlowUrl(first.exactUrl) || normalizeOrigin(first.exactUrl) !== normalizeOrigin(this.boundedScopeHref)) return null;
    await new Promise((resolvePromise) => setTimeout(resolvePromise, AUTH_LANDING_SETTLE_MS));
    try {
  ...
  async inspectClerkPreAuth() {
    ...
    return {
      ok: finallyAdmitted,
      clerk_ready: state?.clerk_ready === true,
      signed_out: state?.signed_out === true,
      session_absent: state?.session_absent === true,
      sign_in_clean: state?.sign_in_clean === true,
  ...
  async authenticateClerkTicket(request) {
    ...
    const exactSessionBinding = Boolean(
      result?.ok &&
      /^[A-Za-z0-9_-]{4,256}$/.test(result.active_session_id ?? "") &&
      result.active_session_id === this.activeClerkSessionId
    );
    ...
    if (this.boundedOnboarding) {
      ...
      this.boundedExpectedClerkUserId = request.expected_user_id;
  ...
  async bindBoundedPostAuthRoute() {
    ...
    try {
      directHandoff = this.boundedPostAuthHandoff.kind === "direct" &&
        !isAuthFlowUrl(this.boundedPostAuthHandoff.navigation.exactUrl);
    } catch {}
    const routeChain = {
      hops: directHandoff ? 1 : 0,
      last: this.boundedPostAuthHandoff.navigation
    };
    ...
    if (initialIsAuth) {
      ...
      const exactPriorTransit = Boolean(
        ...
        this.sameAdmittedAuthTransit(this.boundedPostAuthHandoff.navigation, initialNavigation) &&
      ...
      if (!this.sameAdmittedAuthTransit(initialNavigation, stillCurrent.value)) {
        ...
        if (
          ...
          !this.sameAdmittedAuthTransit(initialNavigation, sealedTransit.value) ||
          !this.sameCurrentPrivateNavigation(sealedTransit.value)
    ...
    let clerkLockedLoaderId = this.boundedPostAuthHandoff.navigation.loaderId;
    while (Date.now() < deadline) {
  ...
  async privateIdentityMaterialAbsent(tree, rawMaterial = []) {
    ...
    if (
      !this.privateIdentityNeedles ||
      containsExactPrivateText(tree, this.privateIdentityNeedles) ||
      containsExactPrivateText(rawMaterial, this.privateIdentityNeedles)
    ) return false;
    let inspected;
    try {
      inspected = await this.callPageFunction(CLERK_PAGE_FUNCTIONS.identityAbsent, [this.privateIdentityNeedles]);
    } catch {
      return false;
    }
    return inspected?.inspection_complete === true && inspected.identity_absent === true;
  }
  ...
  async privatePostAuthReadinessSample() {
    ...
    const actionable = (tree.nodes ?? []).some((node) =>
      !node.ignored && ["button", "link", "textbox", "searchbox", "checkbox", "radio", "tab", "menuitem", "option"].includes(node.role?.value)
    );
    const accessibilityBusy = (tree.nodes ?? []).some((node) =>
      !node.ignored && (
        node.role?.value === "progressbar" ||
        (node.properties ?? []).some((property) => property.name === "busy" && property.value?.value === true)
      )
    );
    if (
      pageReady !== true ||
    ...
    const axSha256 = hashAccessibilityTree(tree.nodes ?? []);
    const screenshotSha256 = sha256Text(screenshot.data);
    return {
  ...
  async confirmPrivatePostAuthReadiness() {
    ...
    while (Date.now() < deadline) {
      ...
      if (typeof sample?.fingerprint === "string") {
        if (sample.fingerprint === previous) {
          stableSamples += 1;
        } else {
          stableSamples = 1;
          stableSince = Date.now();
        ...
        if (
          ...
          Date.now() - stableSince >= POST_AUTH_STABLE_WINDOW_MS
        ) {
  ...
  async privateClerkBootstrapNavigate(request) {
    ...
    try {
      ...
      this.activeNavigation = { allowed_urls: new Set([new URL(request.url).href]), last_url: null };
      this.assertEffectfulAdmissionOpen();
      failurePoint = INITIAL_NAVIGATION_FAILURE_POINTS.navigate_dispatch;
      const dispatched = await this.cdp.send("Page.navigate", { url: request.url }, this.pageSessionId);
      ...
      if (error?.backgroundAbortLatched || this.backgroundAbortLatched) {
  ...
  onCdpEvent(message) {
    ...
    if (message.method === "Network.requestWillBeSent") {
      ...
      if (this.routeBindingRequestRelevant(message.params.request?.url)) {
    ...
    if (message.method === "Target.targetCreated") {
      ...
      if (!["page", "service_worker"].includes(info.type)) {
    ...
    if (message.method === "Target.attachedToTarget") {
      ...
      if (info.type === "service_worker") {
        ...
        try {
          if (!this.allowedOrigins.has(normalizeOrigin(info.url))) this.trackBackgroundAbort("service_worker_origin_allowlist_breach", "Service worker origin was not allowed");
        } catch {
    ...
    if (message.method === "Network.webSocketCreated" || message.method === "Network.eventSourceMessageReceived") {
    ...
    if (message.method === "Network.requestWillBeSent" && message.params.redirectResponse) {
      if (this.activeNavigation && message.params.type === "Document") {
        try {
          if (new URL(message.params.redirectResponse.url).href !== this.activeNavigation.last_url) throw new Error("redirect chain mismatch");
          const redirected = new URL(message.params.request.url).href;
          if (this.activeNavigation.reject_redirects === true) throw new Error("redirect forbidden");
          if (this.allowedNavigationOrigins.has(normalizeOrigin(redirected))) this.activeNavigation.allowed_urls.add(redirected);
        } catch {
          this.trackBackgroundAbort("unbound_document_redirect", "A document redirect did not match the active observed navigation");
        }
      }
      const redirectKey = `${message.sessionId ?? ""}:${message.params.requestId}:${message.params.redirectResponse.url}`;
    ...
    if (message.method === "Page.frameNavigated" && message.params.frame?.parentId === undefined) {
      ...
      this.topFrameLoaderId = frame.loaderId;
    ...
    if (message.method === "Page.navigatedWithinDocument" && message.params.frameId === this.topFrameId) {
      this.navigationSequence += 1;
      const urlSha256 = sha256Text(String(message.params.url ?? ""));
      const duplicateLogicalLocation =
        message.params.navigationType === "historyApi" &&
        this.lastTopLogicalLocation?.urlSha256 === urlSha256 &&
        this.lastTopLogicalLocation.frameId === this.topFrameId &&
        this.lastTopLogicalLocation.loaderId === this.topFrameLoaderId;
      this.lastTopNavigation = {
        kind: "same-document",
        navigationType: message.params.navigationType,
        sequence: this.navigationSequence
      };
      if (!duplicateLogicalLocation) {
        ...
        this.initialLandingEventCandidate?.sequence === this.logicalNavigationSequence &&
        sha256Text(this.initialLandingEventCandidate.exactUrl) === urlSha256
      ) {
        this.initialLandingEventCandidate.sameUrlHistoryApiNotifications += 1;
      }
      this.lastTopLogicalLocation = {
        urlSha256,
        frameId: this.topFrameId,
      ...
      try {
        if (!isAuthFlowUrl(message.params.url)) {
          this.currentUrl = message.params.url;
        }
      } catch {}
    }
  }
  ...
  async configureSession(sessionId, type, resumeWaitingTarget) {
    try {
      ...
      if (type === "page") {
        ...
        if (typeof topFrame?.id === "string" && typeof topFrame?.loaderId === "string" && this.topFrameId === null) {
          this.topFrameId = topFrame.id;
          this.topFrameLoaderId = topFrame.loaderId;
        }
      ...
      try {
        await this.abort("cdp_session_configuration_failed", error.message);
      } catch {}
      throw error;
    }
  }
  ...
  boundedCapsValid(state) {
    ...
    const activeSeconds = state?.browser?.started_at == null
      ? 0
      : monotonicSeconds() - state.browser.started_at;
    const combined = Number(state?.app?.actual_eur) + Number(state?.app?.outstanding_reservations_eur) +
      Number(state?.model?.actual_eur) + Number(state?.model?.outstanding_reservations_eur);
    return Boolean(
      caps &&
      Number.isFinite(activeSeconds) && activeSeconds >= 0 && activeSeconds <= caps.browser_active_seconds &&
      Number.isSafeInteger(state?.browser?.operations) && state.browser.operations >= 0 && state.browser.operations <= caps.browser_operations_total &&
      Number.isSafeInteger(state?.browser?.requests) && state.browser.requests >= 0 && state.browser.requests <= caps.browser_requests_total &&
      rollingCount(this.operationTimes, monotonicSeconds()) <= caps.browser_operations_per_rolling_minute &&
      rollingCount(this.requestTimes, monotonicSeconds()) <= caps.browser_requests_per_rolling_minute &&
      Number.isSafeInteger(state?.app?.one_way_actions) && state.app.one_way_actions >= 0 && state.app.one_way_actions <= caps.listed_one_way_actions_total &&
      Number.isFinite(Number(state?.app?.actual_eur)) && Number(state.app.actual_eur) >= 0 &&
      Number.isFinite(Number(state?.app?.outstanding_reservations_eur)) && Number(state.app.outstanding_reservations_eur) >= 0 &&
      Number(state.app.actual_eur) + Number(state.app.outstanding_reservations_eur) <= caps.app_side_cost_cap_eur &&
      Number.isFinite(Number(state?.model?.actual_eur)) && Number(state.model.actual_eur) >= 0 &&
      Number.isFinite(Number(state?.model?.outstanding_reservations_eur)) && Number(state.model.outstanding_reservations_eur) >= 0 &&
      Number(state.model.actual_eur) + Number(state.model.outstanding_reservations_eur) <= caps.model_cost_cap_eur &&
      Number.isFinite(combined) && combined <= caps.combined_actual_plus_reserved_cap_eur
    );
  }
  ...
  privateAuthUnknownMutationDiagnostic({ rawUrl, origin, method, receipt }) {
    ...
    const diagnostic = {
      ...
      captured_phase: SAFE_DIAGNOSTIC_CLERK_PHASES.has(receipt.clerk_transport_phase)
  ...
  async drainBoundedRequestQueue() {
    for (;;) {
      ...
      if (state.abort) throw new Error("A queued browser request was blocked");
      return;
    }
  }
  ...
  async armBoundedMutationWindow() {
    ...
    this.assertEffectfulAdmissionOpen();
    if (this.activeAction?.action_class !== BOUNDED_ONBOARDING_CLASS) throw new Error("Bounded action disappeared before dispatch");
    this.assertEffectfulAdmissionOpen();
    this.activeAction.bounded_click_dispatched = true;
    this.activeAction.bounded_click_dispatch_generation = ++this.boundedClickDispatchGeneration;
    this.activeAction.bounded_mutation_phase = "input-dispatch-through-after-evidence";
  }
  ...
  async handlePausedRequest(message, receipt = null) {
    ...
    if (this.calibrationNetworkActive) {
      ...
      await this.cdp.send("Fetch.failRequest", { requestId, errorReason: "BlockedByClient" }, message.sessionId);
    ...
    let origin;
    try {
      origin = normalizeOrigin(request.url);
    } catch {
      await this.cdp.send("Fetch.failRequest", { requestId, errorReason: "BlockedByClient" }, message.sessionId);
    ...
    try {
      privateClerkMutationLike = /^\/v1\/client\/(?:sign_(?:ins|ups)(?:\/|$)|sessions\/)/.test(new URL(request.url).pathname);
    } catch {}
    ...
    let clerkTransportMutation = this.config.clerk_auth
      ? isPermittedClerkMutation({
          rawUrl: request.url,
          method,
          frontendApiOrigin: this.config.clerk_auth.frontend_api_origin,
      ...
      : isPermittedClerkMutation({ rawUrl: request.url, method, frontendApiOrigin: origin, phase: null, activeSessionId: null });
    ...
    if (provisionalSessionId !== null) {
      const provisionalAllowed =
        receipt?.clerk_transport_attempt === this.clerkTransportAttempt &&
        Number.isFinite(receipt?.clerk_bootstrap_deadline_ms) &&
        Date.now() <= receipt.clerk_bootstrap_deadline_ms &&
        Date.now() <= this.absoluteDeadlineMs &&
        this.provisionalClerkSessionId === null &&
        this.provisionalClerkTouchCount === 0;
    ...
    if (["EventSource", "WebSocket", "Ping", "Beacon"].includes(resourceType)) {
      await this.appendRequestEvent({
        timestamp: isoNow(), outcome: "forbidden-channel-blocked-before-dispatch",
        url: privateClerkMutationLike ? undefined : sanitizeUrl(request.url),
    ...
    if (resourceType === "Document") {
      ...
      try {
        exactDocumentUrl = new URL(request.url).href;
      } catch {
      ...
      const boundedScopeExit =
        ...
        origin === normalizeOrigin(this.boundedScopeHref) &&
        exactDocumentUrl !== this.boundedScopeHref;
      if (!this.activeNavigation?.allowed_urls.has(exactDocumentUrl) && !boundedScopeExit) {
        await this.appendRequestEvent({
          timestamp: isoNow(), outcome: "unobserved-navigation-blocked-before-dispatch", url: sanitizeUrl(request.url), method, resource_type: resourceType
        }, receipt);
        await this.cdp.send("Fetch.failRequest", { requestId, errorReason: "BlockedByClient" }, message.sessionId);
    ...
    if (!['GET', 'HEAD', 'OPTIONS'].includes(method) && this.activeAction?.action_class === BOUNDED_ONBOARDING_CLASS && origin !== normalizeOrigin(this.boundedScopeHref)) {
      await this.cdp.send("Fetch.failRequest", { requestId, errorReason: "BlockedByClient" }, message.sessionId);
      await this.abort("unknown_mutation_request", "A bound-route action attempted an off-origin mutation", this.privateAuthUnknownMutationDiagnostic({ rawUrl: request.url, origin, method, receipt }));
      return;
    }
    if (privateClerkMutationLike && !clerkTransportMutation) {
      ...
      await this.cdp.send("Fetch.failRequest", { requestId, errorReason: "BlockedByClient" }, message.sessionId);
      await this.abort("unknown_mutation_request", "An unbound mutation-capable request was blocked", this.privateAuthUnknownMutationDiagnostic({ rawUrl: request.url, origin, method, receipt }));
      return;
    }
    let matchedMutation = null;
    let privateAuthAdmission = null;
    if (!["GET", "HEAD", "OPTIONS"].includes(method) && !clerkTransportMutation) {
      const expected = this.activeAction?.allowed_request;
      let exactActualUrl = null;
      try {
        exactActualUrl = new URL(request.url).href;
      } catch {}
      const persistedUrl = privateClerkMutationLike ? undefined : sanitizeUrl(request.url);
      const boundedMutation =
        receipt?.phase === "input-dispatch-through-after-evidence" &&
        receipt.action_id === this.activeAction?.bounded_action_id &&
        receipt.generation === this.activeAction?.bounded_click_dispatch_generation &&
        origin === normalizeOrigin(this.boundedScopeHref) &&
        this.activeAction.same_origin_mutation_requests_dispatched < 1;
    ...
    if (matchedMutation && this.activeAction) {
      ...
      if (matchedMutation.bounded) this.activeAction.same_origin_mutation_requests_dispatched += 1;
    }
    const persistedUrl = clerkTransportMutation ? undefined : sanitizeUrl(request.url);
    const matchedEffect = this.activeAction?.matched_request?.url === new URL(request.url).href;
    await this.appendRequestEvent({
      timestamp: isoNow(),
    ...
    await this.cdp.send("Fetch.continueRequest", { requestId }, message.sessionId);
  }
  ...
  async reserveOperation(request) {
    ...
    await mutateCapState(this.config.cap_state_path, (state) => {
      ...
      if (!denied && request.action_class === "Listed own-account progress") {
        ...
        const registered = this.effectRegistry[request.effect_id];
  ...
  validateAction(request) {
    if (!ALLOWED_METHODS.has(request.method)) return refusal("method_not_exposed", "The browser broker does not expose this operation");
    ...
    if (request.method === "click" && !["Observe", "Reversible own-account", "Listed own-account progress", BOUNDED_ONBOARDING_CLASS].includes(request.action_class)) {
    ...
    if (
      ...
      !/^[0-9a-f]{64}$/.test(request.observation_hash ?? "")
  ...
  async accessibilityObservation(exactUrl, providedTree = null) {
    ...
    for (const node of tree.nodes ?? []) {
      ...
      const name = String(node.name?.value ?? node.value?.value ?? "").replace(/\s+/g, " ").trim();
      if (!name && !["textbox", "searchbox", "button", "link"].includes(role)) continue;
      const ref = `e${++sequence}`;
      if (node.backendDOMNodeId) this.refs.set(ref, { backendDOMNodeId: node.backendDOMNodeId, role, name });
      this.lastObservedNodes.push({ ref, role, name });
  ...
  async waitForAccessibilitySettle(maxWaitMs = 2000, pollIntervalMs = 150) {
    ...
    while (Date.now() < deadline) {
      if (this.networkInflight.size === 0 && Date.now() - this.lastNetworkActivityAt >= POST_AUTH_NETWORK_QUIET_MS) {
        const tree = await this.cdp.send("Accessibility.getFullAXTree", {}, this.pageSessionId);
        const hash = hashAccessibilityTree(tree.nodes ?? []);
  ...
  async captureEvidence(eventId, position, expectedNavigation = null) {
    ...
    if (this.firstPostAuthCapturePending && this.postAuthReadinessConfirmed) {
      ...
      if (hashAccessibilityTree(observationTree.nodes ?? []) !== this.postAuthReadinessFingerprint?.axSha256) {
        await this.restartPrivatePostAuthReadiness();
        return this.captureEvidence(eventId, position, expectedNavigation);
      }
    }
    ...
    if (
      this.firstPostAuthCapturePending &&
      this.postAuthReadinessConfirmed &&
      sha256Text(captured.data ?? "") !== this.postAuthReadinessFingerprint?.screenshotSha256
    ) {
      await this.restartPrivatePostAuthReadiness();
      return this.captureEvidence(eventId, position, expectedNavigation);
    }
    ...
    await writeFile(absolute, Buffer.from(captured.data, "base64"), { mode: 0o600, flag: "wx" });
    const evidence = {
      url: sanitizedUrl,
  ...
  async calibrationPrivateState() {
    ...
    const normalizedCookies = (cookies.cookies ?? []).map((cookie) => ({
      name: cookie.name,
      value: cookie.value,
      domain: cookie.domain,
      path: cookie.path,
    ...
    return {
      ...
      accessibility_sha256: hashAccessibilityTree(tree.nodes ?? []),
      storage_sha256: sha256Text(JSON.stringify({ page: storage?.payload ?? null, cookies: normalizedCookies })),
      durable_storage_absent: storage?.durable_absent === true,
      storage_certain: storage?.certain === true,
      page_guard: storage?.guard ?? null
    };
  }
  ...
  calibrationSelectedButton(selection, expectedAction) {
    ...
    if (!selection || typeof selection !== "object" || Array.isArray(selection) || Object.keys(selection).sort().join("\n") !== expectedKeys.join("\n")) return null;
    if (selection.action !== expectedAction || selection.observation_sha256 !== this.calibrationBindings?.observation?.observation_sha256) return null;
    const matches = (this.calibrationBindings?.buttons ?? []).filter((binding) =>
      binding.public.role === selection.role &&
      binding.public.accessible_name === selection.accessible_name &&
      JSON.stringify(binding.public.nearby_visible_text) === JSON.stringify(selection.nearby_visible_text)
    );
  ...
  async calibrationObserveStart() {
    ...
    try {
      ...
      if (error?.postAuthReadinessRefusal) return error.postAuthReadinessRefusal;
      throw error;
    }
  ...
  async calibrationOpen(selection) {
    ...
    this.calibrationGuard.installation_complete = activated?.activated === true && activated?.installation_complete === true;
    continuation = await this.admitPrivateAuthContinuation();
    if (!continuation.ok) return continuation;
  ...
  async calibrationReverse(selection) {
    ...
    const fresh = await this.calibrationPrivateState();
    if (["raw_url_sha256", "authenticated_landing_intact", "accessibility_sha256", "storage_sha256", "durable_storage_absent", "storage_certain"].some((key) => fresh[key] !== this.calibrationOpenState[key])) {
    ...
    const pageGuard = final.page_guard ?? {};
  ...
  async issueActionToken(request) {
    if (this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    if (!["Reversible own-account", "Listed own-account progress", BOUNDED_ONBOARDING_CLASS].includes(request.action_class)) return refusal("token_class_forbidden", "Only admitted clicks may receive a token");
    const ref = String(request.ref ?? "").replace(/^@/, "");
    const semantic = this.refs.get(ref);
    if (!this.lastEvidence || !semantic) return refusal("token_binding_missing", "A current retained observation and semantic reference are required");
    ...
    if (request.action_class === "Listed own-account progress" && !request.expected_request) {
    ...
    if (
      ...
      (!this.boundedOnboarding ||
        ...
        !["button", "checkbox", "radio"].includes(semantic.role) ||
    ...
    if (request.expected_request) {
      const method = String(request.expected_request.method ?? "").toUpperCase();
      ...
      const exact = new URL(request.expected_request.url);
  ...
  consumeActionToken(request) {
    const token = this.actionTokens.get(request.authorization_token);
    this.actionTokens.delete(request.authorization_token);
    const ref = String(request.ref ?? "").replace(/^@/, "");
    const semantic = this.refs.get(ref);
    if (
      !token ||
  ...
  async clickRef(ref, beforeDispatch = null) {
    ...
    if (!quad || quad.length < 8) throw new Error("Element has no clickable box");
  ...
  async followObservedLink(ref) {
    ...
    const destination = new URL(attributes.href, this.currentUrl).href;
  ...
  async execute(request, caller = "sandbox") {
    if (["calibration_observe_start", "calibration_open", "calibration_reverse"].includes(request.method)) {
    ...
    if (request.method === "navigate") {
      let exactUrl;
      try {
        exactUrl = new URL(request.url).href;
      } catch {
        exactUrl = null;
      }
    ...
    if (request.method === "click" && request.action_class === "Reversible own-account" && this.reversibleActionRegistry[request.effect_id] !== request.effect_class_id) {
    ...
    try {
      ...
      if (error?.postAuthReadinessRefusal) return error.postAuthReadinessRefusal;
      if (error?.privateIdentityExposure) {
        return refusal("private_identity_exposure", "Private identity exposure prevented evidence retention");
      }
    ...
    if (requiresObservationBinding && request.observation_hash !== before.observation_hash) {
    ...
    if (
      tokenBinding &&
      (tokenBinding.url !== before.url ||
        tokenBinding.observation_hash !== before.observation_hash ||
        tokenBinding.role !== this.refs.get(tokenBinding.ref)?.role ||
        tokenBinding.name !== this.refs.get(tokenBinding.ref)?.name)
    ) {
    ...
    try {
      if (request.method === "navigate") {
        ...
        this.activeNavigation = { allowed_urls: new Set([new URL(request.url).href]), last_url: null };
        this.assertEffectfulAdmissionOpen();
        await this.cdp.send("Page.navigate", { url: request.url }, this.pageSessionId);
        ...
        if (!target || !["textbox", "searchbox"].includes(target.role)) throw new Error("Typing requires a visible text input reference");
        this.assertEffectfulAdmissionOpen();
      ...
      const changed = before ? before.observation_hash !== after.observation_hash : true;
  ...
  async recordEvent(request, eventId, before, after, outcome, detail, transitionKind) {
    ...
    const effectEvidence = request.action_class === BOUNDED_ONBOARDING_CLASS
      ...
      : ["Reversible own-account", "Listed own-account progress"].includes(request.action_class) && this.config.redact_private_effect_bindings
      ? {
          supervisor_authorized: true,
          mutation_request_match: request.action_class === "Listed own-account progress" ? Boolean(this.activeAction?.matched_request) : null
        }
      : ["Reversible own-account", "Listed own-account progress"].includes(request.action_class)
      ? {
          effect_id: request.effect_id ?? null,
          effect_class_id: request.effect_class_id ?? null,
      ...
      : null;
    const state = await this.loadState();
    const safeIntent = { method: request.method, ref: request.ref, delta_y: request.delta_y, text_bytes: request.text ? Buffer.byteLength(request.text) : undefined };
    const event = {
      run_id: this.config.run_id,
      event_id: eventId,
      timestamp: isoNow(),
      current_url: after?.url ?? before?.url ?? null,
      current_origin: after?.origin ?? before?.origin ?? null,
      visible_state_summary: after?.visible_state_summary ?? before?.visible_state_summary ?? "unavailable",
      before,
      after,
      ...
      screenshot_path: after?.screenshot_path ?? before?.screenshot_path ?? null,
      ...
      elapsed_browser_seconds: Math.round(state.browser.active_seconds * 1000) / 1000,
      cumulative_browser_actions: state.browser.operations,
      cumulative_browser_requests: state.browser.requests,
      cumulative_app_actual_eur: state.app.actual_eur,
      cumulative_model_actual_eur: state.model.actual_eur,
      outstanding_cost_reservation_eur: state.app.outstanding_reservations_eur + state.model.outstanding_reservations_eur
    };
...
export async function announceBrowserBrokerReadiness(broker, output = process.stdout) {
  try {
    ...
    const message = { ready: false, broker: BROKER_ID, error: String(error.message).slice(0, 200) };
    writeJsonLine(output, message);
  ...
  if (readiness.ok) {
    ...
    attachJsonLineReader(process.stdin, (message) => {
      inboundQueue = inboundQueue.catch(() => {}).then(async () => {
        let response;
        try {
          response = await broker.execute(message.payload ?? {}, message.caller ?? "sandbox");
        } catch {
        ...
        if (response?.ok === false && response.refusal?.code) {
          await recordRefusal(config.cap_state_path, message.payload?.method, response.refusal.code);
        }
