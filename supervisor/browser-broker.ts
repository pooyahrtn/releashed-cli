...
import { isNodeOnScreen } from "../lib/ax-visibility.ts";
import { formatAxLine } from "../lib/ax-line-format.ts";
import type { ClerkPageAuthOutcome } from "../lib/clerk-auth.ts";
import {
  CLERK_BOUNDED_ROUTE_FAILURE_CODES,
  CLERK_BOUNDED_ROUTE_FAILURE_POINTS,
...
import {
  BROWSER_STARTUP_BUDGET_MS,
  CDP_COMMAND_TIMEOUT_MS,
  INITIAL_NAVIGATION_FAILURE_POINTS,
  INITIAL_TOP_FRAME_LOAD_TIMEOUT_MS
} from "../lib/browser-runtime-limits.ts";
import { revalidateOwnedBrowserProfile } from "./browser-profile-custody.mjs";

/** One broker RPC outcome: success payload or a refusal triple. */
export type BrokerResult = {
  ok: boolean;
  refusal?: { code?: unknown; [key: string]: unknown };
  [key: string]: unknown;
};

/** Untyped record flowing across the CDP or page-function boundary. */
...
function hasPrivateIdentityExposure(error: unknown): boolean {
...
function containsExactPrivateText(value: unknown, needles: readonly string[], seen: Set<object> = new Set()): boolean {
  if (typeof value === "string") return needles.some((needle) => value.includes(needle));
  if (!value || typeof value !== "object" || seen.has(value)) return false;
  seen.add(value);
  if (Array.isArray(value)) return value.some((entry) => containsExactPrivateText(entry, needles, seen));
  return Object.values(value).some((entry) => containsExactPrivateText(entry, needles, seen));
}

function privateIdentityExposureError(): PrivateIdentityExposureError {
  return new PrivateIdentityExposureError("Private identity exposure prevented evidence retention");
}

function publicCapState(state: unknown): unknown {
  if (!isRecord(state)) return state;
  const abort = state.abort;
  if (!isRecord(abort) || !abort.operator_diagnostic) return state;
  const clone: JsonRecord = structuredClone(state);
  const cloneAbort = clone.abort;
  if (isRecord(cloneAbort)) delete cloneAbort.operator_diagnostic;
  return clone;
}

function sanitizeCalibrationText(value: unknown): string {
...
/** One accessibility-tree name/role/description/value payload from CDP. */
type AxProtocolValue = {
  type?: unknown;
  value?: unknown;
  relatedNodes?: Array<{ text?: unknown }>;
};

type AxValue = {
  type: unknown;
  value: unknown;
  related_nodes?: Array<{ text: unknown }>;
};

function axValue(value: AxProtocolValue | null | undefined): AxValue | null {
  if (!value || typeof value !== "object") return null;
  const normalized: AxValue = { type: value.type ?? null, value: value.value ?? null };
...
/** One accessibility-tree node from CDP Accessibility.getFullAXTree. */
export type AxNode = {
  nodeId?: unknown;
  ignored?: unknown;
  ignoredReasons?: Array<{ name?: unknown; value?: AxProtocolValue | null }>;
  role?: AxProtocolValue | null;
  name?: AxProtocolValue | null;
  description?: AxProtocolValue | null;
  value?: AxProtocolValue | null;
  properties?: Array<{ name?: unknown; value?: AxProtocolValue | null }>;
  parentId?: unknown;
  childIds?: unknown[];
};

export function hashAccessibilityTree(nodes: AxNode[] = []): string {
...
/** One calibration capture state compared by restoration proof. */
export type CalibrationState = {
  raw_url_sha256?: unknown;
  accessibility_sha256?: unknown;
  storage_sha256?: unknown;
  durable_storage_absent?: unknown;
  storage_certain?: unknown;
};

export type CalibrationGuard = {
  request_attempted?: unknown;
  all_requests_blocked_before_dispatch?: unknown;
  transport_attempted?: unknown;
  storage_mutation_attempted?: unknown;
  installation_complete?: unknown;
};

export type CalibrationProof = {
  open_state_observed: boolean;
  exact_reverse_control_used: boolean;
  reversal_attempted: boolean;
  exact_raw_url_restore: boolean;
  exact_semantic_restore: boolean;
  storage_unchanged: boolean;
  durable_storage_absent: boolean;
  request_observed: boolean;
  all_requests_blocked_before_dispatch: boolean;
  transport_attempt_observed: boolean;
  storage_mutation_attempted: boolean;
  guard_installation_confirmed: boolean;
  safe: boolean;
};

export function evaluateCalibrationRestoration({ baseline, open, final, guard = {} }: {
  baseline?: CalibrationState | null;
  open?: CalibrationState | null;
  final?: CalibrationState | null;
  guard?: CalibrationGuard;
}): CalibrationProof {
  const openStateObserved = Boolean(
    baseline && open &&
  ...
  const durableStorageAbsent = Boolean(
    baseline?.durable_storage_absent && open?.durable_storage_absent && final?.durable_storage_absent
  );
  ...
  const safe = Object.entries(proofBase).every(([key, value]) => {
    if (["request_observed", "transport_attempt_observed", "storage_mutation_attempted"].includes(key)) return value === false;
    return value === true;
  }) && storageCertain;
...
export function calibrationGuardSource({ calibration, allowFixtureServiceWorker }: {
  calibration: unknown;
  allowFixtureServiceWorker: unknown;
}): string {
...
export async function waitForDevToolsPort(devtoolsFile: string, {
  readFileImpl = readFile,
  retryDelayMs = 50,
  attempts = null,
  deadlineMs = Date.now() + BROWSER_STARTUP_BUDGET_MS
}: {
  ...
  for (let attempt = 0; attempts === null || attempt < attempts; attempt += 1) {
    ...
    if ((attempts !== null && attempt + 1 >= attempts) || Date.now() >= deadlineMs) break;
...
/** One CDP frame in either direction. Payloads stay untyped past this point. */
export type CdpMessage = {
  id?: unknown;
  error?: unknown;
  // CDP command results are objects per protocol; `?? {}` below keeps the
  // null-coalescing. Labelling the boundary once preserves runtime exactly.
  result?: JsonRecord;
  sessionId?: unknown;
  method?: unknown;
  params?: unknown;
...
export class CdpConnection {
  socket: WebSocket;
  nextId: number;
  pending: Map<unknown, CdpPending>;
  listeners: Set<CdpEventListener>;
  commandTimeoutMs: number;
  absoluteDeadlineMs: number;
  terminalError: Error | null;

  constructor(socket: WebSocket, { commandTimeoutMs = CDP_COMMAND_TIMEOUT_MS, absoluteDeadlineMs = Date.now() + 60_000 }: {
    commandTimeoutMs?: number;
    absoluteDeadlineMs?: number;
  } = {}) {
    this.socket = socket;
    ...
    socket.addEventListener("message", (event) => {
      ...
      if (message.id) {
        ...
        const responseError: unknown = message.error;
        if (responseError) {
          const detail: { message?: unknown } | null = isRecord(responseError) ? responseError : null;
  ...
  send(method: string, params: JsonRecord = {}, sessionId?: string): Promise<JsonRecord> {
    ...
    const message: { id: number; method: string; params: JsonRecord; sessionId?: string } = { id, method, params };
  ...
  setLimits({ commandTimeoutMs, absoluteDeadlineMs }: { commandTimeoutMs: number; absoluteDeadlineMs: number }): void {
    if (this.pending.size !== 0) throw new Error("CDP limits cannot change while a command is pending");
...
/** Supervisor-issued broker configuration. The four directory/origin fields are
 * required: the constructor dereferences them unconditionally. Everything else
 * is read behind `??`/`?.` or post-start, matching the original access pattern. */
export type BrokerConfig = {
  ...
  suppressed_request_origins?: string[];
  effect_registry?: Record<string, unknown>;
  reversible_action_registry?: Record<string, unknown>;
  runtime_mode?: unknown;
  operation_deadline_ms?: unknown;
  clerk_auth?: {
  ...
  saved_session?: { path: string; sha256: string } | null;
  auth_bootstrap_admissions?: Array<{
    method: string;
    url: string;
    id: string;
  ...
  calibration_probe?: {
    mode?: unknown;
    public_run_path?: unknown;
  };
  chromium_executable?: string;
  profile_ownership?: unknown;
  transient_auth?: {
    mode?: unknown;
    origin?: unknown;
    header_name?: unknown;
  };
  fixed_preflight_service_worker_request?: string;
  app_cost_ledger?: {
    allowed_one_way_classes: Array<{
      id: string;
      maximum_count: number;
      worst_case_eur_per_action: number;
    }>;
  };
  cap_state_path: string;
  run_id?: string;
  redact_private_effect_bindings?: unknown;
};

function isBrokerConfig(value: unknown): value is BrokerConfig {
...
function postAuthReadinessRefusalOf(error: unknown): BrokerResult | null {
  if (!isRecord(error)) return null;
  const refusal = error.postAuthReadinessRefusal;
  if (!isRecord(refusal) || typeof refusal.ok !== "boolean") return null;
...
function eventParams(message: CdpMessage): JsonRecord {
  if (!isRecord(message.params)) throw new Error("CDP event params are unavailable");
  return message.params;
}

/** CDP target info, fail-closed like event params above. */
function eventTargetInfo(message: CdpMessage): JsonRecord {
  const params = eventParams(message);
  if (!isRecord(params.targetInfo)) throw new Error("CDP target info is unavailable");
  return params.targetInfo;
}

/** Full private navigation identity: URL string plus the frame/loader/sequence
 * lineage that distinguishes same-URL history churn from a settled landing. */
export type PrivateNavigationSnapshot = {
  exactUrl: string;
  entryId: number;
  entryIndex: number;
  entryCount: number;
  predecessorId: number | null;
  predecessorUrl: string | null;
  twoBackId: number | null;
  twoBackUrl: string | null;
  frameId: string;
  loaderId: string;
  sequence: number;
...
/** Authorization-relevant phase captured synchronously at CDP receipt time. */
export type BoundedRequestReceipt = {
  readonly action_id: unknown;
  readonly phase: unknown;
  readonly generation: unknown;
  readonly suppress_retention: boolean;
  readonly clerk_transport_phase: string | null;
  readonly clerk_transport_session: string | null;
  readonly clerk_transport_attempt: object | null;
  readonly clerk_bootstrap_deadline_ms: number | null;
};

/** One post-auth readiness poll: abort marker, exposure marker, not-ready, or proof. */
type PostAuthReadinessSample =
  | { aborted: boolean }
  | { identityExposure: boolean }
  | { fingerprint: string; axSha256: string; screenshotSha256: string; networkSequence: number }
  | null
  | false;

/** One in-flight supervised action: the admitted request plus dispatch bookkeeping. */
type ActiveAction = {
  action_class: unknown;
  allowed_request: unknown;
  matched_write: boolean;
  matched_request?: unknown;
  authorization_consumed_once?: unknown;
  same_origin_mutation_requests_dispatched?: unknown;
  bounded_click_dispatched?: unknown;
  bounded_click_dispatch_generation?: unknown;
  bounded_mutation_phase?: unknown;
  bounded_action_id?: unknown;
  effect_id?: unknown;
  effect_class_id?: unknown;
};

/** Frozen initial-landing identity: the bound entry plus its sequence. */
...
/** One retained evidence capture: sanitized URL, hashes and screenshot reference. */
...
/** One calibration capture: hashes plus the retained evidence reference. */
...
/** One top-frame navigation observation: document loads and same-document hops. */
type TopNavigationEvent = {
  kind: string;
  navigationType?: unknown;
  sequence: number;
};

/** Candidate initial landing seen on the wire before the route binds. */
type InitialLandingEventCandidate = {
  exactUrl: string;
  frameId: string;
  loaderId: string;
  sequence: number;
  sameUrlHistoryApiNotifications: number;
};

/** Where the private Clerk location resolved: the landing itself or its auth transit. */
type ClerkLocationAdmission =
  | { kind: "direct"; exactCurrentHref: string; sequence: number; navigation: PrivateNavigationSnapshot }
  | { kind: "transit"; exactCurrentHref: string; sequence: number; navigation: PrivateNavigationSnapshot };

/** The frozen post-auth handoff kept for route binding: kind plus navigation only. */
export type BoundedPostAuthHandoff = {
  kind: "direct" | "transit";
  navigation: PrivateNavigationSnapshot;
};

/** One route-binding lineage walk: hop budget plus the last admitted sample. */
type RouteChain = {
  hops: number;
  last?: PrivateNavigationSnapshot | null;
};
...
export class BrowserBroker {
  ...
  suppressedOrigins: Set<string>;
  operationTimes: number[];
  requestTimes: number[];
  requestQueue: Promise<unknown>;
  refs: Map<string, { backendDOMNodeId: unknown; role: unknown; name: unknown }>;
  currentUrl: string;
  eventSequence: number;
  sessionTypes: Map<string, { targetId: string; type: string }>;
  sessionReady: Map<string, Promise<void>>;
  redirectIds: Set<string>;
  effectRegistry: Record<string, unknown>;
  reversibleActionRegistry: Record<string, unknown>;
  boundedOnboarding: boolean;
  boundedScopeHref: string | null;
  boundedScopeRawUrlSha256: string | null;
  boundedScopeIdentity: PrivateNavigationSnapshot | null;
  boundedScopeComplete: boolean;
  boundedPostAuthStartPending: boolean;
  boundedPostAuthHandoff: BoundedPostAuthHandoff | null;
  boundedExpectedClerkUserId: string | null;
  boundedPostAuthResumeAttempted: boolean;
  boundedPrivateAuthStart: boolean;
  savedSessionConfig: { path: string; sha256: string } | null;
  privateIdentityNeedles: readonly string[] | null;
  boundedClickDispatchGeneration: number;
  authBootstrapAdmissions: Map<string, { id: string; remaining: number }>;
  boundedPendingScopeExit: PrivateNavigationSnapshot | null;
  actionTokens: Map<string, JsonRecord>;
  absoluteDeadlineMs: number;
  calibrationEnabled: boolean;
  calibrationGuard: CalibrationGuard;
  clerkTransportPhase: string | null;
  activeClerkSessionId: string | null;
  clerkTransportAttempt: object | null;
  clerkBootstrapTransportDeadlineMs: number | null;
  provisionalClerkSessionId: string | null;
  provisionalClerkTouchCount: number;
  networkInflight: Set<string>;
  routeBindingNetworkInflight: Set<string>;
  backgroundAbortTasks: Set<Promise<unknown>>;
  backgroundAbortLatched: boolean;
  networkActivitySequence: number;
  routeBindingNetworkActivitySequence: number;
  lastNetworkActivityAt: number;
  postAuthReadinessTimeoutMs: number;
  boundedRouteBindingTimeoutMs: number;
  boundedRouteStableWindowMs: number;
  boundedRouteStableSamples: number;
  boundedRouteHopCap: number;
  boundedRouteFailurePoint: string | null;
  navigationSequence: number;
  initialTopFrameLoadTimeoutMs: number;
  logicalNavigationSequence: number;
  topFrameId: string | null;
  topFrameLoaderId: string | null;
  lastTopNavigation: TopNavigationEvent | null | undefined;
  lastLogicalTopNavigation: TopNavigationEvent | null | undefined;
  lastTopLogicalLocation: { urlSha256: string; frameId: string | null; loaderId: string | null } | null;
  topFrameLoadSequence: number;
  pageLoadWaiters: Set<(error?: Error | null) => void>;
  clerkAuthenticator: OneShotClerkPageAuthenticator;
  chrome!: import("node:child_process").ChildProcess;
  cdp!: CdpConnection;
  pageTargetId!: string;
  pageSessionId!: string;
  executableIdentity!: string;
  transientAuthInstalled: boolean | undefined;
  transientAuthCleared: boolean | undefined;
  activeAction: ActiveAction | null | undefined;
  activeNavigation: {
    allowed_urls: Set<string>;
    last_url: string | null;
    reject_redirects?: boolean;
  } | null | undefined;
  initialLandingIdentity: LandingIdentity | null | undefined;
  authLandingUrlSha256: string | null | undefined;
  authLandingNavigationSequence: number | null | undefined;
  initialNavigationConsumed: boolean | undefined;
  initialNavigationPredecessor: PrivateNavigationSnapshot | null | undefined;
  initialLandingEventCandidate: InitialLandingEventCandidate | null | undefined;
  lastEvidence: JsonRecord | null | undefined;
  lastEvidenceRawUrlSha256: string | null | undefined;
  lastObservedNodes: Array<{ ref: string; role: string; name: string }> | null | undefined;
  postAuthReadinessConfirmed: boolean | undefined;
  postAuthReadinessDeadlineMs: number | null | undefined;
  postAuthReadinessFingerprint: {
    axSha256: unknown;
    screenshotSha256: unknown;
    networkSequence: unknown;
  } | null | undefined;
  firstPostAuthCapturePending: boolean | undefined;
  calibrationBaseline: CalibrationCapture | null | undefined;
  calibrationOpenState: CalibrationCapture | null | undefined;
  calibrationBindings: JsonRecord | null | undefined;
  calibrationNetworkActive: boolean | undefined;
  fixtureWorkerReserved: boolean | undefined;
  fixtureWorkerObserved: boolean | undefined;
  clerkAuthenticationAttempted: boolean | undefined;

  constructor(config: BrokerConfig, {
    startupBudgetMs = BROWSER_STARTUP_BUDGET_MS,
    findChromiumImpl = findChromium,
    spawnBrowserImpl = spawn,
    waitForDevToolsPortImpl = waitForDevToolsPort,
    fetchImpl = fetch,
    WebSocketImpl = WebSocket
  }: {
    startupBudgetMs?: number;
    findChromiumImpl?: typeof findChromium;
    spawnBrowserImpl?: typeof spawn;
    waitForDevToolsPortImpl?: typeof waitForDevToolsPort;
    ...
    this.authBootstrapAdmissions = new Map(
      (config.auth_bootstrap_admissions ?? []).map((entry) => [
        `${entry.method}\n${entry.url}`,
    ...
    const operationDeadlineMs: unknown = config.operation_deadline_ms;
    ...
    this.clerkAuthenticator = new OneShotClerkPageAuthenticator({
      start: ({ ticket, expectedUserId, expectedCurrentHref }) =>
        this.startClerkAuthentication(ticket, expectedUserId, expectedCurrentHref),
  ...
  async start(): Promise<void> {
    ...
    if (!(this.boundedOnboarding && this.config.clerk_auth)) {
      ...
      if (profileEntries.length !== 0) throw new Error("The exclusively created browser profile is not empty");
    }
    const existingRunFiles = await readdir(this.runDirectory);
    if (existingRunFiles.length !== 0) throw new Error("Run directory must be new and empty");
    ...
    if (connectRemaining <= 0) throw new Error("Browser startup deadline reached");
    const version: { webSocketDebuggerUrl?: unknown } = await (await this.fetch(`http://127.0.0.1:${port}/json/version`, {
      signal: AbortSignal.timeout(connectRemaining)
    })).json();
    const debuggerUrl = version.webSocketDebuggerUrl;
    if (typeof debuggerUrl !== "string" || !debuggerUrl) throw new Error("Chromium did not publish its DevTools endpoint");
    ...
    const page = Array.isArray(targetInfos)
      ...
      : undefined;
    if (!page) throw new Error("Chromium did not create an initial page target");
    const pageTargetId = page.targetId;
    if (typeof pageTargetId !== "string" || !pageTargetId) throw new Error("Chromium did not create an initial page target");
    ...
    if (typeof attachedSessionId !== "string" || !attachedSessionId) throw new Error("Chromium did not attach to the initial page target");
  ...
  async waitForSavedSessionLanding(deadlineMs: number): Promise<PrivateNavigationSnapshot> {
    ...
    while (Date.now() < deadlineMs) {
      if (isAuthFlowUrl(snapshot.exactUrl)) return snapshot;
      if (previous && this.samePrivateNavigation(snapshot, previous)) {
        stableSamples += 1;
        if (stableSamples >= 4) return snapshot;
      } else {
        stableSamples = 0;
      }
      previous = snapshot;
  ...
  async loadSavedSession(): Promise<void> {
    ...
    if (!bound) throw new Error("saved_session_unreadable: the saved session file could not be read");
    let raw: Buffer;
    try {
    ...
    if (sha256Text(raw) !== bound.sha256) throw new Error("saved_session_hash_mismatch: the saved session file changed after preparation");
    let session: { cookies?: unknown; origin_storage?: unknown };
    try {
      session = JSON.parse(raw.toString("utf8"));
    } catch {
      throw new Error("saved_session_invalid: the saved session file is not valid JSON");
    }
    const cookiesSource: unknown = session.cookies;
    const originStorageSource: unknown = session.origin_storage;
    ...
    const cookies = cookiesSource.map((cookie: unknown) => {
    ...
    const originStorage: unknown[] = Array.isArray(originStorageSource) ? originStorageSource : [];
    if (cookies.length === 0 && originStorage.length === 0) throw new Error("saved_session_invalid: the saved session contains no cookies or storage");
    ...
    const hasLiveCookie = cookies.some((cookie) => {
      const expires: unknown = cookie.expires;
      return typeof expires !== "number" || !Number.isFinite(expires) || expires < 0 || expires > nowSeconds;
    });
    if (cookies.length > 0 && !hasLiveCookie) throw new Error("saved_session_expired: every captured cookie has already expired");
    if (cookies.length > 0) await this.cdp.send("Network.setCookies", { cookies }, this.pageSessionId);
    for (const entry of originStorage) {
      if (!isRecord(entry) || typeof entry.origin !== "string") throw new Error("saved_session_invalid: an origin-storage entry is missing its origin");
      let origin: string;
      try {
      ...
      const local: unknown = entry.local_storage && typeof entry.local_storage === "object" ? entry.local_storage : {};
      const session_: unknown = entry.session_storage && typeof entry.session_storage === "object" ? entry.session_storage : {};
  ...
  async installTransientAuth(request: JsonRecord): Promise<BrokerResult> {
    if (this.transientAuthInstalled) return refusal("transient_auth_already_consumed", "Transient authentication can be installed only once");
    const policy = this.config.transient_auth;
    const sentinel: unknown = request?.sentinel;
    if (
      policy?.mode !== "header" ||
    ...
    if (this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
  ...
  async clearTransientAuth(): Promise<BrokerResult> {
    if (!this.transientAuthInstalled || this.transientAuthCleared) return refusal("transient_auth_not_active", "Transient authentication is not active");
    await this.cdp.send("Network.setExtraHTTPHeaders", { headers: {} }, this.pageSessionId);
    this.transientAuthCleared = true;
    return { ok: true, cleared: true };
  }

  async callPageFunction(functionDeclaration: string, values: unknown[] = [], effectful = false): Promise<unknown> {
    if (effectful) this.assertEffectfulAdmissionOpen();
    ...
    const rootObjectId: unknown = rootResult?.objectId;
    if (!rootObjectId) throw new Error("The page JavaScript context is unavailable");
    if (effectful) this.assertEffectfulAdmissionOpen();
    ...
    if (response.exceptionDetails) throw new Error("The page rejected Clerk authentication");
    const responseResult = isRecord(response.result) ? response.result : null;
    return responseResult?.value;
  }

  async startClerkAuthentication(ticket: string, expectedUserId: string, expectedCurrentHref: unknown): Promise<ClerkPageAuthOutcome> {
    const attempt = this.clerkTransportAttempt;
    if (!attempt) return { failure_stage: "precondition_failed" };
    this.clerkTransportPhase = "bootstrap";
    if (this.backgroundAbortLatched) return { failure_stage: "precondition_failed" };
    const exchange: unknown = await this.callPageFunction(CLERK_PAGE_FUNCTIONS.start, [ticket, expectedUserId, expectedCurrentHref], true);
    if (this.clerkTransportAttempt !== attempt) return { failure_stage: "precondition_failed" };
    if (isRecord(exchange) && "failure_stage" in exchange) return exchange;
    const createdSessionId: unknown = isRecord(exchange) ? exchange.created_session_id : undefined;
    if (typeof createdSessionId !== "string" || !/^[A-Za-z0-9_-]{4,256}$/.test(createdSessionId)) return { failure_stage: "session_missing" };
    const continuation = await this.admitPrivateAuthContinuation();
    if (!continuation.ok || this.clerkTransportAttempt !== attempt) return { failure_stage: "precondition_failed" };
    ...
    if (this.backgroundAbortLatched) return { failure_stage: "precondition_failed" };
    const activated: unknown = await this.callPageFunction(CLERK_PAGE_FUNCTIONS.activate, [createdSessionId, expectedUserId], true);
    if (this.clerkTransportAttempt !== attempt) return { failure_stage: "precondition_failed" };
    return isRecord(activated) ? activated : null;
  }

  inspectClerkAuthentication(): Promise<ClerkPageAuthOutcome> {
    return this.callPageFunction(CLERK_PAGE_FUNCTIONS.inspect).then((value) => (isRecord(value) ? value : null));
  }

  assertEffectfulAdmissionOpen(): void {
    if (!this.backgroundAbortLatched) return;
    throw new BackgroundAbortError("The browser run is not live");
  }

  async lockClerkAuthenticationMethods(): Promise<void> {
    const result: unknown = await this.callPageFunction(CLERK_PAGE_FUNCTIONS.lock);
    if (!isRecord(result) || result.locked !== true) throw new Error("The page Clerk methods could not be locked");
  }

  async privateNavigationSnapshot(): Promise<PrivateNavigationSnapshot> {
    const history = await this.cdp.send("Page.getNavigationHistory", {}, this.pageSessionId);
    const rawIndex: unknown = history.currentIndex;
    const rawEntries: unknown = history.entries;
    const safeEntries: unknown[] = Array.isArray(rawEntries) ? rawEntries : [];
    const safeIndex: number = typeof rawIndex === "number" && Number.isSafeInteger(rawIndex) ? rawIndex : -1;
    const current: unknown = safeIndex >= 0 ? safeEntries[safeIndex] : null;
    const currentFields = isRecord(current) ? current : null;
    const exactUrl: unknown = currentFields?.url;
    const entryId: unknown = currentFields?.id;
    const frameId = this.topFrameId;
    const loaderId = this.topFrameLoaderId;
    if (
      typeof exactUrl !== "string" ||
      !exactUrl ||
    ...
    const predecessor: unknown = safeIndex > 0 ? safeEntries[safeIndex - 1] : null;
    const twoBack: unknown = safeIndex > 1 ? safeEntries[safeIndex - 2] : null;
    const predecessorFields = isRecord(predecessor) ? predecessor : null;
    const twoBackFields = isRecord(twoBack) ? twoBack : null;
    const predecessorId: unknown = predecessorFields?.id;
    const predecessorUrl: unknown = predecessorFields?.url;
    const twoBackId: unknown = twoBackFields?.id;
    const twoBackUrl: unknown = twoBackFields?.url;
    return {
      exactUrl,
      entryId,
  ...
  samePrivateNavigation(left: PrivateNavigationSnapshot | null | undefined, right: PrivateNavigationSnapshot | null | undefined): boolean {
    return Boolean(
      left &&
      right &&
  ...
  sameCurrentPrivateNavigation(snapshot: PrivateNavigationSnapshot | null | undefined): boolean {
    return Boolean(
      snapshot &&
  ...
  samePrivateLogicalNavigation(left: PrivateNavigationSnapshot | null | undefined, right: PrivateNavigationSnapshot | null | undefined): boolean {
    return Boolean(
      left &&
      right &&
      left.exactUrl === right.exactUrl &&
  ...
  sameAdmittedAuthTransit(left: PrivateNavigationSnapshot | null | undefined, right: PrivateNavigationSnapshot | null | undefined): boolean {
    try {
      if (!left || !right) return false;
      return Boolean(
        isAuthFlowUrl(left.exactUrl) &&
  ...
  nextPrivateLogicalNavigation(left: PrivateNavigationSnapshot | null | undefined, right: PrivateNavigationSnapshot | null | undefined): boolean {
    if (
      !left ||
      !right ||
      left.exactUrl === right.exactUrl ||
    ...
    const replacedCurrentEntry =
      right.entryId === left.entryId &&
    ...
    const appendedEntry =
      right.entryIndex === left.entryIndex + 1 &&
  ...
  noteBoundedRouteFailure(point: string): null {
  ...
  advanceBoundedRouteChain(chain: RouteChain, sample: PrivateNavigationSnapshot): boolean {
    if (!chain?.last) {
  ...
  isExactLanding(snapshot: PrivateNavigationSnapshot | null | undefined, sequence: number | null | undefined = this.authLandingNavigationSequence): boolean {
    ...
    if (!landing || !snapshot) return false;
    return Boolean(
      snapshot.exactUrl === landing.exactUrl &&
  ...
  matchesClerkTransitContract(rawUrl: string, landingUrl: string): boolean {
    let transit;
    let landing;
    try {
  ...
  bindInitialLanding(snapshot: PrivateNavigationSnapshot | null | undefined): boolean {
    if (this.initialLandingIdentity) return this.isExactLanding(snapshot, this.initialLandingIdentity.sequence);
    if (!snapshot) return false;
    let origin: string;
    try {
  ...
  initialAuthHistoryShape(snapshot: PrivateNavigationSnapshot, landingUrl: string): "replace" | "push" | null {
    ...
    const replaced =
      snapshot.entryIndex === predecessor.entryIndex + 1 &&
    ...
    const pushed =
      this.initialLandingEventCandidate?.sameUrlHistoryApiNotifications === 0 &&
  ...
  deriveInitialLandingFromAuthTransit(snapshot: PrivateNavigationSnapshot): boolean {
    if (this.initialLandingIdentity) return true;
  ...
  privateInitialAuthHandoff(snapshot: PrivateNavigationSnapshot | null | undefined): boolean {
    try {
      if (!snapshot || !isAuthFlowUrl(snapshot.exactUrl)) return false;
      if (!this.deriveInitialLandingFromAuthTransit(snapshot)) return false;
  ...
  clearUnretainedObservation(): void {
    this.refs.clear();
    this.lastObservedNodes = [];
  }

  admitPrivateClerkLocation(snapshot: PrivateNavigationSnapshot): ClerkLocationAdmission | null {
    const landing = this.initialLandingIdentity;
    if (!landing || snapshot.frameId !== landing.frameId || snapshot.loaderId !== landing.loaderId) return null;
  ...
  async confirmPrivateAuthLanding(admission: ClerkLocationAdmission): Promise<PrivateNavigationSnapshot | null> {
    ...
    while (Date.now() < deadline) {
      let snapshot: PrivateNavigationSnapshot;
      try {
        snapshot = await this.privateNavigationSnapshot();
      } catch {
        return null;
      ...
      if (this.isExactLanding(snapshot, expectedLandingSequence)) {
        ...
        if (remaining < AUTH_LANDING_SETTLE_MS) return null;
        await new Promise<void>((resolvePromise) => setTimeout(resolvePromise, AUTH_LANDING_SETTLE_MS));
        try {
      ...
      const stillAtAdmittedTransit =
        admission.kind === "transit" &&
        this.samePrivateLogicalNavigation(snapshot, admission.navigation);
      if (!stillAtAdmittedTransit) return null;
      await new Promise<void>((resolvePromise) => setTimeout(resolvePromise, Math.min(25, deadline - Date.now())));
    }
    return null;
  }

  async settleBeforeRouteDeadline<T>(deadline: number, operation: () => Promise<T>): Promise<{ ok: true; value: T } | { ok: false }>;
  async settleBeforeRouteDeadline(deadline: number, operation: () => unknown): Promise<{ ok: true; value: unknown } | { ok: false }>;
  async settleBeforeRouteDeadline(deadline: number, operation: () => unknown): Promise<{ ok: boolean; value?: unknown }> {
    if (!Number.isFinite(deadline) || Date.now() >= deadline) return { ok: false };
    let timer: ReturnType<typeof setTimeout> | undefined;
    const settled = Promise.resolve()
      .then(operation)
      .then((value) => ({ ok: true, value }), () => ({ ok: false }));
    const expired = new Promise<{ ok: boolean }>((resolvePromise) => {
  ...
  routeBindingRequestRelevant(rawUrl: unknown): boolean {
    try {
  ...
  async admitPrivateRouteState(deadline: number, { sealNetwork = false }: { sealNetwork?: boolean } = {}): Promise<BrokerResult> {
    if (this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    const initialDrain = await this.settleBeforeRouteDeadline(deadline, () => this.drainBackgroundAbortTasks());
    if (!initialDrain.ok || this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    ...
    if (!requestsSettled.ok || this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    const finalDrain = await this.settleBeforeRouteDeadline(deadline, () => this.drainBackgroundAbortTasks());
    if (!finalDrain.ok || this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    const loaded = await this.settleBeforeRouteDeadline(deadline, () => this.loadState());
    if (!loaded.ok || this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    const state: unknown = loaded.value;
    if (isRecord(state) && state.abort) return refusal("run_aborted", "The browser run is not live");
    ...
    if (
      !Number.isFinite(workingDayDeadline) ||
    ...
    if (!this.boundedCapsValid(state)) return refusal("run_aborted", "The browser run is not live");
    if (
      sealNetwork &&
  ...
  async confirmBoundedOnboardingScope(deadline: number, { initialSample = null, chain = { hops: 0 } }: {
    initialSample?: PrivateNavigationSnapshot | null;
    chain?: RouteChain;
  } = {}): Promise<PrivateNavigationSnapshot | null> {
    if (!Number.isFinite(deadline)) return null;
    let admittedTransit: PrivateNavigationSnapshot | null = null;
    let candidate: PrivateNavigationSnapshot | null = null;
    let pendingSample: PrivateNavigationSnapshot | null = initialSample;
    let stableSamples = 0;
    let stableSince: number | null = null;
    while (Date.now() < deadline) {
      if (this.activeNavigation) return this.noteBoundedRouteFailure("route_admission");
      const continuation = await this.admitPrivateRouteState(deadline);
      if (!continuation.ok || Date.now() >= deadline) return this.noteBoundedRouteFailure("route_admission");
      ...
      pendingSample = null;
      if (!sampled.ok || Date.now() >= deadline) return this.noteBoundedRouteFailure("route_admission");
      const sample = sampled.value;
      let sameOrigin = false;
      try {
        sameOrigin = normalizeOrigin(sample.exactUrl) === normalizeOrigin(this.config.initial_url);
      } catch {}
      if (!sameOrigin) return this.noteBoundedRouteFailure("route_off_origin");
      if (isAuthFlowUrl(sample.exactUrl)) {
        ...
        try { returnedToAuth ||= Boolean(chain.last && !isAuthFlowUrl(chain.last.exactUrl)); } catch {}
        if (
          returnedToAuth ||
        ...
        chain.last = sample;
        await new Promise<void>((resolvePromise) => setTimeout(resolvePromise, Math.max(0, Math.min(25, deadline - Date.now()))));
        continue;
      }
      const sameLogicalCandidate = candidate && this.samePrivateLogicalNavigation(candidate, sample);
      if (!this.advanceBoundedRouteChain(chain, sample)) return null;
      ...
      if (
        stableSamples >= this.boundedRouteStableSamples &&
        ...
        if (!finalSnapshot.ok || Date.now() >= deadline) return this.noteBoundedRouteFailure("route_admission");
        const final = finalSnapshot.value;
        if (!this.samePrivateLogicalNavigation(candidate, final)) {
          let finalSameOrigin = false;
          try { finalSameOrigin = normalizeOrigin(final.exactUrl) === normalizeOrigin(this.config.initial_url); } catch {}
          if (!finalSameOrigin) return this.noteBoundedRouteFailure("route_off_origin");
          if (isAuthFlowUrl(final.exactUrl)) return this.noteBoundedRouteFailure("route_returned_to_auth");
          if (!this.advanceBoundedRouteChain(chain, final)) return null;
          candidate = final;
          stableSamples = 0;
          stableSince = null;
          continue;
        ...
        if (!this.sameCurrentPrivateNavigation(final)) continue;
  ...
  async finalizeBoundedRouteBinding(candidate: PrivateNavigationSnapshot, deadline: number): Promise<PrivateNavigationSnapshot | null> {
    ...
    if (!beforeSnapshot.ok || Date.now() >= deadline) return null;
    ...
    if (!sealedSnapshot.ok || Date.now() >= deadline) return null;
    const sealed = sealedSnapshot.value;
    if (Date.now() >= deadline || !this.samePrivateNavigation(candidate, sealed)) return null;
  ...
  async confirmBoundedScopeExit(): Promise<PrivateNavigationSnapshot | null> {
    let first: PrivateNavigationSnapshot;
    try { first = await this.privateNavigationSnapshot(); } catch { return null; }
    const scopeHref: unknown = this.boundedScopeHref;
    if (!this.boundedOnboarding || first.exactUrl === scopeHref || isAuthFlowUrl(first.exactUrl) || normalizeOrigin(first.exactUrl) !== normalizeOrigin(typeof scopeHref === "string" ? scopeHref : "")) return null;
    await new Promise<void>((resolvePromise) => setTimeout(resolvePromise, AUTH_LANDING_SETTLE_MS));
    try {
  ...
  async boundedScopeIsCurrent(): Promise<boolean> {
    if (!this.boundedOnboarding || this.boundedScopeComplete) return false;
    try {
      const snapshot = await this.privateNavigationSnapshot();
      if (this.boundedScopeIdentity) return this.samePrivateNavigation(snapshot, this.boundedScopeIdentity);
  ...
  waitForTopFrameLoad(afterSequence: number): Promise<void> {
    if (this.topFrameLoadSequence > afterSequence) return Promise.resolve();
    const remaining = Math.min(this.initialTopFrameLoadTimeoutMs, this.absoluteDeadlineMs - Date.now());
    if (remaining <= 0) return Promise.reject(new Error("The initial page load deadline was reached"));
  ...
  async waitForClerkReadiness(deadline: number = this.absoluteDeadlineMs): Promise<unknown> {
  ...
  async lockClerkAfterPreTicketFailure(): Promise<void> {
    try {
      await this.clerkAuthenticator.lockMethods();
    } catch {}
  }

  async inspectClerkPreAuth(): Promise<BrokerResult> {
    ...
    let admission: ClerkLocationAdmission | null = null;
    try {
      admission = this.admitPrivateClerkLocation(await this.privateNavigationSnapshot());
    } catch {}
    if (!admission) return refusal("clerk_auth_policy_mismatch", "Clerk authentication is not admitted in the current browser state");
    let state: unknown;
    try {
      const ready: unknown = await this.waitForClerkReadiness();
      state = ready === true
        ? await this.callPageFunction(CLERK_PAGE_FUNCTIONS.preAuth)
        : null;
    } catch {}
    const continuation = await this.admitPrivateAuthContinuation();
    let finalAdmission: ClerkLocationAdmission | null = null;
    if (continuation.ok) {
      try {
  ...
  async authenticateClerkTicket(request: JsonRecord): Promise<BrokerResult> {
    const policy = this.config.clerk_auth;
    if (
      policy?.mode !== "one-use-ticket" ||
    ...
    const boundedHandoff: BoundedPostAuthHandoff | null = this.boundedOnboarding
      ? Object.freeze({
          kind: admission.kind,
    ...
    this.provisionalClerkSessionId = null;
    this.provisionalClerkTouchCount = 0;
    let result: Awaited<ReturnType<OneShotClerkPageAuthenticator["authenticate"]>>;
    try {
    ...
    const authOutcome: unknown = result;
    const outcomeFields = isRecord(authOutcome) ? authOutcome : null;
    const exactSessionBinding = Boolean(
      outcomeFields?.ok &&
    ...
    if (!("authenticated" in result)) return refusal("clerk_auth_failed", "Clerk authentication failed");
  ...
  async bindBoundedPostAuthRoute(): Promise<BrokerResult> {
    if (
      !this.boundedOnboarding ||
    ...
    this.boundedRouteFailurePoint = null;
    const fail = (stage: string, point: string | null = null): BrokerResult => {
      this.clerkTransportPhase = null;
      this.activeClerkSessionId = null;
      this.boundedPostAuthHandoff = null;
      this.boundedExpectedClerkUserId = null;
    ...
    const initialSample: { ok: true; value: PrivateNavigationSnapshot } | { ok: false } = initialAdmission.ok
      ? await this.settleBeforeRouteDeadline(deadline, () => this.privateNavigationSnapshot())
      : { ok: false };
    if (!initialAdmission.ok) return fail("candidate_wait", "initial_route_admission");
    if (!initialSample.ok) return fail("candidate_wait", "initial_route_snapshot");
    const initialNavigation = initialSample.value;
    let initialIsAuth = false;
    try { initialIsAuth = isAuthFlowUrl(initialNavigation.exactUrl); } catch { return fail("candidate_wait", "initial_route_snapshot"); }
    let candidate: PrivateNavigationSnapshot | null = null;
    if (initialIsAuth) {
      const admittedTransit = this.admitPrivateClerkLocation(initialNavigation);
      const exactPriorTransit = Boolean(
        postAuthHandoff.kind === "transit" &&
      ...
      if (!exactPriorTransit || this.boundedPostAuthResumeAttempted) return fail("candidate_wait", "auth_transit_lineage");
      const landingUrl: unknown = this.initialLandingIdentity?.exactUrl;
      if (typeof landingUrl !== "string") return fail("candidate_wait", "resume_target");
      let exactLandingTarget = false;
      try {
        exactLandingTarget = Boolean(
          new URL(landingUrl).href === landingUrl &&
      ...
      if (!exactLandingTarget || this.activeNavigation) return fail("candidate_wait", "resume_target");
      ...
      const explicitMismatch = Boolean(
        (authentication?.user_id != null && authentication.user_id !== this.boundedExpectedClerkUserId) ||
        (authentication?.session_id != null && authentication.session_id !== this.activeClerkSessionId)
      );
      if (explicitMismatch) return fail("exact_clerk_inspection", "clerk_initial_inspection");
      const exactAuthentication = Boolean(
        authentication?.user_id === this.boundedExpectedClerkUserId &&
      ...
      const stillCurrent: { ok: true; value: PrivateNavigationSnapshot } | { ok: false } = routeRecheck.ok
        ? await this.settleBeforeRouteDeadline(deadline, () => this.privateNavigationSnapshot())
        : { ok: false };
      if (!stillCurrent.ok) return fail("candidate_wait", "auth_transit_recheck");
      if (!this.sameAdmittedAuthTransit(initialNavigation, stillCurrent.value)) {
        ...
        if (!changedToNonAuth) return fail("candidate_wait", "auth_transit_recheck");
        ...
        const sealedTransit: { ok: true; value: PrivateNavigationSnapshot } | { ok: false } = beforeResume.ok
          ? await this.settleBeforeRouteDeadline(deadline, () => this.privateNavigationSnapshot())
          : { ok: false };
        const afterResumeSeal: BrokerResult = sealedTransit.ok
        ...
        try {
          ...
          const navigated = await this.settleBeforeRouteDeadline(
            deadline,
            () => this.cdp.send("Page.navigate", { url: landingUrl }, this.pageSessionId)
          );
          if (!navigated.ok || navigated.value?.errorText) return fail("candidate_wait", "resume_navigation");
        } finally {
    ...
    if (!candidate) return fail("candidate_wait");
    let failureStage = "exact_clerk_inspection";
    let finalDestination: PrivateNavigationSnapshot | null = null;
    let clerkLockedLoaderId = postAuthHandoff.navigation.loaderId;
    while (Date.now() < deadline) {
      if (this.activeNavigation) return fail(failureStage, "route_admission");
      const ready = await this.settleBeforeRouteDeadline(deadline, () => this.waitForClerkReadiness(deadline));
      if (!ready.ok || ready.value !== true) return fail(failureStage, "clerk_readiness");
      ...
      if (newPageContext) {
        ...
        if (!relocked.ok) return fail("post_inspection_reseal", "clerk_context_relock");
      ...
      if (explicitMismatch) return fail("exact_clerk_inspection", "clerk_final_inspection");
      const exactAuthentication = Boolean(
        authentication?.user_id === this.boundedExpectedClerkUserId &&
        authentication?.session_id === this.activeClerkSessionId &&
        authentication?.token_present === true
      ...
      if (exactAuthentication) {
        ...
        if (finalDestination && !this.backgroundAbortLatched) break;
      }
      if (Date.now() >= deadline || this.backgroundAbortLatched) return fail(failureStage, failureStage === "post_inspection_reseal" ? "clerk_final_reseal" : "clerk_final_inspection");
      const routeAdmission = await this.admitPrivateRouteState(deadline);
      const sampled: { ok: true; value: PrivateNavigationSnapshot } | { ok: false } = routeAdmission.ok
        ? await this.settleBeforeRouteDeadline(deadline, () => this.privateNavigationSnapshot())
        : { ok: false };
      if (!sampled.ok) return fail(failureStage, "route_admission");
      if (!this.samePrivateNavigation(candidate, sampled.value) || !this.sameCurrentPrivateNavigation(sampled.value)) {
        ...
        if (!candidate) return fail(failureStage);
      }
      const remaining = deadline - Date.now();
      if (remaining > 0) {
    ...
    if (!finalDestination) return fail(failureStage, failureStage === "post_inspection_reseal" ? "clerk_final_reseal" : "clerk_final_inspection");
  ...
  async privateIdentityMaterialAbsent(tree: unknown, rawMaterial: unknown[] = []): Promise<boolean> {
    if (!(this.boundedOnboarding && this.config.clerk_auth)) return true;
  ...
  async assertPrivateIdentityAbsent(tree: unknown, rawMaterial: unknown[] = []): Promise<void> {
    if (await this.privateIdentityMaterialAbsent(tree, rawMaterial)) return;
  ...
  async privatePostAuthReadinessSample(): Promise<PostAuthReadinessSample> {
    const continuation = await this.admitPrivateAuthContinuation();
    if (!continuation.ok) return { aborted: true };
    if (this.networkInflight.size !== 0) return null;
    const before = await this.privateNavigationSnapshot();
    if (!this.isExactLanding(before)) return false;
    ...
    const treeNodes: unknown[] = Array.isArray(tree.nodes) ? tree.nodes : [];
    const pageReady: unknown = await this.callPageFunction(CLERK_PAGE_FUNCTIONS.postAuthReady);
    ...
    const finalContinuation = await this.admitPrivateAuthContinuation();
    if (!finalContinuation.ok) return { aborted: true };
    if (!this.samePrivateNavigation(before, after) || !this.isExactLanding(after)) return false;
    ...
    const actionableRole = (node: unknown): string | null => {
      const fields = isRecord(node) ? node : null;
      if (fields?.ignored) return null;
    ...
    const accessibilityBusy = treeNodes.some((node) => {
      ...
      if (busyFields?.ignored) return false;
      const busyRole = isRecord(busyFields?.role) ? busyFields.role : null;
      if (busyRole?.value === "progressbar") return true;
      const busyProperties: unknown = busyFields?.properties;
      return Array.isArray(busyProperties) && busyProperties.some((busyProperty: unknown) => {
    ...
    const screenshotData: unknown = screenshot?.data;
    if (
      pageReady !== true ||
      !actionable ||
      accessibilityBusy ||
  ...
  async confirmPrivatePostAuthReadiness(): Promise<BrokerResult> {
    if (!this.firstPostAuthCapturePending || this.postAuthReadinessConfirmed) return { ok: true };
    ...
    const deadline: number = this.postAuthReadinessDeadlineMs!;
    let previous: string | null = null;
    let stableSamples = 0;
    let stableSince: number | null = null;
    while (Date.now() < deadline) {
      let sample: PostAuthReadinessSample | null;
      try {
      ...
      if (isRecord(sample) && "fingerprint" in sample && typeof sample.fingerprint === "string") {
        const fingerprint: string = sample.fingerprint;
        ...
        if (
          stableSamples >= POST_AUTH_STABLE_SAMPLES &&
  ...
  async privatePostAuthCertificateCurrent(): Promise<boolean> {
    if (!this.postAuthReadinessConfirmed || !this.postAuthReadinessFingerprint) return false;
    const continuation = await this.admitPrivateAuthContinuation();
    return Boolean(
      continuation.ok &&
      this.networkInflight.size === 0 &&
  ...
  postAuthReadinessError(response: unknown): Error {
  ...
  async restartPrivatePostAuthReadiness(): Promise<void> {
    ...
    if (!readiness.ok) throw this.postAuthReadinessError(readiness);
  }

  admitPrivateClerkBootstrapSnapshot(snapshot: PrivateNavigationSnapshot | null | undefined): ClerkLocationAdmission | null {
    if (!snapshot) return null;
    try {
      if (isAuthFlowUrl(snapshot?.exactUrl)) {
        if (!this.privateInitialAuthHandoff(snapshot)) return null;
  ...
  async privateClerkBootstrapNavigate(request: JsonRecord): Promise<BrokerResult> {
    ...
    let failurePoint: string = INITIAL_NAVIGATION_FAILURE_POINTS.bootstrap_admission;
    const failedAt = (response: BrokerResult, point: string = failurePoint): BrokerResult => ({ ...response, failure_point: point });
    try {
      ...
      if (!workerReservation.ok) return failedAt(workerReservation);
      ...
      if (dispatched?.errorText) throw new Error("Initial navigation dispatch was refused");
      ...
      if (!admission) throw new Error("The private Clerk bootstrap location was not admitted");
      let continuation = await this.admitPrivateAuthContinuation();
      if (!continuation.ok) return failedAt(continuation);
      admission = this.admitPrivateClerkLocation(await this.privateNavigationSnapshot());
      if (!admission) throw new Error("The private Clerk bootstrap location changed");
  ...
  inspectObservedControl(request: JsonRecord): BrokerResult {
    if (this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    const ref = String(request?.ref ?? "").replace(/^@/, "");
    const semantic = this.refs.get(ref);
    if (!this.lastEvidence || !semantic) return refusal("observed_control_missing", "The control is not present in the current retained observation");
    return {
      ok: true,
      control: {
  ...
  onCdpEvent(message: CdpMessage): void {
    ...
    if (method === "Network.requestWillBeSent") {
      ...
      const requestUrl: unknown = isRecord(params.request) ? params.request.url : undefined;
    ...
    if (method === "Target.attachedToTarget") {
      ...
      if (["worker", "shared_worker"].includes(info.type)) this.trackBackgroundAbort("unexpected_worker_target", "A dedicated or shared worker target caused a fail-closed abort");
      if (info.type === "service_worker") {
        if (!this.config.fixed_preflight_service_worker_request) this.trackBackgroundAbort("unexpected_service_worker", "A service worker target caused a fail-closed abort");
        ...
        try {
          const workerUrl: unknown = info.url;
          if (typeof workerUrl !== "string") throw new Error("Service worker target had an invalid origin");
          if (!this.allowedOrigins.has(normalizeOrigin(workerUrl))) this.trackBackgroundAbort("service_worker_origin_allowlist_breach", "Service worker origin was not allowed");
        } catch {
    ...
    if (method === "Fetch.requestPaused") {
      ...
      this.networkActivitySequence += 1;
      this.lastNetworkActivityAt = Date.now();
      const pausedRequestUrl: unknown = isRecord(params.request) ? params.request.url : undefined;
    ...
    if (method === "Page.loadEventFired") {
      ...
      for (const waiter of [...this.pageLoadWaiters]) waiter();
      return;
    }
    if (method === "Network.requestWillBeSent") {
      const params = eventParams(message);
      ...
      if (activeNavigation && params.type === "Document") {
        try {
          const redirectSourceUrl: unknown = redirectResponse.url;
          if (typeof redirectSourceUrl !== "string") throw new Error("redirect chain mismatch");
          if (new URL(redirectSourceUrl).href !== activeNavigation.last_url) throw new Error("redirect chain mismatch");
          const pausedRedirectRequest: unknown = params.request;
          const pausedRedirectUrl: unknown = isRecord(pausedRedirectRequest) ? pausedRedirectRequest.url : undefined;
          const redirected = new URL(typeof pausedRedirectUrl === "string" ? pausedRedirectUrl : "").href;
          if (activeNavigation.reject_redirects === true) throw new Error("redirect forbidden");
          if (this.allowedNavigationOrigins.has(normalizeOrigin(redirected))) activeNavigation.allowed_urls.add(redirected);
        } catch {
    ...
    if (method === "Page.frameNavigated") {
      ...
      if (!frameFields) throw new Error("CDP frame is unavailable");
      const frameId: unknown = frameFields.id;
      const frameLoaderId: unknown = frameFields.loaderId;
      if (typeof frameId !== "string" || typeof frameLoaderId !== "string") throw new Error("CDP frame is unavailable");
      ...
      try {
        const frameUrl: unknown = frameFields.url;
        if (typeof frameUrl !== "string") throw new Error("CDP frame URL is unavailable");
    ...
    if (method === "Page.navigatedWithinDocument") {
      const params = eventParams(message);
      if (params.frameId !== this.topFrameId) return;
      this.navigationSequence += 1;
      const urlSha256 = sha256Text(String(params.url ?? ""));
      const lastLogicalLocation = this.lastTopLogicalLocation;
      const duplicateLogicalLocation =
        params.navigationType === "historyApi" &&
      ...
      if (!duplicateLogicalLocation) {
        ...
        if (
          landingCandidate?.sequence === this.logicalNavigationSequence &&
      ...
      try {
        const documentUrl: unknown = params.url;
        if (typeof documentUrl !== "string") throw new Error("CDP document URL is unavailable");
  ...
  async configureSession(sessionId: string, type: string, resumeWaitingTarget: boolean): Promise<void> {
    try {
      ...
      try {
        const errorMessage: unknown = isRecord(error) ? error.message : undefined;
  ...
  boundedCapsValid(state: unknown): boolean {
  ...
  async admitPrivateAuthContinuation(deadline: number = this.absoluteDeadlineMs): Promise<BrokerResult> {
    for (;;) {
      if (this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
      const initialDrain = await this.settleBeforeRouteDeadline(deadline, () => this.drainBackgroundAbortTasks());
      if (!initialDrain.ok || this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
  ...
  async abort(code: string, detail: unknown, operatorDiagnostic: unknown = null): Promise<void> {
    ...
    await mutateCapState(this.config.cap_state_path, (state) => {
      if (!state.abort) {
        ...
        if (operatorDiagnostic) state.abort.operator_diagnostic = operatorDiagnostic;
      }
    });
  }
  ...
  trackBackgroundAbort(code: string, detail: unknown): void {
  ...
  boundedRequestReceipt(): BoundedRequestReceipt | null {
    const action = this.activeAction;
    if (action?.action_class !== BOUNDED_ONBOARDING_CLASS && !this.boundedPrivateAuthStart) return null;
  ...
  privateAuthUnknownMutationDiagnostic({ rawUrl, origin, method, receipt }: {
    ...
    origin: string;
    method: string;
    receipt: BoundedRequestReceipt | null;
  }): JsonRecord | null {
    if (!this.boundedOnboarding || !this.boundedPrivateAuthStart || receipt === null || receipt === undefined) return null;
    const targetOrigin = normalizeOrigin(this.config.initial_url);
    const clerkOrigin: unknown = this.config.clerk_auth?.frontend_api_origin;
    ...
    if (origin === clerkOrigin) {
      ...
      try {
        ...
        if (/^\/v1\/client\/sign_ins(?:\/|$)/.test(pathname)) requestClass = "clerk_sign_in";
  ...
  admitPrivateAuthBootstrapMutation({ method, href, origin, receipt }: {
    ...
    if (this.boundedPrivateAuthStart !== true || receipt?.suppress_retention !== true) return null;
    if (href === null || origin !== normalizeOrigin(this.config.initial_url)) return null;
  ...
  async drainBoundedRequestQueue(): Promise<void> {
    for (;;) {
      const pending = this.requestQueue;
      await pending;
      const state: unknown = await this.loadState();
      if (pending !== this.requestQueue) continue;
      if (!isRecord(state) || state.abort) throw new Error("A queued browser request was blocked");
      return;
    }
  }

  async armBoundedMutationWindow(): Promise<void> {
    this.assertEffectfulAdmissionOpen();
    ...
    this.assertEffectfulAdmissionOpen();
    const action = this.activeAction;
    if (action?.action_class !== BOUNDED_ONBOARDING_CLASS) throw new Error("Bounded action disappeared before dispatch");
    this.assertEffectfulAdmissionOpen();
  ...
  async closeBoundedMutationWindow(): Promise<void> {
    if (this.activeAction?.action_class !== BOUNDED_ONBOARDING_CLASS) return;
  ...
  async appendRequestEvent(event: JsonRecord, receipt: BoundedRequestReceipt | null = null): Promise<void> {
    if (this.boundedPrivateAuthStart || receipt?.suppress_retention === true) return;
    await appendJsonl(join(this.runDirectory, "request-events.jsonl"), event);
  }

  async handlePausedRequest(message: CdpMessage, receipt: BoundedRequestReceipt | null = null): Promise<void> {
    const params = eventParams(message);
    const requestId: unknown = params.requestId;
    const pausedRequest: unknown = params.request;
    const resourceType: unknown = params.resourceType;
    ...
    let origin: string;
    let pausedUrl: string;
    try {
      const pausedUrlProbe: unknown = isRecord(pausedRequest) ? pausedRequest.url : undefined;
      if (typeof pausedUrlProbe !== "string") throw new Error("invalid_request_origin");
    ...
    const pausedMethod: unknown = isRecord(pausedRequest) ? pausedRequest.method : undefined;
    ...
    const provisionalSessionId = this.config.clerk_auth
      ...
      : null;
    let provisionalSessionToBind: string | null = null;
    if (provisionalSessionId !== null) {
      const receiptAttempt: unknown = receipt?.clerk_transport_attempt;
      const receiptBootstrapDeadline: unknown = receipt?.clerk_bootstrap_deadline_ms;
      const provisionalAllowed =
        receiptAttempt === this.clerkTransportAttempt &&
    ...
    if (resourceType === "Document") {
      let exactDocumentUrl: string | null;
      try {
      ...
      if (this.activeNavigation) this.activeNavigation.last_url = exactDocumentUrl;
    }
    ...
    let matchedMutation: { method: string; url: string | null; bounded?: boolean } | null = null;
    let privateAuthAdmission: { id: string; remaining: number } | null = null;
    if (!["GET", "HEAD", "OPTIONS"].includes(method) && !clerkTransportMutation) {
      ...
      let exactActualUrl: string | null = null;
      try {
      ...
      privateAuthAdmission = !boundedMutation
        ...
        : null;
      if (!boundedMutation && !privateAuthAdmission && (
        this.activeAction?.action_class !== "Listed own-account progress" ||
    ...
    if (denied) {
      await this.cdp.send("Fetch.failRequest", { requestId, errorReason: "BlockedByClient" }, eventSessionId);
      if (!this.backgroundAbortLatched) await this.abort("browser_request_cap", "A request cap would be exceeded");
      return;
    }
    ...
    if (matchedMutation && this.activeAction) {
      ...
      if (matchedMutation.bounded) this.activeAction.same_origin_mutation_requests_dispatched = Number(this.activeAction.same_origin_mutation_requests_dispatched) + 1;
    }
  ...
  async reserveFixedFixtureWorkerRequest() {
    if (!this.config.fixed_preflight_service_worker_request || this.fixtureWorkerReserved) return { ok: true };
    if (this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    const workerUrl = this.config.fixed_preflight_service_worker_request;
    if (!this.allowedOrigins.has(normalizeOrigin(workerUrl))) return refusal("fixture_worker_origin_not_allowed", "Fixed worker request origin is not allowed");
    const timestamp = monotonicSeconds();
    const rolling = rollingCount(this.requestTimes, timestamp);
    let denied = false;
    await mutateCapState(this.config.cap_state_path, (state) => {
  ...
  async reserveOperation(request: JsonRecord): Promise<BrokerResult> {
    if (this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    const now = monotonicSeconds();
    const rolling = rollingCount(this.operationTimes, now);
    let denied: BrokerResult | null = null;
    let resultingState: unknown;
    await mutateCapState(this.config.cap_state_path, (state) => {
      if (this.backgroundAbortLatched) denied = refusal("run_aborted", "The browser run is not live");
      else if (state.abort) denied = refusal("run_aborted", state.abort.code);
      else if (Date.now() >= Date.parse(state.working_day_deadline)) denied = refusal("working_day_deadline", "Working-day deadline reached");
      else {
        if (!state.browser.started_at) state.browser.started_at = now;
        state.browser.active_seconds = now - state.browser.started_at;
        if (state.browser.active_seconds > state.caps.browser_active_seconds) denied = refusal("browser_time_cap", "Browser time cap reached");
        else if (state.browser.operations + 1 > state.caps.browser_operations_total) denied = refusal("browser_operation_cap", "Browser operation cap reached");
        else if (rolling + 1 > state.caps.browser_operations_per_rolling_minute) denied = refusal("browser_operation_rate_cap", "Browser operation rate cap reached");
      }
      if (!denied && request.action_class === "Listed own-account progress") {
        ...
        if (!ledgerClass || registered !== request.effect_class_id) denied = refusal("unproven_one_way_effect", "No owner-proven effect registry entry admits this action");
        else {
          const count = state.app.class_counts[ledgerClass.id] ?? 0;
          const reservation = ledgerClass.worst_case_eur_per_action;
          if (count + 1 > ledgerClass.maximum_count || state.app.one_way_actions + 1 > state.caps.listed_one_way_actions_total) denied = refusal("one_way_action_cap", "One-way action cap reached");
          else if (state.app.actual_eur + state.app.outstanding_reservations_eur + reservation > state.caps.app_side_cost_cap_eur) denied = refusal("app_cost_cap", "App-side cost cap would be exceeded");
          else if (combinedReservedAndActual(state) + reservation > state.caps.combined_actual_plus_reserved_cap_eur) denied = refusal("combined_cost_cap", "Combined cost cap would be exceeded");
          else {
      ...
      if (!denied && request.action_class === BOUNDED_ONBOARDING_CLASS) {
        ...
        if (!this.boundedOnboarding || count + 1 > 20 || state.app.one_way_actions + 1 > 20 || state.app.one_way_actions + 1 > state.caps.listed_one_way_actions_total) denied = refusal("one_way_action_cap", "Bound-route action cap reached");
        else {
          state.app.one_way_actions += 1;
  ...
  validateAction(request: JsonRecord): BrokerResult {
    if (typeof request.method !== "string" || !ALLOWED_METHODS.has(request.method)) return refusal("method_not_exposed", "The browser broker does not expose this operation");
    if (["ping", "metrics", "close"].includes(request.method)) return { ok: true };
    if (this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    ...
    if (expected && request.action_class !== expected) return refusal("action_matrix_mismatch", `Operation requires ${expected}`);
    ...
    if (this.boundedOnboarding && (request.method === "type" || (request.method === "click" && request.action_class === "Observe"))) return refusal("bounded_action_forbidden", "Typing and link-following are outside the bound route action boundary");
    if (
      (request.method === "type" || request.method === "scroll" || (request.method === "click" && request.action_class === "Observe")) &&
  ...
  async accessibilityObservation(exactUrl: string, providedTree: JsonRecord | null = null): Promise<{
    url: string;
    visible_state_summary: string;
    refs: number;
  }> {
    ...
    this.refs.clear();
    const observedNodes: Array<{ ref: string; role: string; name: string }> = [];
    this.lastObservedNodes = observedNodes;
    const candidates: Array<{ ref: string; role: string; name: string; backendDOMNodeId: unknown }> = [];
    const treeNodes: unknown[] = Array.isArray(tree.nodes) ? tree.nodes : [];
    let sequence = 0;
    for (const node of treeNodes) {
      const nodeFields = isRecord(node) ? node : null;
      const nodeRole = isRecord(nodeFields?.role) ? nodeFields.role : null;
      if (nodeFields?.ignored || typeof nodeRole?.value !== "string") continue;
      const role: string = nodeRole.value;
      ...
      if (!name && !["textbox", "searchbox", "button", "link"].includes(role)) continue;
      const ref = `e${++sequence}`;
      const backendDOMNodeId: unknown = nodeFields?.backendDOMNodeId;
      if (backendDOMNodeId) this.refs.set(ref, { backendDOMNodeId, role, name });
    ...
    const send = (method: string, params: JsonRecord): Promise<JsonRecord> => this.cdp.send(method, params, this.pageSessionId);
    const onScreenFlags = await Promise.all(
      candidates.map((candidate) => isNodeOnScreen(send, candidate.backendDOMNodeId)),
    );
    const lines = candidates.map((candidate, index) =>
      formatAxLine(candidate.ref, candidate.role, candidate.name, { onScreen: onScreenFlags[index] }).slice(0, 500),
    );
  ...
  async observe(): Promise<{
    ...
    const before = await this.privateNavigationSnapshot();
    if (isAuthFlowUrl(before.exactUrl)) throw new Error("Auth-flow evidence retention is forbidden");
    const observation = await this.accessibilityObservation(before.exactUrl);
    const after = await this.privateNavigationSnapshot();
    if (!this.samePrivateNavigation(before, after)) throw new Error("The page changed during observation");
    this.currentUrl = after.exactUrl;
    return observation;
  }

  async refreshCurrentUrlFromHistory(): Promise<string> {
    const snapshot = await this.privateNavigationSnapshot();
  ...
  async waitForAccessibilitySettle(maxWaitMs = 2000, pollIntervalMs = 150): Promise<JsonRecord | null> {
    const deadline = Math.min(Date.now() + maxWaitMs, this.absoluteDeadlineMs);
    let previousHash: string | null = null;
    let previousTree: JsonRecord | null = null;
    while (Date.now() < deadline) {
      if (this.networkInflight.size === 0 && Date.now() - this.lastNetworkActivityAt >= POST_AUTH_NETWORK_QUIET_MS) {
        ...
        if (hash === previousHash) return tree;
        previousHash = hash;
        previousTree = tree;
      }
  ...
  async captureEvidence(eventId: string, position: string, expectedNavigation: PrivateNavigationSnapshot | null = null): Promise<CapturedEvidence | typeof PRIVATE_AUTH_HANDOFF> {
    if (
      this.firstPostAuthCapturePending &&
    ...
    const before = await this.privateNavigationSnapshot();
    if (expectedNavigation && !this.samePrivateNavigation(before, expectedNavigation)) throw new Error("The bounded scope-exit destination changed before evidence");
    const initialLandingCapture =
      position === "after" && this.initialNavigationConsumed && !this.initialLandingIdentity;
    const privateInitialCapture =
      position === "after" &&
      this.initialNavigationConsumed &&
    ...
    if (privateInitialCapture) {
      ...
      if (!this.bindInitialLanding(before)) throw new Error("The initial landing identity is unavailable");
    ...
    let observationTree: JsonRecord | null = null;
    if (this.firstPostAuthCapturePending && this.postAuthReadinessConfirmed) {
      ...
      const observationNodes: AxNode[] = Array.isArray(observationTree.nodes) ? observationTree.nodes : [];
    ...
    const screenshotData: unknown = captured.data ?? "";
    if (typeof screenshotData !== "string") throw new Error("The screenshot capture is unavailable");
    if (
      this.firstPostAuthCapturePending &&
      this.postAuthReadinessConfirmed &&
    ...
    if (expectedNavigation && !this.samePrivateNavigation(afterScreenshot, expectedNavigation)) throw new Error("The bounded scope-exit destination changed during evidence");
    if (
      this.firstPostAuthCapturePending &&
      this.postAuthReadinessConfirmed &&
    ...
    if (this.firstPostAuthCapturePending) this.firstPostAuthCapturePending = false;
    return evidence;
  }

  async calibrationPrivateState(): Promise<CalibrationCapture> {
    const before = await this.privateNavigationSnapshot();
    if (isAuthFlowUrl(before.exactUrl)) throw new Error("Auth-flow evidence retention is forbidden");
    ...
    const after = await this.privateNavigationSnapshot();
    if (!this.samePrivateNavigation(before, after)) throw new Error("The page changed during private calibration observation");
    const normalizedCookies = (Array.isArray(cookies.cookies) ? cookies.cookies : []).map((cookie: unknown) => {
  ...
  async calibrationObservation(position: string): Promise<JsonRecord> {
    const evidence = await this.captureEvidence("calibration", position);
    if (!isRecord(evidence)) throw new Error("Auth-flow evidence retention is forbidden");
    ...
    const bindings: Array<{
      ref: string;
      public: { role: string; accessible_name: string; nearby_visible_text: string[] };
    }> = [];
    for (let index = 0; index < nodes.length && bindings.length < 30; index += 1) {
      const node = nodes[index];
      if (node.role !== "button" || !this.refs.has(node.ref)) continue;
  ...
  calibrationSelectedButton(selection: unknown, expectedAction: string): JsonRecord | null {
    ...
    if (selection.action !== expectedAction || selection.observation_sha256 !== boundObservation?.observation_sha256) return null;
    const buttons: unknown = bindingsRecord?.buttons ?? [];
    const matches = (Array.isArray(buttons) ? buttons : []).filter((binding: unknown) => {
      ...
      return (
        publicFields?.role === selection.role &&
  ...
  async calibrationObserveStart(): Promise<BrokerResult> {
    if (!this.calibrationEnabled || this.calibrationBaseline) return refusal("calibration_state_invalid", "Calibration is unavailable");
    const continuation = await this.admitPrivateAuthContinuation();
    ...
    if (this.config.clerk_auth) {
      ...
      if (!exactLanding) return refusal("calibration_start_url_changed", "The authenticated start URL changed");
    }
    const evidenceContinuation = await this.admitPrivateAuthContinuation();
    if (!evidenceContinuation.ok) return evidenceContinuation;
    ...
    try {
      ...
      if (readinessRefusal) return readinessRefusal;
      throw error;
    }
    ...
    if (
      baseline.raw_url_sha256 !== expectedLanding ||
  ...
  async calibrationOpen(selection: unknown): Promise<BrokerResult> {
    if (!this.calibrationEnabled || !this.calibrationBaseline || this.calibrationOpenState) return refusal("calibration_state_invalid", "Calibration is unavailable");
    let continuation = await this.admitPrivateAuthContinuation();
    if (!continuation.ok) return continuation;
    const selected = this.calibrationSelectedButton(selection, "probe");
    if (!selected) return refusal("calibration_selection_invalid", "The candidate is not exactly bound to the retained observation");
    ...
    const activated: unknown = await this.callPageFunction(CALIBRATION_PAGE_FUNCTIONS.activate, [], true);
  ...
  async calibrationReverse(selection: unknown): Promise<BrokerResult> {
    if (!this.calibrationEnabled || !this.calibrationOpenState) return refusal("calibration_state_invalid", "Calibration is unavailable");
    let continuation = await this.admitPrivateAuthContinuation();
    if (!continuation.ok) return continuation;
    const selected = this.calibrationSelectedButton(selection, "reverse");
    if (!selected) return refusal("calibration_selection_invalid", "The reverse control is not exactly bound to the retained open observation");
    const fresh = await this.calibrationPrivateState();
  ...
  async issueActionToken(request: JsonRecord): Promise<BrokerResult> {
    if (this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    if (typeof request.action_class !== "string" || !["Reversible own-account", "Listed own-account progress", BOUNDED_ONBOARDING_CLASS].includes(request.action_class)) return refusal("token_class_forbidden", "Only admitted clicks may receive a token");
    const ref = String(request.ref ?? "").replace(/^@/, "");
    const semantic = this.refs.get(ref);
    const evidence = this.lastEvidence;
    if (!evidence || !semantic) return refusal("token_binding_missing", "A current retained observation and semantic reference are required");
    ...
    const rawExpectedRequest: unknown = request.expected_request;
    ...
    const semanticRole: unknown = semantic?.role;
    if (
      bounded &&
      (!this.boundedOnboarding ||
    ...
    let expectedRequest: { method: string; url: string } | null = null;
    if (rawExpectedRequest) {
      const rawExpectedMethod: unknown = isRecord(rawExpectedRequest) ? rawExpectedRequest.method : undefined;
      const method = String(rawExpectedMethod ?? "").toUpperCase();
      if (!["POST", "PUT", "PATCH", "DELETE"].includes(method)) return refusal("token_request_method_invalid", "Bound request must name one mutation-capable method");
      const rawExpectedUrl: unknown = isRecord(rawExpectedRequest) ? rawExpectedRequest.url : undefined;
      const exact = new URL(typeof rawExpectedUrl === "string" ? rawExpectedUrl : "");
      if (exact.hash) return refusal("token_request_url_invalid", "Bound request URL must not contain a fragment");
      expectedRequest = { method, url: exact.href };
      if (!this.allowedOrigins.has(normalizeOrigin(expectedRequest.url))) return refusal("token_request_origin_invalid", "Bound request origin is not allowed");
    }
    if (this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
  ...
  consumeActionToken(request: JsonRecord): BrokerResult {
    const authorizationToken: unknown = request.authorization_token;
    const token = typeof authorizationToken === "string" ? this.actionTokens.get(authorizationToken) : undefined;
    if (typeof authorizationToken === "string") this.actionTokens.delete(authorizationToken);
    const ref = String(request.ref ?? "").replace(/^@/, "");
    const semantic = this.refs.get(ref);
    if (
      !token ||
  ...
  async clickRef(ref: unknown, beforeDispatch: (() => unknown) | null = null): Promise<void> {
    const target = this.refs.get(String(ref).replace(/^@/, ""));
    if (!target) throw new Error("Unknown or stale semantic reference");
    this.assertEffectfulAdmissionOpen();
    ...
    const quad: unknown = isRecord(box.model) ? box.model.border : undefined;
  ...
  async followObservedLink(ref: unknown): Promise<void> {
    const target = this.refs.get(String(ref).replace(/^@/, ""));
    if (!target || target.role !== "link") throw new Error("Observe-class click requires a visible link reference");
    ...
    const rawAttributes: unknown = node?.attributes;
    const attributeList: unknown[] = Array.isArray(rawAttributes) ? rawAttributes : [];
    const attributes: Record<string, unknown> = Object.fromEntries(Array.from({ length: attributeList.length / 2 }, (_, index) => [attributeList[index * 2], attributeList[index * 2 + 1]]));
    ...
    const href: unknown = attributes.href;
    if (typeof href !== "string") throw new Error("Observe-class click is limited to an ordinary same-window anchor");
    const destination = new URL(href, this.currentUrl).href;
    if (!this.allowedNavigationOrigins.has(normalizeOrigin(destination))) throw new Error("Observed link destination origin is not navigable");
  ...
  async execute(request: JsonRecord, caller = "sandbox"): Promise<BrokerResult> {
    if (typeof request.method === "string" && ["calibration_observe_start", "calibration_open", "calibration_reverse"].includes(request.method)) {
      if (caller !== "supervisor" || !this.calibrationEnabled) return refusal("method_not_exposed", "Calibration methods are not exposed");
      if (request.method === "calibration_observe_start") return this.calibrationObserveStart();
      if (request.method === "calibration_open") return this.calibrationOpen(request.selection);
      return this.calibrationReverse(request.selection);
    }
    if (request.method === "authenticate_clerk_ticket") {
      if (caller !== "supervisor") return refusal("method_not_exposed", "Only the supervisor may authenticate Clerk");
      return this.authenticateClerkTicket(request);
    }
    if (request.method === "inspect_clerk_pre_auth") {
      if (caller !== "supervisor") return refusal("method_not_exposed", "Only the supervisor may inspect pre-authentication state");
      return this.inspectClerkPreAuth();
    }
    if (request.method === "bind_bounded_post_auth_route") {
      if (caller !== "supervisor") return refusal("method_not_exposed", "Only the supervisor may bind the authenticated route");
      return this.bindBoundedPostAuthRoute();
    }
    if (request.method === "install_transient_auth") {
      if (caller !== "supervisor") return refusal("method_not_exposed", "Only the supervisor may install transient authentication");
      return this.installTransientAuth(request);
    }
    if (request.method === "clear_transient_auth") {
      if (caller !== "supervisor") return refusal("method_not_exposed", "Only the supervisor may clear transient authentication");
      return this.clearTransientAuth();
    }
    if (request.method === "inspect_observed_control") {
      if (caller !== "supervisor") return refusal("method_not_exposed", "Only the supervisor may inspect a retained control binding");
      return this.inspectObservedControl(request);
    }
    if (request.method === "issue_action_token") {
      if (caller !== "supervisor") return refusal("method_not_exposed", "Only the supervisor may issue action tokens");
      return this.issueActionToken(request);
    }
    const validation = this.validateAction(request);
    if (!validation.ok) return validation;
    if (this.boundedOnboarding && this.boundedScopeComplete && request.method !== "metrics" && request.method !== "close") return refusal("scope_complete", "The run has already left its bound route");
    const privateBootstrapNavigation =
      caller === "supervisor" && request.method === "navigate" && Boolean(this.config.clerk_auth);
    ...
    if (request.method === "navigate") {
      let exactUrl: string | null;
      try {
        const navigateUrl: unknown = request.url;
        exactUrl = new URL(typeof navigateUrl === "string" ? navigateUrl : "").href;
      } catch {
        exactUrl = null;
      }
      if (this.initialNavigationConsumed || exactUrl !== new URL(this.config.initial_url).href) return refusal("direct_navigation_locked", "Only the exact configured initial URL may be directly navigated once");
    }
    ...
    let tokenBinding: JsonRecord | null = null;
    if (request.method === "click" && request.action_class !== "Observe") {
      const consumed = this.consumeActionToken(request);
      if (!consumed.ok) return consumed;
      const consumedBinding: unknown = consumed.binding;
      if (!isRecord(consumedBinding)) return refusal("action_token_invalid", "Supervisor token is absent, stale, reused, or does not match the observed action");
    ...
    if (this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    if (privateBootstrapNavigation) return this.privateClerkBootstrapNavigate(request);
    const readiness = await this.confirmPrivatePostAuthReadiness();
    if (!readiness.ok) return readiness;
    if (this.backgroundAbortLatched) return refusal("run_aborted", "The browser run is not live");
    const eventId = `event-${String(++this.eventSequence).padStart(4, "0")}`;
    let before: CapturedEvidence | typeof PRIVATE_AUTH_HANDOFF;
    try {
    ...
    try {
      if (request.method === "navigate") {
        this.initialNavigationConsumed = true;
        const workerReservation = await this.reserveFixedFixtureWorkerRequest();
        if (!workerReservation.ok) return workerReservation;
        const rawNavigateUrl: unknown = request.url;
        ...
        await new Promise<void>((resolvePromise) => setTimeout(resolvePromise, 700));
        await this.requestQueue;
        outcome = "navigated";
        ...
        if (request.action_class === "Observe") await this.followObservedLink(request.ref);
        else await this.clickRef(request.ref, request.action_class === BOUNDED_ONBOARDING_CLASS ? () => this.armBoundedMutationWindow() : null);
        await new Promise<void>((resolvePromise) => setTimeout(resolvePromise, 300));
        await this.requestQueue;
        if (request.action_class === BOUNDED_ONBOARDING_CLASS && (await this.refreshCurrentUrlFromHistory()) !== this.boundedScopeHref) {
          ...
          if (!destination) throw new Error("The destination outside the bound route did not settle");
          this.boundedPendingScopeExit = destination;
        }
        outcome = "clicked";
      } else if (request.method === "type") {
        if (!request.synthetic || typeof request.text !== "string" || Buffer.byteLength(request.text) > 512) throw new Error("Typing requires bounded synthetic text");
        const target = this.refs.get(String(request.ref).replace(/^@/, ""));
        if (!target || typeof target.role !== "string" || !["textbox", "searchbox"].includes(target.role)) throw new Error("Typing requires a visible text input reference");
        this.assertEffectfulAdmissionOpen();
      ...
      let after: CapturedEvidence | typeof PRIVATE_AUTH_HANDOFF;
      try {
      ...
      if (request.action_class === BOUNDED_ONBOARDING_CLASS) await this.closeBoundedMutationWindow();
      ...
      const postActionState: unknown = await this.loadState();
      if (!isRecord(postActionState)) throw new Error("Browser operation failed");
  ...
  async recordEvent(
    request: JsonRecord,
    eventId: string,
    before: CapturedEvidence | typeof PRIVATE_AUTH_HANDOFF | null,
    after: CapturedEvidence | typeof PRIVATE_AUTH_HANDOFF | null,
    outcome: string,
    detail: unknown,
    ...
    const effectEvidence = request.action_class === BOUNDED_ONBOARDING_CLASS
      ...
      : null;
    const state: unknown = await this.loadState();
    if (!isRecord(state)) throw new Error("Browser operation failed");
    ...
    if (!browserState || !appState || !modelState) throw new Error("Browser operation failed");
  ...
  async shutdown(): Promise<boolean | undefined> {
    const waitForChromeExit = async (timeoutMs: number): Promise<boolean> => {
      if (!this.chrome || this.chrome.exitCode !== null || this.chrome.signalCode !== null) return true;
    ...
    if (!(this.boundedOnboarding && this.config.clerk_auth)) {
      if (this.chrome && this.chrome.exitCode === null) this.chrome.kill("SIGTERM");
      return;
    }
    if (await waitForChromeExit(1_000)) return true;
...
export async function recordRefusal(capStatePath: string, operation: unknown, code: unknown): Promise<void> {
  const entry = {
...
export async function announceBrowserBrokerReadiness(broker: BrowserBroker, output: NodeJS.WritableStream = process.stdout): Promise<{
  ok: boolean;
  message: { ready: boolean; broker: string; error?: string };
  error?: unknown;
}> {
  try {
    await broker.start();
    ...
    const errorDetail: unknown = isRecord(error) ? error.message : undefined;
...
async function main(): Promise<void> {
  ...
  const rawConfig: unknown = await readJson(configPath);
  if (!isBrokerConfig(rawConfig)) throw new Error("The browser broker config is invalid");
  ...
  if (readiness.ok) {
    ...
    attachJsonLineReader(process.stdin, (message) => {
      inboundQueue = inboundQueue.catch(() => {}).then(async () => {
        if (!isRecord(message)) throw new Error("The browser broker request is invalid");
        const payload = isRecord(message.payload) ? message.payload : {};
        let response: BrokerResult;
        try {
        ...
        if (response?.ok === false && response.refusal?.code) {
          const payloadMethod: unknown = payload.method;
